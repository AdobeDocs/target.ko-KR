...
description: 모바일 뷰포트는 Adobe [!DNL Target] 활동이 다양한 크기의 화면에서 표시되는 방식을 보는 데 유용합니다. 인기 있는 장치 뷰포트 크기 및 해상도 목록을 찾습니다.
...
>[!NOTE]
>
> * 사이트가 응답하고 데스크톱 페이지에 있는 것과 동일한 요소가 다른 구성으로 모바일 페이지에서 사용되는 경우 모바일 뷰포트를 사용하십시오. `m.mysite.com`과 같은 별도의 구조를 사용하는 별도의 모바일 사이트가 있는 경우 대신 [다중 페이지 활동](/help/c-experiences/c-visual-experience-composer/multipage-activity.md#concept_277E096063E14813AC5D8EDFA1D2ED48)을 사용하십시오.
   >
   >
...
다음은 인기 있는 장치의 뷰포트 및 해상도입니다. [!DNL Target]에서 뷰포트 크기를 사용해야 합니다.

>[!NOTE]
>
>다양한 웹 사이트가 많이 사용되는 장치에 대한 뷰포트 크기 목록을 제공합니다. 예를 들어 `https://viewportsizer.com/devices/` 을 참조하십시오. 가장 정확하고 최신 정보는 장치 제조업체의 웹 사이트를 참조하십시오.
...
응답형 사이트의 경우 일반적으로 사이트는 특정 화면 크기가 있는 장치가 액세스할 때 다른 보기에서 열리도록 설계되었습니다. 새 보기를 트리거하는 이러한 화면 크기는 CSS 중단점으로 알려져 있습니다. CSS 중단점은 방문자에게 최적의 레이아웃을 표시하기 위해 웹 사이트 컨텐츠가 장치 너비에 따라 응답하는 지점입니다. CSS 중단점을 [미디어 쿼리](https://developer.mozilla.org/en-US/docs/Web/CSS/Media_Queries/Using_media_queries)라고도 합니다.

정의한 각 보기에 대한 경험을 미리 볼 수 있도록 CSS 중단점을 [!DNL Target]에 저장합니다. 이러한 각 경험은 [!DNL Target] 인터페이스의 모바일 뷰포트에 표시됩니다. 디스플레이 상단에 있는 해당 뷰포트를 클릭하여 각 화면 크기에 대한 보기를 여십시오.

사이트가 응답하지 않으면, 활동이 특정 장치를 타깃팅하는 경우 모바일 웹 작성기를 사용하여 사이트를 보십시오.

>[!IMPORTANT]
>
>모바일 뷰포트 내에서 경험을 편집할 수 있습니다. 그러나 이러한 변경 사항은 작업 중인 뷰포트뿐만 아니라 모든 뷰포트 및 장치에 적용됩니다. 마찬가지로, 일반 데스크톱 보기에서 경험을 편집하면 페이지가 데스크톱 보기뿐만 아니라 모든 화면 크기용으로 변경됩니다. 현재 [!DNL Target]은 뷰포트별 페이지 변경을 지원하지 않습니다.
...
1. **[!UICONTROL 모바일 뷰포트 구성]** 섹션에서 **[!UICONTROL 추가]**&#x200B;를 클릭합니다.
   ...
   기존 모바일 뷰포트의 구성을 변경하려면 해당 뷰포트를 선택하고 [!UICONTROL 편집] (연필) 아이콘을 클릭합니다.
...
1. (선택 사항) 장치의 운영 체제를 선택합니다.
   ...
   * Blackberry
   ...
   [고급 경험 작성기](/help/c-experiences/experiences.md#section_34265986611B4AB8A0E4D6ACC25EF91D)를 사용하고 운영 체제를 선택하는 경우, [!DNL Target]는 페이지를 볼 때 해당 장치를 에뮬레이션합니다. 예를 들어 응답형 사이트에서 iOS용의 모양과 느낌이 다른 경우 [!DNL Target]은 해당 동작을 모방합니다.
...
모바일 화면을 위한 응답형 경험을 만들려면 모바일 뷰포트를 [!DNL Target] 활동에 추가하십시오.
...
1. **[!UICONTROL 장치]** 아이콘을 클릭한 다음, 모바일 뷰포트가 필요한 각 장치를 활성화합니다.
   ...
   ![모바일 뷰포트 활성화](/help/c-experiences/c-visual-experience-composer/assets/mobileviewports.png)
...
1. 원하는 대로 모바일 뷰포트를 편집합니다.
   ...
   ![iPhone 11 Pro Max 응답형 경험](/help/c-experiences/c-visual-experience-composer/assets/iphone11.png)
