---
description: Adobe Target에서 사용되는 용어의 정의입니다.
seo-description: 용어 정의입니다.
seo-title: Adobe Target 용어 설명
solution: Target
title: 용어 설명
topic: Standard
...
활동을 변경한 사람과 변경 사항이 발생했을 때의 레코드.
...
방문자가 활동의 첫 번째 디스플레이 mbox를 처음 본 시점부터 시작하여 사이트에서 방문한 페이지에 할당된 값에 따라 집계된 점수를 계산하는 참여 지표입니다.
...
자동으로 사용자가 방문하는 카테고리를 캡처하고 카테고리에 대한 사용자의 친화성을 계산하여 카테고리를 타깃팅하고 세그먼트화할 수 있습니다. 이 기능은 해당 정보에 대해 행동할 가능성이 가장 큰 방문자를 타겟으로 컨텐츠를 지정합니다.
...
모든 테스트 또는 활동 유형의 일부일 수 있습니다. 추가적인 프로그래밍이 필요하지 않습니다. 도메인에서 방문자 세션은 유지됩니다.

**동적 mbox**

DHTML 및 AJAX와 같은 기술을 사용하여 페이지가 이미 로드된 후 리치 인터넷 애플리케이션 (RIA) 를 사용하여 HTML를 조작할 수 있습니다.
...
사이트에서 보낸 방문자 관심의 측정으로, 사이트에서 시간, 클릭 수, 전환 및 기타 참여 지표로 측정됩니다.
...
오퍼, 이미지, 텍스트, 단추, 비디오, 페이지, 전체 웹 페이지 또는 구매 단계 또는 다른 논리적 페이지 순서를 형성할 수 있는 일련의 페이지에 대한 이러한 여러 요소의 조합입니다. 음성 도우미, 고객 서비스 스크립트 또는 음료대의 개인화된 향에 대한 답변일 수도 있습니다. You test or personalize experiences in [!DNL Target] activities.
...
방문자의 지리적 위치에 따라 활동 및 경험을 타깃팅할 수 있습니다. 지리 기반 타깃팅 매개 변수라고도 합니다.
...
랜딩 페이지 활동을 사용하면 타깃팅을 사용하여 방문에 대해 서로 다른 랜딩 페이지 컨텐츠를 표시할 수 있습니다. 그렇지 않으면 랜딩 페이지에 방문별로 동일한 컨텐츠가 표시됩니다. 랜딩 페이지 활동은 여러 버전의 페이지를 비교하여 더 성공적인 결과를 만들어내는 랜딩 페이지 버전을 확인하는 데 도움이 됩니다. Target Standard에서 *경험 타깃팅*&#x200B;으로 대체되었습니다.
...
URL이 아닌 페이지로 전달됩니다. 매개 변수는 데이터베이스에서 데이터를 전달하고 영업 데이터를 추적하거나 Adobe Recommendations를 실행하는 데 사용됩니다. 페이지 매개 변수라고도 합니다.
...
오퍼는 활동 및 테스트 동안 mbox 내에 표시되는 컨텐츠입니다. 경험은 특정 조건이 충족될 경우 표시되는 오퍼를 결정합니다.

**옵트아웃 링크**

방문자가 모든 추적 및 컨텐츠 제공을 옵트아웃할 수 있습니다.
...
참조 URL 매개 변수 및 값으로 대상 페이지의 mbox에 값을 전달합니다. 예를 들어 방문자의 원본을 기준으로 타깃팅할 수 있습니다.
...
보고서는 데이터를 기반으로 결정을 내리는 데 도움이 되는 활동의 진행 상황과 결과에 대한 정보를 제공합니다. 보고서 데이터를 통해 테스트 종료 시기를 결정하고 우승자인지 여부를 보여주며 다음 작업을 판별하는 데 필요한 통찰력을 제공할 수 있습니다.
...
활동을 타깃팅하는 데 사용되는 지정된 기준 세트입니다. 기준을 충족하는 방문자만 해당 세그먼트에 타깃팅된 활동의 컨텐츠를 봅니다. 일부 세그먼트는 여러 활동 간에 다시 사용할 수 있으며 다른 세그먼트는 활동에 고유합니다.
...
단일 페이지 애플리케이션 (SPA) 는 데스크탑 애플리케이션과 유사한 보다 유연한 사용자 경험을 제공하기 위해 단일 웹 페이지에 맞는 웹 애플리케이션이나 웹 사이트입니다.

**성공 지표**

활동의 성공을 측정하는 데 사용되는 매개 변수입니다. 성공 지표에는 주어진 경험의 성공 또는 타겟 활동에서 오퍼를 제공할 수 있는 주요 비즈니스 측정이 포함됩니다. 예를 들어 장바구니에 새 오퍼를 추가하거나 품목을 장바구니에 추가하면 방문자당 매출이 증가하는지 확인할 수 있습니다. 성공 지표는 등록, 주문 또는 구매 흐름과 관련된 문제를 발견하는 데 유용하며 방문자나 고객 참여와 함께 사용하면 유용합니다.

## T {#section_1841144841AF41169AF1E83B90C18292}

**테스트**

두 개 이상의 경험을 지정한 성공 지표에 대해 비교하는 활동을 제공하므로 원하는 결과를 제공할 가능성이 가장 높은 경험을 선택할 수 있습니다.

**사이트에서 보낸 시간**

방문자가 활동의 첫 번째 디스플레이 mbox를 본 시점부터 mbox가 있는 세션에서 최종 페이지가 로드될 때까지 방문에서 보낸 시간 (초) 를 나타내는 참여 유형입니다.

## U {#section_6407B81326FF43049D92265746E6C402}

**URL 매개 변수**

모든 참조 페이지 및 페이지 URL 변수는 방문자가 mbox를 포함하는 페이지를 볼 때마다 mbox에 자동으로 전달됩니다. 그런 다음 이러한 URL 변수를 보고서에서 디스플레이 타깃팅 또는 세그먼트 필터를 통해 사용할 수 있습니다.

## V {#section_0B85CE959BA948CDAC3BAEFB4EEDFAD2}

**방문자**

방문자는 사이트에 액세스하는 개인입니다. 방문자가 활동에 포함되는지 여부를 결정하기 위해 활동 기준에 대해 방문자가 평가됩니다. *참여자*&#x200B;를 참조하십시오.

**시각적 경험 작성기**

Target Standard 에서, 사이트 컨텍스트에서 개인화된 경험과 오퍼를 쉽게 만들고 테스트할 수 있는 WYSIWYG 사용자 인터페이스입니다. 웹 페이지 (또는 오퍼) 의 레이아웃 및 컨텐츠를 드래그 앤 드롭하고, 교환하고, 수정하여 타겟 활동에 대한 경험과 오퍼를 만들 수 있습니다.
