...
계산된 지표는 현재 상승도 및 신뢰도 함수에서 지원되지 않습니다. 이것은 Analytics이 방문자 수준이 아니라 집계 수준에서 지표를 계산하기 때문입니다. 신뢰도는 특히 방문자 수준 계산입니다.
...
* 표준 이벤트의 합계인 계산된 지표입니다. 예를 들어 10개의 서로 다른 리드 양식을 10개의 개별 이벤트로 추적한 다음 이를 모두 추가하여 총 리드 제출을 얻을 수 있습니다. 이러한 이벤트를 추적하는 권장 방법은 Analytics에서 단일 리드 제출 이벤트를 구현한 다음 eVar를 사용하여 리드 양식 유형을 수집하는 것입니다. 이 방법을 사용하면 더 적은 변수가 필요하며 리프트 및 신뢰 함수에서 단일 리드 제출 지표를 사용할 수 있습니다.
