...
description: Analytics for [!DNL Target] (A4T)을 사용할 때 고객이 직면한 일반적인 문제를 살펴보십시오.
...
이 항목에서는 [!DNL Adobe Target](A4T)의 보고 소스로 [!DNL Adobe Analytics]을 사용할 때 발생하는 몇 가지 일반적인 문제에 대해 설명합니다.
...
보고서에 데이터를 표시하는 데 사용하는 지표에 따라 보고서에 &quot;[!UICONTROL 지정되지 않음]&quot; 이벤트 행이 있을 수 있습니다.

일반적으로 이 행은 보고서에서 [!DNL Target]페이지 보기 횟수], [!UICONTROL 방문 횟수], [!UICONTROL 고유 방문자 수] 등이 아닌 일반적인 지표를 선택하는 경우 표시됩니다. [!UICONTROL  이 경우 [!UICONTROL &quot;지정되지 않음&quot;] 행에는 [!UICONTROL 페이지 보기], [!UICONTROL 방문 횟수] 및 [!UICONTROL 고유 방문자]가 모두 포함되어 있으며, 이 행은 [!DNL Target] 활동과 연결되어 있지 않습니다.

해당 행에는 [!DNL Target] 관련 정보가 없습니다(예: 방문자, 방문 또는 노출 횟수 없음). 자세한 내용은 *Analytics 기술 노트*&#x200B;의 보고](https://experienceleague.adobe.com/docs/analytics/technotes/unspecified.html?lang=en)에서 [&quot;지정되지 않음,&quot; &quot;없음,&quot; &quot;기타&quot; 및 &quot;알 수 없음&quot;을 참조하십시오.

보고서에서 [!DNL Target] 특정 지표를 선택하는 경우 [!UICONTROL &quot;Unspecified&quot;] 행이 표시되지 않습니다. 보고서에서 모두 표시하지 않는 유일한 방법은 해당 페이지에서 보낸 모든 요청에 대해 [!DNL Target] 호출을 설정하는 것입니다. 이 호출은 일반적이거나 필요하지 않습니다.
...
A4T 활동을 사용하려면 Analytics 추적 서버를 지정해야 합니다. Analytics 추적 서버를 올바로 설정하려면  [Analytics 추적 ](/help/c-integrating-target-with-mac/a4t/analytics-tracking-server.md#task_72077BA7E93C4A65A715A18F32228823) 서버 사용 을 참조하십시오.
...
[!DNL Target Standard/Premium]에 표시되는 보고서 세트 목록은 [!DNL Target]에 대한 보고 소스로 구성된 보고서 세트 목록입니다(A4T). [!DNL Analytics] 보유한 모든 보고서 세트가 표시되지 않을 수 있습니다.

여러 보고 소스를 사용하는 경우 보고서 세트가 [!DNL Target]에 설정된 기본 보고 소스에도 있어야 합니다. 보고서 세트가 기본 보고 소스에 없는 경우 보고서 세트가 표시되지 않습니다.

찾고 있는 보고서 세트가 여전히 표시되지 않으면 [Client Care](/help/cmp-resources-and-contact-information.md#reference_ACA3391A00EF467B87930A450050077C)에 문의하여 활성화하십시오.
...
* **at.js 1.x**: 호출에서는  [!DNL Target] 보조 ID가  `mboxMCSDID` 매개 변수에 포함되어 있습니다. [!DNL Analytics] 호출에서는 보조 ID가 `sdid` 매개 변수에 포함되어 있습니다.
* **at.js 2.x**: 호출에서  [!DNL Target] 보조 ID는 HTTP 헤더에서  `experienceCloud.analytics.supplementalDataId`의 값으로 반환됩니다. [!DNL Analytics] 호출에서는 보조 ID가 `sdid` 매개 변수에 포함되어 있습니다.
...
[!DNL Target] 호출에 보조 데이터 ID가 없다면 [!DNL VisitorAPI.js] 파일이 [!DNL at.js] 앞에 로드되었는지 확인하십시오. [!DNL Analytics] 호출에 보조 데이터 ID가 없다면 [!DNL Target] 호출 전에 [!DNL Analytics] 호출이 발생하는지 확인하십시오.
