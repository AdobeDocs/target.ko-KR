...
>[!NOTE]
>
>아래 나열된 제한 사항은 "권장" 로 지정하지 않는 한 "어려운" 제한으로 간주해야 합니다.
>
>" 권장 "으로 지정된 제한이 가까워지거나 초과되면 성능이 저하될 수 있습니다. 하나의 활동에 많은 대상, 타겟 및 경험이 모두 있는 경우와 같이 활동이 매우 복잡해도 인터페이스 로드가 느려질 수 있습니다.
>
>매우 복잡한 활동은 Adobe 컨설턴트와 함께 검토해야 하며 프로덕션 상태에 들어가기 전에 제한된 환경에서 테스트해야 합니다.

## 활동

**권장 제한**: 10,000 개의 활성 라이브 활동.

**권장 제한**: 10,000 개의 활성 저장 (및 종료되지 않은) activiites.
...
단일 값 엔티티 사용자 정의 속성의 최대 길이는 15,000 자 (1 바이트 및 2 바이트 UTF -8 인코딩 언어 (예: 영어 및 기타 라틴 스크립트 알파벳) 또는 10,000 자 (3 바이트 UTF -8 인코딩 언어 (중국어, 일본어 및 한국어)) 입니다.
...
**제한**: 경험당 2,000 개의 경험 타깃팅 (XT), A/B 테스트, 다변량 테스트 (MVT) 및 자동 타겟 활동.

자동화된 개인화 (AP) 활동당 30,000 개의 경험.
...
글로벌 mbox를 사용하는 경우 이 제한은 페이지에 대해 반환되는 전체 컨텐츠 세트에 해당됩니다. 오퍼 크기를 제한하면 페이지 로드 시간이 향상됩니다. 이 제한을 초과하는 경우 다음 메시지가 나타납니다.

"경험에 대한 컨텐츠가 너무 커서 전달할 수 없습니다. 더 적은 페이지 코드에 영향을 미치도록 경험을 수정하십시오."

## 오퍼

**권장 제한**: 총 50,000 개의 프로모션.
...
**권장 제한**: 300 개의 활성 프로파일 스크립트.

**권장 제한**: 프로필 스크립트당 최대 루프 수 500,000 개입니다.
...
**권장 제한**: 타깃팅 규칙당 2,500 개의 고유 값.

**권장 제한**: 타깃팅 규칙 전체에서 대상자당 30,000 개의 고유 값.

**권장 제한**: 활동당 100,000 개의 고유한 타깃팅 규칙 값.
