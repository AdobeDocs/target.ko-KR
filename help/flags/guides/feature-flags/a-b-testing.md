...
1. **기본 정보**&#x200B;에서 제목, 키 및 설명을 입력하십시오.
1. **백분율 롤아웃**&#x200B;을 설정하여 대상자가 테스트에 얼마나 참여하는지 정의하세요.
1. **Variants**&#x200B;을(를) 1보다 큰 값으로 설정합니다(예: 클래식 A/B 테스트의 경우 2개의 변형). 최대 **3개의 변형과 컨트롤 그룹**&#x200B;을 정의할 수 있습니다.
1. 노출 비율이 여러 변형에 어떻게 분포되어 있는지 이해하려면 [기능 그룹을 점진적으로 롤아웃하도록 설정](set-feature-group-gradual-rollout.md)을 참조하십시오.

>[!NOTE]
>
>노출은 변형에서 **동일하게** 분할됩니다(예: 두 변형의 경우 50/50). 60/40과 같은 사용자 정의 분할은 지원되지 않습니다. 단일 기능 플래그를 **둘 이상의 변형**&#x200B;에 추가할 수 있습니다. 대상은 변형별로 설정되지 않고 기능 그룹별로 **한 번**&#x200B;로 설정됩니다.
