...
1. 플래그 콘솔에 로그인하고 **플래그 > 응용 프로그램**(으)로 이동합니다.

   >[!NOTE]
   >
   >**새 응용 프로그램** 단추가 표시되지 않으면 **관리자** 역할이 있는지 확인하십시오.
...
4. 다음 정보를 제공합니다.
   ...
   *로 표시된 필드는 필수입니다.
   ...
   | 필드 | 설명 |
   | --- | --- |
   | **응용 프로그램 이름** * | 응용 프로그램의 표시 이름입니다. |
   | **응용 프로그램 ID** * | 코드에서 플래그를 호출할 때 사용되는 고유 식별자입니다. 애플리케이션의 클라이언트 ID를 사용합니다. |
   | **투표 간격** | 애플리케이션당 캐시를 새로 고치는 폴링 간격(초)입니다. 서버측 SDK에만 적용됩니다. |
