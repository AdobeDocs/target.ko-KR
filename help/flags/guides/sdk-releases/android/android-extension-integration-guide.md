...
## 기능 평가를 위한 주요 개념 {#key-concepts}

앱에서 기능 게이트를 구현할 때에는 다음 사항을 염두에 두십시오.

* **특성 값을 전달하고 레이블을 표시하지 않습니다.** 컨텍스트 특성 값은 **대/소문자를 구분합니다**. 콘솔에 표시된 레이블이 아니라 앱이나 웹 사이트에서 보내는 원시 값(예: `"en_US"` 또는 `"ANDROID"`)을 전달합니다.
* **기능(플래그) 수준에서 평가합니다.** 플래그가 기능 그룹에 속하는 경우에도 항상 개별 **기능 키**&#x200B;로 API를 호출하십시오. 그룹 수준의 평가가 없습니다. 응답은 사용자가 속한 변형을 반환합니다.
* **ID를 프로필에 연결할 필요가 없습니다.** 평가는 런타임에 수행됩니다. ID가 알려진 프로필에 연결되어 있는지 여부에 관계없이 평가 이벤트가 Customer Journey Analytics으로 전송됩니다.
* **각 새 플래그에는 코드를 변경해야 합니다.** 코드에 각 플래그 키의 게이트를 추가합니다. 부울 온/오프 상태를 확인하려면 `isFeatureEnabled()`을(를) 사용하고, 변형을 포함한 전체 기능 페이로드를 검색하려면 `getFeature()`을(를) 사용하십시오.
