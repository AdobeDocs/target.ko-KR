...
1. 사용하려는 개체 속성이 비어 있지 않은지 확인하십시오(Target Recommendations의 템플릿 구문 분석기가 템플릿을 확인하고 저장하는 데 필요).
1. 1단계에서 만든 더미 변수의 `parseInt` 또는 `parseDouble` 메서드에 entity 속성을 전달하여 문자열을 정수 또는 이중 값으로 변환합니다.
1. 수학 연산을 수행하거나 새 숫자 값에 대해 비교를 수행합니다.
