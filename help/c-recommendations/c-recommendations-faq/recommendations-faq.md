...
## 숫자 값으로 사용자 지정 속성을 검색할 때 [!UICONTROL 카탈로그 검색]에 올바른 결과가 표시되지 않는 이유는 무엇입니까?
...
| 피드를 통해 업데이트된 항목 속성 | <ul><li>Recommendations은 피드 인제스트 후(2-8시간) 업데이트됩니다.</li><li>업데이트를 사용할 수 있을 때까지 기존 권장 사항 및 항목 속성이 표시됩니다.</li><li>카탈로그 검색은 피드 인제스트(2-8시간) 후 및 후속 카탈로그 색인(3-8시간) 후에 업데이트됩니다. 카탈로그 검색은 총 5-16시간 내에 업데이트됩니다.</li></ul> |
| [!DNL Target] UI 또는 API를 통해 카탈로그에서 삭제된 항목 | <ul><li>Recommendations은 15분 이내에 업데이트됩니다.</li><li>업데이트를 사용할 수 있을 때까지 기존 권장 사항 및 항목 속성이 표시됩니다.</li><li>카탈로그 검색은 카탈로그 색인(3-8시간) 후에 업데이트됩니다.</li></ul> |
...
| 피드를 통해 카탈로그에 추가된 항목 | <ul><li>Recommendations은 피드를 인제스트한 후 업데이트됩니다(2-8시간). 후속 알고리즘 실행은 1일 알고리즘에 대해 12시간마다, 7일 이상의 알고리즘에 대해 24시간마다 예약됩니다. Recommendations은 총 2-32시간 이내에 업데이트됩니다.</li><li>추가된 항목이 요청된 키가 아닌 경우 업데이트를 사용할 수 있을 때까지 기존 권장 사항이 표시됩니다.</li><li>추가된 항목이 요청된 키인 경우 업데이트를 사용할 수 있을 때까지 백업 권장 사항이 표시됩니다.</li><li>카탈로그 검색은 피드 인제스트(2-8시간) 후 및 카탈로그 색인(3-8시간) 후에 업데이트됩니다. 카탈로그 검색은 총 5-16시간 내에 업데이트됩니다.</li></ul> |
...
* 항목이 이전에 제외되었지만 현재 포함되어야 하는 경우, 해당 항목이 다음 알고리즘 실행(12-24시간)에 포함됩니다.
   ...
   이러한 상황은 [!DNL Target]이(가) 온라인 및 오프라인 모두에서 제외를 적용하므로 발생합니다. 항목이 새로 제외되면 온라인 제외가 빠르게 적용됩니다. 항목이 새로 포함되면 온라인 제외는 빠르게 제거되지만 다음 알고리즘이 실행되기 전까지 오프라인 제외는 사라지지 않습니다.
...
>[!NOTE]
>
>피드 파일은 &quot;항목 가져오는 중&quot;에서 &quot;검색 색인 업데이트 준비 중&quot;으로 상태가 변경될 때 가져온 것으로 간주됩니다. 업데이트는 카탈로그 검색 사용자 인터페이스에 반영되는 데 60분 이상 걸릴 수 있습니다. 피드 상태가 &quot;업데이트 완료&quot;로 바뀌는 경우 카탈로그 검색이 최신이 상태가 됩니다. 카탈로그 검색이 아직 최신 상태가 아니어도 사이트는 위에 나열된 기간에 대한 업데이트를 반영합니다. 가장 최근의 카탈로그 검색 색인 업데이트 시간이 카탈로그 검색 페이지에 표시됩니다.

## 내 사이트에 반영될 [!UICONTROL Recommendations] 활동, 오퍼, 프로모션 또는 기준 설정의 구성을 변경하는 데 얼마나 걸립니까?

* 프로모션 설정을 변경하면 사이트에서 반영하기까지 최대 5시간이 걸릴 수 있습니다.
* 다음 알고리즘을 실행할 때까지 다른 기준 설정에 대한 변경 사항이 반영되지 않을 수 있습니다.

   * 일부 기준 설정(예: &quot;동적 포함 규칙 추가&quot;)은 즉시 반영됩니다.
   * 다른 기준 설정(예: &quot;동적 포함 규칙 제거&quot;, 전환 확인 창 변경 등)은 다음 알고리즘을 실행하기 전까지 통합할 수 없습니다.
   * 이러한 변경 사항에 의해 알고리즘 실행이 트리거되지만 완료하는 데 최대 24시간이 걸릴 수 있습니다. 또한 알고리즘은 12-24시간마다 예약된 기준으로 실행됩니다.

## 사용자가 받는 권장 사항 *에 제품 A를 클릭하고 제품 B를 구입하는 등의 사용자 행동이 반영되는 데 얼마나 걸립니까?*

* 현재 조회/구매한 제품/컨텐츠는 사용자가 동일한 페이지 뷰/[!DNL Target] 컨텐츠 요청에서 받은 권장 사항에 영향을 줍니다.
* &quot;마지막으로 본 제품&quot;, &quot;가장 많이 본 제품&quot; 및 전체 보기/구매 내역과 같은 사용자 행동 내역은 해당 요청과 함께 업데이트되며, 사용자가 다음 페이지 보기/[!DNL Target] 컨텐츠 요청에서 받은 추천에 영향을 줍니다. 예를 들어 &quot;최근에 본 항목&quot; 및 &quot;사용자에게 권장됨&quot; 알고리즘은 각 제품 보기/구매로 업데이트되며 이후 컨텐츠 요청에 반영됩니다.

## 사용자 행동(예: 제품 A 클릭 및 제품 B 구입)이 권장 사항 *기타* 사용자가 받는 경우 얼마나 걸립니까?

집계 사용자의 동작은 12-24시간마다 발생하는 모든 알고리즘 실행과 함께 오프라인 알고리즘 처리에 포함됩니다.
...
[!DNL Target] 알고리즘  [선택기의 ](/help/c-recommendations/plan-implement.md#concept_C1E1E2351413468692D6C21145EF0B84) 지능적인 필터링을 제어하기 위해 호환되지 않는 필터 필터가 있습니다.

>[!NOTE]
>
>이 설정은 시각적 경험 작성기(VEC)에서 만들어진 활동에만 적용되고, 이 설정은 양식 기반 경험 작성기에서 만든 활동에는 적용되지 않습니다([!DNL Target] 에는 위치 컨텍스트가 없음).

[!UICONTROL 호환되지 않는 기준 필터링] 설정에 액세스하려면 [!UICONTROL 권장 사항] > [!UICONTROL 설정]을 클릭하십시오.

![](assets/recs_settings_filter.png)

[!UICONTROL 호환되지 않는 기준 필터링] 설정이 활성화되어 있지 않으면 [!DNL Target]는 알고리즘 선택기에서 알고리즘을 필터링하지 않고 모든 알고리즘이 표시됩니다.

[!UICONTROL 호환되지 않는 기준 필터링] 설정이 활성화된 경우 VEC 활동에서 [!DNL Target]는 선택한 위치의 entityId 및 카테고리 ID를 읽은 다음 `currentItem|currentCategory`을(를) 기준으로 알고리즘을 표시합니다(해당 위치에 각각의 값이 있는 경우). 따라서 기본적으로 알고리즘 선택기에는 선택한 위치를 위한 호환되는 알고리즘만 표시됩니다.
...
다음 목록에는 [!DNL Target]Compatible] 확인란이 표시되지 않는 특별한 사례가 포함되어 있습니다.[!UICONTROL 
...
* [!DNL Target] 는 (고객의 페이지/앱에서) 배달에 대한 업데이트를 가능한 한 빨리 수행할 수 있도록 최선의 노력을 다하고 있습니다. 그러나 [!DNL Target]은(는) 마케터용 UI에서 일부 표현을 제공해야 합니다. [!DNL Target] 는 UI 업데이트가 동기화될 때까지 기다리도록 배달 업데이트를 지연하지 않습니다. [mboxTrace](/help/c-activities/c-troubleshooting-activities/content-trouble.md) 를 사용하여 요청이 들어올 당시 시스템의 내용을 확인할 수 있습니다.
...
[!DNL Target] 사용 가능한 권장 사항의 수가 낮아 권장 사항을 표시할 수 없는 경우도 있습니다.
...
[!DNL Target] 애플리케이션 수준에서 50MB 게시물 제한을 적용합니다.그러나 이는  `application/x-www-form-urlencoded` 컨텐츠 유형 헤더를 전달하는 경우에만 해당합니다.
...
피드의 CSV 파일 업로드에 대한 행 수 또는 파일 크기에는 엄격한 제한이 없습니다. 그러나 파일 업로드 프로세스 중 실패를 방지하기 위해 CSV 파일 크기를 1GB로 제한하는 것이 좋습니다. 파일 크기가 1GB를 초과하는 경우 이상적으로 여러 피드 파일로 분할해야 합니다. 사용자 지정 속성 컬럼의 최대 수는 100개이며 사용자 지정 속성은 4096자로 제한됩니다. 필요한 열 길이에 대한 다른 제한은 [[!DNL Target] 제한 페이지](/help/r-troubleshooting-target/target-limits.md#reference_BEFE60C3AAA442FF94D4EBFB9D3CC9B1)에서 사용할 수 있습니다.
...
현재 [!DNL Target] 호출에만 제외가 수행됩니다.`excludedIds` 값이 다시 전달되지 않으면 후속 [!DNL Target] 호출에서 항목이 제외되지 않습니다. 장바구니의 항목을 모든 페이지의 권장 사항에서 제외하려면, 모든 페이지의 `excludedIds` 값을 계속 전달하십시오.
...
방문자에게 동시에 두 개의 활성 세션이 있는 경우 장치가 세션 ID를 강제로 공유하지 않는 한, 한 장치에서 최근 본 항목은 다른 장치에서 최근에 본 항목을 업데이트하지 않습니다. 문제에 대한 잠재적 해결 방법이 있지만 [!DNL Target]은(는) 여러 장치에서 세션 ID 공유를 직접 지원하지 않습니다. 고객은 이 ID 공유 자체를 관리해야 합니다.
