...
## 숫자 값이 있는 사용자 지정 속성을 검색할 때 [!UICONTROL 카탈로그 검색]에 올바른 결과가 표시되지 않는 이유는 무엇입니까?

숫자 값을 사용하는 사용자 지정 속성에 대해 카탈로그 검색을 수행하면 결과가 사용자 지정 속성을 숫자 값 대신 문자열 유형으로 처리합니다.
...
| 피드를 통한 항목 속성 업데이트 | <ul><li>피드 수집 후에는 권장 사항이 업데이트됩니다(2~8시간).</li><li>기존 권장 사항 및 항목 속성은 업데이트를 사용할 수 있을 때까지 표시됩니다.</li><li>피드 수집(2~8시간) 및 후속 카탈로그 색인(3~8시간) 후에는 카탈로그 검색이 업데이트됩니다. 카탈로그 검색은 총 5-16시간 내에 업데이트됩니다.</li></ul> |
...
| 항목이 피드를 통해 카탈로그에 추가됨 | <ul><li>피드가 수집된 후에는 권장 사항이 업데이트됩니다(2~8시간). 후속 알고리즘 실행은 1~2일 알고리즘의 경우 12시간마다 예약되며, 7일 이상 알고리즘의 경우에는 24시간마다 예약됩니다. Recommendations은 총 2-32시간 내에 업데이트됩니다.</li><li>추가된 항목이 요청된 키가 아닌 경우에는 업데이트를 사용할 수 있을 때까지 기존 권장 사항이 표시됩니다.</li><li>추가된 항목이 요청된 키가 아닌 경우에는 업데이트를 사용할 수 있을 때까지 백업 권장 사항이 표시됩니다.</li><li>피드 수집(2~8시간) 및 카탈로그 색인(3~8시간) 후에는 카탈로그 검색이 업데이트됩니다. 카탈로그 검색은 총 5-16시간 내에 업데이트됩니다.</li></ul> |
...
* 이전에는 제외되었지만 지금은 포함되어야 하는 경우에는 항목이 다음 알고리즘 실행에 포함됩니다(12~24시간).
   ...
   이 문제는 [!DNL Target]이(가) 온라인 및 오프라인 모두에서 제외를 적용하므로 발생합니다. 항목을 새로 제외하는 경우에는 온라인 제외가 빠르게 적용됩니다. 항목이 새로 포함되는 경우에는 온라인 제외가 빠르게 사라지지만, 오프라인 제외는 다음 알고리즘일 실행될 때까지 사라지지 않습니다.
...
## 내 사이트에 반영될 [!UICONTROL Recommendations] 활동, 오퍼, 프로모션 또는 기준 설정의 구성을 변경하는 데 시간이 얼마나 걸립니까?

* 프로모션 설정을 변경하는 데 최대 5시간이 걸릴 수 있습니다.
* 다음 알고리즘이 실행될 때까지 다른 기준 설정에 대한 변경 사항이 반영되지 않을 수 있습니다.

   * 일부 기준 설정(예: &quot;동적 포함 규칙 추가&quot;)이 즉시 반영됩니다.
   * 다른 기준 설정(예: &quot;동적 포함 규칙 제거&quot;, 전환 확인 기간 변경 등)은 다음 알고리즘이 실행될 때까지 통합할 수 없습니다.
   * 이러한 변경 사항에 의해 알고리즘 실행이 트리거되지만 완료하는 데 최대 24시간이 걸릴 수 있습니다. 알고리즘은 또한 12-24시간마다 예약된 기준으로 실행됩니다.

## 사용자가 받은 권장 사항 *에 반영되는 사용자의 행동(예: 제품 A를 클릭하고 제품 B를 구매하는 경우)이 얼마나 걸립니까?*

* 현재 조회/구매한 제품/컨텐츠는 사용자가 동일한 페이지 보기/[!DNL Target] 컨텐츠 요청에서 수신하는 권장 사항에 영향을 줍니다.
* &quot;마지막으로 본 제품&quot;, &quot;가장 많이 본 제품&quot; 및 전체 보기/구매 내역 등의 내역 사용자 동작은 해당 요청으로 업데이트되며, 사용자가 다음 페이지 보기/[!DNL Target] 컨텐츠 요청에서 받은 권장 사항에 영향을 줍니다. 예를 들어, &quot;최근에 본 항목&quot; 및 &quot;사용자에게 권장&quot; 알고리즘은 각 제품 보기/구매로 업데이트되고 후속 컨텐츠 요청에 반영됩니다.

## 권장 사항 *기타* 사용자가 수신하는 권장 사항에 반영되는 사용자의 행동(예: 제품 A를 클릭하고 제품 B를 구매하는 경우)이 얼마나 걸립니까?

집계에서 사용자의 동작은 12-24시간마다 발생하는 모든 알고리즘 실행을 사용하여 오프라인 알고리즘 처리에 통합되어 있습니다.
...
[!DNL Target] 에는  [호환되지 않는 ](/help/c-recommendations/plan-implement.md#concept_C1E1E2351413468692D6C21145EF0B84) 기준이 있으므로 알고리즘 선택기의 지능형 필터링을 제어할 수 있습니다.

>[!NOTE]
>
>이 설정은 시각적 경험 작성기(VEC)에서 만들어진 활동에만 적용되고, 이 설정은 양식 기반 경험 작성기에서 만든 활동에는 적용되지 않습니다([!DNL Target]에 위치 컨텍스트가 없음).

[!UICONTROL 호환되지 않는 기준 필터링] 설정에 액세스하려면 [!UICONTROL 권장 사항] > [!UICONTROL 설정]을 클릭하십시오.

![](assets/recs_settings_filter.png)

[!UICONTROL 호환되지 않는 기준 필터링] 설정이 활성화되어 있지 않으면 [!DNL Target]이 알고리즘 선택기에서 알고리즘을 필터링하지 않고 모든 알고리즘이 표시됩니다.

[!UICONTROL 호환되지 않는 기준 필터링] 설정이 활성화되어 있으면 VEC 활동에서 [!DNL Target]는 선택한 위치로부터 entityId와 카테고리 ID를 읽은 다음 `currentItem|currentCategory`(각 값이 해당 위치에 있는 경우)를 기반으로 알고리즘을 표시합니다. 따라서 기본적으로 알고리즘 선택기에는 선택한 위치를 위한 호환되는 알고리즘만 표시됩니다.
...
다음 목록에는 [!DNL Target]Compatible] 확인란이 표시되지 않는 특수 사례가 포함되어 있습니다.[!UICONTROL 
...
* [!DNL Target] 은 게재(고객의 페이지/앱에서)에 대한 업데이트를 가능한 한 빨리 수행하는 데 최선을 다합니다. 그러나 [!DNL Target] 역시 마케터의 UI에 일부 표현을 제공해야 합니다. [!DNL Target] 는 UI 업데이트가 동기화될 때까지 기다리기 위해 게재 업데이트를 지연하지 않습니다. [mboxTrace](/help/c-activities/c-troubleshooting-activities/content-trouble.md) 를 사용하여 요청이 들어올 당시 시스템의 내용을 확인할 수 있습니다.
...
[!DNL Target] 때로는 사용 가능한 권장 사항 수가 적어서 권장 사항이 표시되지 않을 수 있습니다.
...
[!DNL Target] 애플리케이션 수준에 50MB의 사후 제한을 지정합니다.그러나  `application/x-www-form-urlencoded` 콘텐츠 유형 헤더를 전달하는 경우에만 해당됩니다.
...
피드의 CSV 파일 업로드에 대한 행 수 또는 파일 크기에는 엄격한 제한이 없습니다. 그러나 파일 업로드 프로세스 중 실패를 방지하기 위해 CSV 파일 크기를 1GB로 제한하는 것이 좋습니다. 파일 크기가 1GB를 초과하는 경우 이상적으로 여러 피드 파일로 분할해야 합니다. 사용자 지정 속성 컬럼의 최대 수는 100개이며 사용자 지정 속성은 4096자로 제한됩니다. 필요한 열 길이에 대한 다른 제한 사항은 [[!DNL Target] 제한 페이지](/help/r-troubleshooting-target/target-limits.md#reference_BEFE60C3AAA442FF94D4EBFB9D3CC9B1)에서 사용할 수 있습니다.
...
제외는 현재 [!DNL Target] 호출에 대해서만 수행됩니다.`excludedIds` 값이 다시 전달되지 않는 한 후속 [!DNL Target] 호출에서 항목이 제외되지 않습니다. 장바구니의 항목을 모든 페이지의 권장 사항에서 제외하려면, 모든 페이지의 `excludedIds` 값을 계속 전달하십시오.
...
방문자에게 동시에 두 개의 활성 세션이 있는 경우, 한 장치에서 최근 본 항목은 장치가 세션 ID를 공유하도록 강제 되지 않는 한 다른 장치에서 최근에 본 항목을 업데이트하지 않습니다. 문제에 대한 잠재적인 해결 방법이 있기는 하지만, [!DNL Target]은 디바이스 여러 대의 세션 ID에 대한 공유를 직접 지원하지 않습니다. 고객은 직접 이러한 ID 공유를 관리해야 합니다.

이 동작은 방문자가 한 장치에서 활성 상태이고 몇 분 후에 다른 장치에서 활성 상태가 될 경우에도 계속 발생합니다. 첫 번째 디바이스의 세션은 30분 동안 만료되지 않으며, 프로필 상태가 영구 상태에 기록되어 처리되기 전에 최대 5분 동안 지연될 수 있습니다. 이 행동을 테스트할 때, 35분 안에 세션을 만료할 수 있으며 프로필을 저장할 수 있습니다.
...
[!DNL Recommendations Classic]에서 생성된 알고리즘은 [!DNL Recommendations Premium]에서 지원되지 않습니다. [!DNL Target Premium];에서 기존 알고리즘을 사용할 수 있습니다.그러나 알고리즘에서는 [!DNL Target Premium] UI에서 활동을 비활성화하거나 삭제할 때 동기화 문제를 만들 수 있습니다. 두 솔루션 간의 차이에 대한 자세한 내용은 [!DNL Target Premium]](/help/c-recommendations/c-recommendations-faq/recommendations-classic-versus-recommendations-activities-target-premium.md)의 [[!DNL Recommendations Classic] versus [!DNL Recommendations] 활동 을 참조하십시오.
