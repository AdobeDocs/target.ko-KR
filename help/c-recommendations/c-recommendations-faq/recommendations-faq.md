...
* 현재 보고/구입한 제품/컨텐츠는 사용자가 동일한 페이지 보기/Target 컨텐츠 요청에서 받은 권장 사항에 영향을 줍니다.
* &quot;마지막으로 본 제품&quot;, &quot;가장 많이 본 제품&quot; 및 전체 보기/구매 내역과 같은 사용자 행동 내역은 해당 요청과 함께 업데이트되며, 다음 페이지 보기/Target 컨텐츠 요청에서 받은 권장 사항에 영향을 줍니다. 예를 들어 &quot;최근에 본 항목&quot; 및 &quot;사용자에게 권장됨&quot; 알고리즘은 각 제품 보기/구매로 업데이트되며 이후 컨텐츠 요청에 반영됩니다.
...
Target에는 알고리즘 선택기의 지능적 필터링을 제어하기 위한 [호환되지 않는 기준 필터링](/help/c-recommendations/plan-implement.md#concept_C1E1E2351413468692D6C21145EF0B84) 설정이 있습니다.
...
* Target은 최대한 빨리 전달(고객의 페이지/앱에서)이 업데이트되도록 합니다. 하지만 Target에서는 마케터용 UI에서 몇 가지 표현도 제공해야 합니다. UI 업데이트가 동기화될 때까지 기다리기 위해 Target에서 전달 업데이트가 지연되지는 않습니다. [mboxTrace](/help/c-activities/c-troubleshooting-activities/content-trouble.md) 를 사용하여 요청이 들어올 당시 시스템의 내용을 확인할 수 있습니다.
...
피드의 CSV 파일 업로드에 대한 행 수 또는 파일 크기에는 엄격한 제한이 없습니다. 그러나 파일 업로드 프로세스 중 실패를 방지하기 위해 CSV 파일 크기를 1GB로 제한하는 것이 좋습니다. 파일 크기가 1GB를 초과하는 경우 이상적으로 여러 피드 파일로 분할해야 합니다. 사용자 지정 속성 컬럼의 최대 수는 100개이며 사용자 지정 속성은 4096자로 제한됩니다. 필요한 열의 길이에 대한 다른 제한은 [대상 제한 페이지](/help/r-troubleshooting-target/target-limits.md#reference_BEFE60C3AAA442FF94D4EBFB9D3CC9B1)에서 사용할 수 있습니다.
...
제외는 현재 대상 호출에 대해서만 수행되며 `excludedIds` 값이 다시 전달되지 않는 한 후속 대상 호출에서는 항목이 제외되지 않습니다. 장바구니의 항목을 모든 페이지의 권장 사항에서 제외하려면, 모든 페이지의 `excludedIds` 값을 계속 전달하십시오.
