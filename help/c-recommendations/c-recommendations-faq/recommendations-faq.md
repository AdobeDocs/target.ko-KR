...
## 내 카탈로그에 있는 항목 업데이트가 내 사이트에 반영되는 데 얼마나 걸립니까?

피드 파일을 가져오거나 API 또는 mbox를 통해 엔티티 업데이트를 받은 후 다음 변경 사항이 60 분 이내에 반영됩니다.

* 디자인 템플릿에서 반환되는 항목 속성입니다.
* 항목이 반환된 추천에 포함되지 않도록 하는 전역 제외 규칙에 사용된 항목 특성입니다.
* 항목이 반환된 추천에서 포함 또는 제외되는지 여부에 영향을 주는 조건 내의 포함 규칙에 사용된 항목 특성입니다.

다음 알고리즘 실행이 발생할 때까지 다음 변경 사항이 반영되지 않습니다 (12-24 시간 이내).

* 활동에 사용된 컬렉션 규칙에 사용된 항목 속성입니다.
* 활동과 연관된 특성 또는 컬렉션을 기반으로 한 판촉 행사에 사용된 항목 특성입니다.
* 항목이 최상위 판매자 또는 가장 많이 본 알고리즘의 &quot;현재 카테고리&quot; 또는 &quot;즐겨찾기 카테고리&quot; 에 대해 표시되는 항목 카테고리.
* 속성을 변경할 때 권장되는 항목 등급은 알고리즘에 대한 사용자 지정 키로 사용되는 사용자 지정 속성입니다.
* 권장 사항 로직이 &quot;비슷한 속성을 갖는 항목&quot; 인 경우, &quot;컨텐츠 유사성&quot; 가중치가 사용되는 경우 또는 &quot;속성 가중치&quot; 인수가 사용되는 경우 변경된 속성을 기반으로 권장 항목의 등급.

>[!NOTE]
>
>피드 파일은 &quot;항목 가져오기&quot; 에서 &quot;검색 색인 업데이트 준비&quot; 로 변경될 때 가져온 것으로 간주됩니다. 업데이트는 카탈로그 검색 사용자 인터페이스에 반영되는 데 60 분 이상 소요될 수 있습니다. 피드 상태가 &quot;업데이트 완료&quot; 로 바뀌는 경우 카탈로그 검색이 업데이트되었습니다. 카탈로그 검색이 아직 최신 상태가 아닌 경우에도 사이트에 나열된 시간대에 대한 업데이트가 반영됩니다. 카탈로그 검색 페이지에 가장 최근의 카탈로그 검색 색인 업데이트 시간이 표시됩니다.
...
`entityIds`를 제외하려면 `&excludes=${mbox.excludedIds}` 토큰을 오퍼 콘텐츠 URL에 추가합니다. 컨텐츠 URL이 추출되면 필수 매개 변수는 현재 mbox 요청 매개 변수를 사용하여 대체됩니다.
