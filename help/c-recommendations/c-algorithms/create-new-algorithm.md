---
description: 기준은 권장 사항 활동의 컨텐츠를 제어합니다. 활동에 가장 적합한 권장 사항을 표시하는 기준을 만드십시오.
...
기준은 권장 사항 활동의 컨텐츠를 제어합니다. 활동에 가장 적합한 권장 사항을 표시하는 기준을 만드십시오.
...
1. **[!UICONTROL 기준 만들기]** 또는 **[!UICONTROL 새로 만들기]**를 클릭합니다.
   ...
   ![만들기 기준 단추](/help/c-recommendations/c-algorithms/assets/button_CreateCriteria_new.png)
...
1. **[!UICONTROL 추천 키]**를 선택합니다.
   ...
   키를 기준으로 기준화하는 방법에 대한 자세한 내용은 권장 사항 키를 기준으로 권장 사항을 [참조하십시오](../../c-recommendations/c-algorithms/create-new-algorithm.md#task_2B0ED54AFBF64C56916B6E1F4DC0DC3B).

1. **[!UICONTROL 추천 논리]**를 선택합니다.
...
* **mbox**: 기준이 mbox를 행동 데이터 소스로 사용하는 경우, 만들어진 기준이 즉시 실행됩니다. 사용되는 동작 데이터의 분량과 카탈로그 크기에 따라 알고리즘을 실행하는 데 최대 12시간이 걸릴 수 있습니다. 기준 구성을 변경하면 기준이 재실행됩니다.

* **분석**: 기준이 행동 데이터 소스로 사용되는 [!DNL Adobe Analytics] 경우, 작성 후에는 선택한 보고서 세트와 확인 창이 다른 기준에 사용되었는지 여부에 따라 기준 사용 가능 여부가 결정됩니다. 보고서 세트가 이전에 전환 창과 함께 사용된 적이 있는 경우, 전환 창을 선택한 동안에는 동작 데이터를 Target에서 사용할 수 있으며 권장 사항은 즉시 기준을 실행합니다. 사용되는 동작 데이터의 분량과 카탈로그 크기에 따라 알고리즘을 실행하는 데 최대 12시간이 걸릴 수 있습니다. 보고서 세트가 이전에 사용되지 않았거나 더 오래된 전환 창에 사용되는 경우에는 권장 사항에서 Adobe Analytics의 데이터를 요청하여 수신한 다음, 그 이후에 알고리즘을 실행해야 합니다. Analytics를 사용하는 동기화 프로세스는 일반적으로 2일 이상 소요되며 Analytics 시스템 로드에 따라 완료하는 데 7일이 걸릴 수 있습니다.

## 권장 사항 키를 기준으로 권장 사항 기준 {#task_2B0ED54AFBF64C56916B6E1F4DC0DC3B}
...
1. 새 추천을 만들거나 기존 추천을 선택하고 **[!UICONTROL 편집]**을 클릭합니다.
1. 추천 키를 변경하려면 [!UICONTROL 추천 키] 드롭다운 목록에서 새 키를 선택한 다음, **[!UICONTROL 저장]**을 클릭합니다.
...
사용자 지정 프로필 속성 값에 대한 권장 사항을 만들 수 있습니다. 예를 들어, 방문자가 가장 최근에 큐에 추가한 동영상을 기반으로 권장 동영상을 표시하려 한다고 가정합니다.

1. **[!UICONTROL 권장 사항 키]** 드롭다운 목록에서 사용자 지정 프로필 속성을 선택합니다 (예: &quot;감시 목록에 추가된 마지막 표시&quot;).
1. 그런 다음 **[!UICONTROL 권장 사항 논리를]** 선택합니다 (예: &quot;본 사람, 이 항목을 본 사람&quot;).

   ![새 기준 만들기 대화 상자](/help/c-recommendations/c-algorithms/assets/create-new-criteria-1.png)

사용자 지정 프로필 속성이 단일 개체 ID와 직접 일치하지 않는 경우, 개체가 발생하기를 원하는 [!DNL Recommendations] 방법을 설명해야 합니다. 예를 들어 방문자가 가장 좋아하는 브랜드의 상위 판매 항목을 표시한다고 가정합니다.

1. **[!UICONTROL 권장 사항 키]** 드롭다운 목록에서 사용자 지정 프로필 속성을 선택합니다 (예: &quot;즐겨찾는 브랜드&quot;).

1. 그런 다음 이 키와 함께 사용할 **[!UICONTROL 권장 사항 로직을]** 선택합니다 (예: &quot;최상위 판매자&quot;).

   옵션의 고유 값별 [!UICONTROL 그룹이] 표시됩니다.

1. 선택한 키와 일치하는 엔티티 속성을 선택합니다. 이 경우 «Favorite Brand» `entity.brand`와 일치합니다.

   [!DNL Recommendations] 이제 각 브랜드에 대한 «최상위 판매자» 목록을 생성하고 방문자가 선호하는 브랜드 프로필 속성에 저장된 값을 기반으로 적절한 «최상위 판매자» 목록을 표시합니다.
...
이제 [최근에 본 항목] 기준이 주어진 [환경](/help/administrating-target/hosts.md)에 따른 결과를 반환합니다. 두 사이트가 서로 다른 환경에 속하고 방문자가 두 사이트 간을 전환하는 경우, 각 사이트에는 해당 사이트에서 최근에 본 항목만 표시됩니다. 방문자가 동일한 환경에 속한 두 사이트 간에 전환하는 경우 방문자에게 두 사이트에서 최근에 본 동일한 항목이 표시됩니다.
...
1. 특정 기준을 충족할 때만 항목을 표시하도록 권장 사항을 구성합니다.
   ...
   >[!NOTE]
   >
   >이 선택 사항은 추천에 표시되는 품목을 제한하며, 권장 사항이 어느 페이지에 표시되는지에는 영향을 주지 않습니다. 권장 사항이 표시되는 위치를 제한하려면 경험 작성기에서 페이지를 선택합니다.
