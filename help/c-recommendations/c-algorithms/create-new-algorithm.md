...
* **Analytics**: 기준이 [!DNL Adobe Analytics]을 동작 데이터 소스로 사용하는 경우 기준 가용성의 시간은 선택한 보고서 세트 및 전환 창이 다른 모든 기준에 사용되었는지 여부에 따라 달라집니다.

   * **초기 지연**: 초기 지연 시간은 2 ~ 7 일입니다. 이 초기 지연은 이전에 사용되지 않은 보고서 세트와 함께 기준이 설정되거나 더 긴 조회 기간과 함께 사용되는 경우 한 번만 발생합니다.
   * **지속적인 지연**: 보고서 세트가 미리 보기 창과 함께 이전에 사용되던 경우, 새 기준 및 기존 기준에 대한 예상 지연은 사용된 행동 데이터 양과 카탈로그 크기에 따라 12 시간 미만입니다.
   예를 들어 "본 친화성" 권장 사항의 경우 사용자가 제품을 볼 때 제품-보기 추적 호출이 Analytics에 실시간을 가깝게 전달됩니다. Analytics 데이터가 다음 날 일찍 타깃팅하도록 푸시되고 알고리즘이 12 시간 미만입니다.
