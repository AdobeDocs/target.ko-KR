...
기준을 저장한 후 추천을 [!DNL Target] 계산합니다. 이 계산은 수행하는 데 시간이 걸리고 기간은 선택한 권장 사항 로직, 데이터 범위, 카탈로그의 항목 수, 고객이 생성한 동작 데이터 및 선택한 행동 데이터 소스에 따라 달라집니다. 행동 데이터 소스는 처리 시간에 가장 큰 영향을 줍니다. 다음과 같습니다.

### mbox에 대해 Target Classic 캠페인이 실행되고 있지 않은지 확인합니다

mbox를 행동 데이터 소스로 선택한 경우, 만들어진 기준이 즉시 실행됩니다. 사용되는 동작 데이터의 분량과 카탈로그 크기에 따라 알고리즘을 실행하는 데 최대 12시간이 걸릴 수 있습니다. 일반적으로 기준 구성을 변경하면 알고리즘이 다시 실행됩니다. 수행된 변경 내용에 따라 다시 실행이 완료될 때까지 이전에 계산된 권장 사항을 사용할 수 있고, 더 큰 변경을 위해 다시 실행이 완료될 때까지 백업이나 기본 컨텐츠만 사용할 수 있습니다. 알고리즘이 수정되지 않으면 선택한 데이터 범위에 따라 12-48 시간 [!DNL Target] 간격으로 자동으로 다시 실행됩니다.
...
* **1 회 보고서 세트 설정**: 보고서 세트가 지정된 데이터 범위 확인 기간과 함께 처음 사용되는 경우, 2 ~ 7 일 동안 선택한 보고서 세트에 대한 행동 데이터를 완전히 다운로드할 수 [!DNL Target Recommendations] 있습니다 [!DNL Analytics]. 이 기간은 [!DNL Analytics] 시스템 로드에 따라 다릅니다.
* **이미 사용 가능한 보고서 세트를 사용하여 새 기준 또는 편집된 기준**: 새 기준을 만들거나 기존 기준을 편집할 때 선택한 보고서 세트를 이미 사용한 경우 선택한 데이터 [!DNL Target Recommendations]범위와 같거나 그보다 적은 데이터 범위가 있는 경우 데이터를 즉시 사용할 수 있으며 일회성 설정이 필요하지 않습니다. 이 경우, 선택한 보고서 세트나 데이터 범위를 수정하지 않고 알고리즘 설정이 편집된 경우 알고리즘이 12 시간 이내에 실행되거나 다시 실행됩니다.
* **지속적인 알고리즘 실행**: 데이터 흐름은 [!DNL Analytics][!DNL Target Recommendations] 매일 시작됩니다. 예를 들어 [!UICONTROL , 본 친화성] 권장 사항의 경우 사용자가 제품을 볼 때 제품-보기 추적 호출이 실시간에 [!DNL Analytics] 가깝게 전달됩니다. [!DNL Analytics] 데이터가 다음 날 [!DNL Target] 일찍 푸시되고 알고리즘이 12 시간 이내에 [!DNL Target] 실행됩니다.
...
>[!NOTE]
>
>최근에 본 항목은 활동에 대한 제외 글로벌 설정과 선택한 컬렉션 설정을 모두 준수합니다. 항목이 전역 제외에 의해 제외되거나 선택한 컬렉션에 포함되지 않은 경우, 표시되지 않습니다. 따라서 최근에 본 항목 기준을 사용할 때는 일반적으로 "모든 컬렉션" 설정을 사용해야 합니다.

### 이전에 구입한 항목 {#previously-purchased}
