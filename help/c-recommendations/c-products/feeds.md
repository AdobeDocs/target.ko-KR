...
많은 소매점의 경우 제품을 Google에 업로드하여, 방문자가 Google 제품 검색을 사용할 때 제품이 나타나도록 합니다. [!DNL Recommendations]는 정확히 개체 피드에 대해 Google의 사양을 따릅니다. 개체 [!DNL Recommendations] 피드는 [!DNL .xml], [!DNL .txt]또는 [!DNL .tsv]을 통해, 또는 Google에서 정의한 [특성을 사용할 수](https://support.google.com/merchants/answer/188494?hl=en&topic=2473824&ctx=topic#US)있습니다. 결과는 [Google 쇼핑 페이지](https://www.google.com/prdhp)에서 검색할 수 있습니다.

>[!NOTE]
>
>POST 방법은 Google 피드 컨텐츠를 호스팅하는 서버에서만 허용됩니다.

[!DNL Recommendations] 사용자는 이미 URL 또는 FTP를 통해 Google로 전송할 [!DNL .xml] 또는 [!DNL .txt] 피드를 구성했으므로, 개체 피드는 해당 제품 데이터를 승인하고 이를 사용하여 권장 사항 카탈로그를 구성합니다. 피드가 존재하며 추천 서버가 데이터를 검색하는 위치를 지정합니다.
...
Analytics 제품 분류는 추천에서 사용할 수 있는 분류입니다. 이 분류 파일에 대한 자세한 내용은 *Analytics 도움말 및 참조* 안내서의 [분류](https://marketing.adobe.com/resources/help/en_US/reference/classifications.html)를 참조하십시오. 추천에서 필요한 일부 정보는 현재 구현에서 사용하지 못할 수 있으므로, 분류 파일에 추가할 경우 이 사용자 안내서를 따르십시오.
...
[상태]에 &quot;성공&quot;이 표시되면 이것은 파일을 찾았으며 올바로 구문 분석했음을 의미합니다. 파일이 색인화되기 전까지는 [!DNL Recommendations]에서 이 정보를 사용할 수 없습니다. 색인화는 파일의 크기에 따라 시간이 좀 걸릴 수 있습니다. 프로세스가 실패한다면 이것은 파일을 찾지 못했거나(잘못된 URL을 사용했거나 FTP 정보가 올바르지 않은 경우 등) 구문 분석 오류가 있음을 의미합니다.
...
| 피드 실행 안 함 | 피드를 만들었지만 예약하지 않았습니다(빈도가 [사용 안 함]으로 설정됨). |
| 예약 일시: *날짜 및 시간* | 피드가 실행되지 않았지만 지정된 날짜 및 시간에 실행되도록 예약되었습니다. |
| 다운로드 대기 중 | Target에서 피드 파일을 다운로드할 준비를 하고 있습니다. |
| 피드 파일 다운로드 | Target에서 피드 파일을 다운로드하는 중입니다. |
| 항목 가져오기 | Target 이 피드 파일에서 항목을 가져오고 있습니다. 참고: 이 단계가 완료되고 &quot;검색 색인 업데이트 준비&quot; 가 표시되면, 항목 속성에 대한 변경 사항이 중앙 시스템으로 가져왔으며 60 분 내에 지리적 에지 노드에서 반환된 전달된 추천 컨텐트에 반영됩니다. |
| 검색 색인 업데이트 준비 | Target는 카탈로그 검색 색인을 업데이트할 준비를 하고 있습니다. 참고: 이 상태가 나열되면, 항목 속성에 대한 변경 사항이 이미 만들어지고, 아직 카탈로그 검색에 반영되지 않지만 제공된 권장 사항에 곧 반영됩니다. |
| 검색 색인 업데이트 | Target에서 카탈로그 검색 색인을 업데이트하고 있습니다. 참고: 이 상태가 나열되면, 항목 속성에 대한 변경 사항이 이미 만들어졌고, 아직 카탈로그 검색에 반영되지 않더라도 제공된 권장 사항에 반영될 수 있습니다. |
| 업데이트 완료 | Target 이 피드 파일과 관련된 모든 업데이트를 완료했습니다. |
...
>[!IMPORTANT]
>
>업로드된 엔티티는 61 일 후에 만료됩니다. 즉, Recommendations 활동이 중단되는 것을 방지하기 위해 적어도 60 일마다 피드 파일을 업로드해야 합니다. 60 일마다 한 번 이상 피드 파일 (또는 다른 엔티티 업데이트 메서드) 에 항목이 포함되어 있지 않으면 Adobe Target는 해당 항목을 더 이상 관련성이 없으며 카탈로그에서 제거합니다.
