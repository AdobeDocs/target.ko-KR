...
keywords: GDPR; EU; 유럽연합 (EU); 개인정보 보호; FAQ; FAQCalifornia Consumer Privacy Act; CCPA
...
캘리포니아 소비자 개인 정보 보호법 (CCPA) 는 2020 년 1 월 1 일부터 유효합니다. 디자인에 의해, California Regulator Leeway는 조정 작업을 수행하고 세부 사항을 명확히 합니다. 이것은 당연히 여러분이 많은 질문 사항을 가질 수 있음을 의미합니다. 질문이 있는 경우, 법률 관련 불확실성을 해결하고 법률, 운영 및 기술 요구 사항을 해결하기 위한 접근 방식을 이해하고 개발하려는 만큼 많은 기업이 단독으로 작업하고 있지 않습니다.

CCPA는 캘리포니아 소비자에게 개인 정보와 관련된 새로운 권리를 제공하고 캘리포니아에서 비즈니스를 수행하는 특정 법인에 대한 데이터 보호 책임을 부과한다. 이 법은 높은 수준에서 캘리포니아 시민에게 다음과 같은 권리를 부여합니다. (1) 정보 요청 (데이터 액세스), (2) 개인 정보 판매로부터 옵트아웃 (제 3 자와 정보 공유를 선택할 수 있는 매우 광범위하게 정의된 권리), (3) 개인 정보를 삭제하며, (4) 개인 정보가 공개 또는 판매되고 있다는 통지를 받을 수 있습니다.
...
Adobe Target 에만 적용되는 GDPR (General Data Protection Regulation) 및 CCPA (California Consumer Privacy Act) 에 대한 FAQ 입니다.
...
아니요. Adobe는 데이터 컨트롤러가 GDPR 및 CCPA 요구 사항을 충족시키는 데 도움을 줄 수 있는 중앙 방법을 제공합니다. 데이터 관리자는 각 솔루션에 직접 관여하지 않아도 됩니다.

Target를 비롯한 Experience Cloud 솔루션의 모든 GDPR 및 CCPA 요청은 현재 GDPR API 라는 중앙 Adobe API를 통해 수행됩니다. 그러면 API가 데이터 관리자의 Experience Cloud 솔루션 세트에 대한 요청을 완료합니다.
...
>[!NOTE]
>
>Adobe Target는 자사 및 타사 도메인 간 쿠키를 모두 지원하지만 GDPR 및 CCPA 에는 퍼스트 파티 Adobe Target 쿠키만 권장됩니다.

### Adobe Target은 동의 관리를 어떻게 처리합니까? {#section_C86BF5EE4FAA47039659850E7594A6BA}

GDPR 및 CCPA는 동의를 얻어야 하는 경우 변경되지 않고, 고객의 동의를 얻어야 합니다. 각 고객의 동의 전략은 자체적인 데이터 수집 및 사용 약관과 개인정보 보호정책에 따라 다릅니다. 동의 관리는 GDPR 및 CCPA에 대한 Target를 통해 지원되지 않습니다.
...
For more information on GDPR, CCPA and Adobe Launch, see [The Adobe Privacy JavaScript Library and GDPR](https://www.adobe.io/apis/cloudplatform/gdpr/services/allservices.html). 또한 위의 "Adobe Target 및 Adobe Launch 선택" 섹션도 참조하십시오.
...
Central Privacy Service의 요구 사항 이외에도 Target에 대한 유효한 GDPR 또는 CCPA 메시지는 다음을 포함합니다.
...
Target 에서는 Target에서 IP 난독화를 GDPR 또는 CCPA 구현 전략의 일부로 사용하도록 선택할 수 있습니다. For more information, see [Privacy](../../../c-implementing-target/c-considerations-before-you-implement-target/c-privacy/privacy.md#concept_639482A343DB4963A6144378E1D8D7F0).
