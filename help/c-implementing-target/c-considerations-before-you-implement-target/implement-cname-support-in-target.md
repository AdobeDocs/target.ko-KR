...
1. Adobe의 인증 기관(DigiCert)은 Adobe가 도메인 아래에 인증서를 생성할 수 있도록 인증되었는지 확인해야 합니다.
   ...
   DigiCert가 이 프로세스를 [DCV(Domain Control Validation)](https://docs.digicert.com/manage-certificates/dv-certificate-enrollment/domain-control-validation-dcv-methods/)호출하며, 아래 DCV 방법 중 하나 이상에 대해 이 프로세스가 완료될 때까지 Adobe는 도메인 아래에 인증서를 생성할 수 없습니다.

   * 가장 빠른 DCV 메서드는 __DNS CNAME 메서드로__, DNS CNAME 레코드(토큰 포함)를 DigiCert의 DCV 호스트 이름(dcv.digicert.com)을 가리키는 도메인에 추가합니다. 이 CNAME 레코드는 Adobe가 인증서를 생성할 권한이 있음을 DigiCert에 표시합니다. Adobe Client Care에서 필요한 DNS 레코드와 함께 지침을 보냅니다. 예:
      ...
      >[!NOTE]
      >
      >이러한 DCV 토큰은 30일 후 만료되며 Adobe Client Care에서 업데이트된 토큰을 수신하게 됩니다. CNAME 요청을 가장 빨리 확인하려면 요청을 제출하기 전에 요청된 모든 도메인에서 이러한 DNS를 변경할 수 있도록 준비하십시오.
   ...
   * 또한 DigiCert는 __이메일 방법을__&#x200B;시도하게 됩니다. 이 방법을 사용하면 도메인의 WHOIS 정보에 있는 주소와 미리 결정된 이메일 주소(admin, administrator, webmaster, hostmaster 및 postmaster `@[domain_name]`)에 이메일을 보냅니다. 자세한 내용은 [DCV 메서드 설명서를](https://docs.digicert.com/manage-certificates/dv-certificate-enrollment/domain-control-validation-dcv-methods/) 참조하십시오.
...
### 직접 인증서를 제공할 수 있습니까(BYOC라고도 함)? 그렇다면, 어떻게 해야 합니까?

예. 고유한 인증서를 제공할 수 있지만 권장하지 __않습니다__. Adobe가 인증서를 구매하고 제어할 때 SSL 인증서 라이프사이클의 관리가 Adobe와 고객 모두에게 매우 쉬워졌습니다. SSL 인증서는 매년 갱신되어야 합니다. 즉, Adobe Client Care는 적시에 Adobe에 새 인증서를 보내기 위해 매년 귀하에게 연락해야 합니다. 일부 고객은 인증서가 만료되면 브라우저가 연결을 거부하므로 매년 적시에 갱신된 인증서를 만드는 데 어려움을 겪을 수 있습니다. [!DNL Target]
...
2020년 9월 1일 이전에 발행된 인증서는 2년 인증입니다. 2020년 9월 1일 이후 발행된 인증서는 1년 인증입니다. 1년 인증서로 이동하는 방법에 대한 자세한 내용을 [여기에서](https://www.digicert.com/position-on-1-year-certificates)확인할 수 있습니다.

### 어떤 호스트 이름을 선택해야 합니까? 도메인당 호스트 이름은 몇 개입니까?

[!DNL Target] CNAME 구현에는 SSL 인증서 및 고객의 DNS에 도메인당 하나의 호스트 이름만 있으면 되므로 권장됩니다. 일부 고객은 고유한 목적(예: 스테이징에서 테스트)을 위해 도메인당 추가 호스트 이름을 요구할 수 있으며, 이는 지원됩니다.

대부분의 고객은 호스트 이름 같은 `target.example.com`것을 선택하므로 권장하지만 선택은 궁극적으로 귀하의 것입니다. 기존 DNS 레코드의 호스트 이름을 요청하지 마십시오. 이렇게 하면 CNAME 요청의 충돌 및 해결 시간이 지연됩니다. [!DNL Target]
...
1. 새로운 DNS CNAME을 다른 curl 요청과 함께 확인할 수 있습니다. `CN=target.example.com`다음과 같습니다.
   ...
   >[!NOTE]
   >
   >이 명령이 실패했지만 위의 `validateEdgeFpsslSni` 명령이 성공하면 DNS 업데이트가 완전히 전파될 때까지 기다려야 할 수 있습니다. DNS 레코드에는 관련 [TTL(time-to-live)](https://en.wikipedia.org/wiki/Time_to_live#DNS_records) 이 있으므로 해당 레코드의 DNS 답글에 대한 캐시 만료 시간이 지정되므로 TTL이 있는 한 기다려야 할 수 있습니다. 명령 또는 G Suite `dig target.example.com` 도구 상자를 사용하여 [](https://toolbox.googleapps.com/apps/dig/#CNAME) 특정 TTL을 조회할 수 있습니다.
