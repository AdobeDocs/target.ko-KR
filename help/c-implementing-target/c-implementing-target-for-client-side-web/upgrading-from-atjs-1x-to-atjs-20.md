---
description: at. js 1에서 업그레이드. * x * to at. js 2. * x *
keywords: at. js 릴리스; at. js 버전; 단일 페이지 앱; SPA; 크로스 도메인; 크로스 도메인
seo-description: Adobe Target. js 1에서 업그레이드하는 방법에 대한 자세한 정보입니다. * x * - at. js 버전 2.0.0
seo-title: . js 버전 1의 Adobe Target에서 업그레이드.* x * 에서. js 버전 2 로*x에는 사용할 수 없습니다*
solution: Target
subtopic: 시작하기
title: at. js 1에서 업그레이드. * x * to at. js 2. * x *
...
1. at.js 2 배포.*x*[를 클릭합니다.](/help/c-implementing-target/c-implementing-target-for-client-side-web/how-to-deployatjs/cmp-implementing-target-using-adobe-launch.md)
   ...
   . js 2에서 수동으로 다운로드하십시오.*x* 대상 UI를 사용하고 원하는 [방법으로 배포합니다](/help/c-implementing-target/c-implementing-target-for-client-side-web/how-to-deployatjs/how-to-deployatjs.md).
...
### can at. js 2.*x* 는 at. js 1의 일부 페이지에서 사용됩니다.*x*&#x200B;나 mbox.js와 다른 페이지에 있을 때 사용할 수 있습니까?
...
at.js 2.*x*, 더 이상 네트워크 호출에서 "`target-global-mbox`» 눈에 띄지 않습니다. 대신, 아래에서 보듯이 [!DNL Target] 서버에 전송된 JSON 페이로드에서 "`target-global-mbox`" 구문을 "`execute > pageLoad`"로 대체했습니다.
...
도메인 간 추적을 통해 여러 도메인 간에 방문자를 연결할 수 있습니다. 각 도메인에 대해 새 쿠키를 만들어야 하기 때문에 방문자가 도메인에서 도메인으로 이동할 때 방문자를 추적하기가 어렵습니다. To accomplish cross-domain tracking, [!DNL Target] uses a third-party cookie to track visitors across domains. This allows you to create a Target activity that spans `siteA.com` and `siteB.com` and visitors remain in the same experience when they navigate across unique domains. 이 기능은 Target의 타사 및 퍼스트 파티 쿠키 동작에 연결되어 있습니다.

>[!NOTE]
>
>크로스 도메인 추적은 at. js 2의 상자에서 지원되지 않습니다.*x*&#x200B;에는 사용할 수 없습니다. 크로스 도메인 추적은 at. js 2에서 지원됩니다.*x* (ECID) 라이브러리 v 4.3.0 +를 통한 X.

In Target, the third-party cookie is stored in `<CLIENTCODE>.tt.omtrdc.net`. The first-party cookie is stored in `clientdomain.com`. 첫 번째 요청은 `mboxSession` 및 `mboxPC`라는 타사 쿠키를 설정하는 HTTP 응답 헤더를 반환하는 반면, 리디렉션 요청이 추가 매개 변수(`mboxXDomainCheck=true`)를 사용하여 다시 전송됩니다. 브라우저가 서드 파티 쿠키를 수락하면 리디렉션 요청에 해당 쿠키가 포함되고 경험이 반환됩니다. 여기서는 HTTP GET 메서드를 사용하므로 이 워크플로우가 가능합니다.

However, in at.js 2.*x*, HTTP GET is no longer used and instead we use HTTP POST. 이제 HTTP POST가 at. js 2를 통해 사용됩니다.*x* 를 클릭하여 Target Edge Server에 JSON 페이로드 전송. 이것은 브라우저가 타사 쿠키를 지원하는지 여부를 확인하는 리디렉션 요청이 이제 중단됨을 의미합니다. HTTP GET 요청은 idempotent 트랜잭션이고 HTTP POST는 idempowerful 이 아니며 임의의 반복이 되지 않아야 하기 때문입니다. 따라서 at. js 2의 도메인 간 추적.*X* 는 더 이상 지원되지 않습니다. at. js 1 에만 해당됩니다.*X* 는 도메인 간 추적을 위한 기본 지원을 제공합니다.

If you want to use cross-domain tracking, you must install the [ECID library v4.3.0+](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-release-notes.html) in conjunction with at.js 2.*x*&#x200B;에는 사용할 수 없습니다. Ecid 라이브러리는 도메인 간에 방문자를 식별하는 데 사용되는 영구 ID를 관리하기 위해 존재합니다. ECID Library v 4.3.0 + 및 at. js 2를 설치한 후&#x200B;*X*&#x200B;를 사용하면 고유한 도메인을 확장하고 사용자를 추적할 수 있는 활동을 만들 수 있습니다.
...
## at.js 1.*x* 매개 변수를 at. js 2에 추가했습니다.*X* 페이로드 매핑 {#payload-mapping}
...
See [Understanding how at.js 2.*X* Work](https://helpx.adobe.com/target/kt/using/atjs20-diagram-technical-video-understand.html) for more information.