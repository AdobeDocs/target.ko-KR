...
태그 관리자를 사용하지 [!DNL Adobe Target] 않고 구현에 대한 정보 (Adobe Launch 또는 다이내믹 태그 관리)

## 태그 관리자 없이 Target 구현 {#topic_397FFA3D6918456BBE02A9FBE9537894}

태그 관리자(Adobe Launch 또는 동적 태그 관리)를 사용하지 않고 Adobe Target을 구현하는 방법에 대한 정보입니다.
...
## at.js 구성{#concept_2FA0456607D04F82B0539C5BF5309812}을 참조하십시오 

at.js [설정] 페이지에서 몇 가지 설정을 지정하는 데 도움이 되는 정보입니다.
...
이 설정을 변경하려면 먼저 고객 지원팀에 문의하십시오. 대부분의 구현에는 이 설정이 필요합니다.

| 설정 | 설명 |
|--- |--- |
| 글로벌 mbox를 자동으로 만들기 | 각 페이지 로드 시 자동으로 실행할 at.js 파일에 글로벌 mbox 호출을 포함할지 여부를 선택하십시오.<br>이 설정을 변경하는 것은 at.js와 mbox.js 모두에 영향을 줍니다. |
| 글로벌 mbox 이름 | 글로벌 mbox의 이름을 선택하십시오. 기본적으로 이 이름은 target-global-mbox입니다.<br>at.js를 사용하는 mbox 이름에 앰퍼샌드(&amp;)를 포함한 특수 문자를 사용할 수 있습니다.<br>이 설정을 변경하는 것은 at.js와 mbox.js 모두에 영향을 줍니다. |

## 고급 설정 {#section_33B697B77BA64A01B5939D7EC75231F2}
...
## Target 인터페이스를 사용한 at.js 다운로드 {#section_1F5EE401C2314338910FC57F9592894E}
...
## Target Download API를 사용한 at.js 다운로드 {#section_C0D9D2A9068144708D08526BA5CA10D0}

API를 사용하여 [!DNL at.js]를 다운로드하려면 다음을 수행하십시오.

1. 클라이언트 코드를 가져옵니다.

   클라이언트 코드는 [!DNL Target] 인터페이스의 **[!UICONTROL 설정]** &gt; **[!UICONTROL 구현]** &gt; **[!UICONTROL at.js 설정 편집]** 페이지의 맨 위에 있습니다.
...
* 사전 연결 및 미리 가져오기는 웹 페이지 로드 속도를 높일 수 있는 옵션입니다. 이러한 구성을 사용하는 경우 `<client code>`를 보유한 클라이언트 코드로 바꾸십시오. 클라이언트 코드는 **[!UICONTROL 설정]** &gt; **[!UICONTROL 구현]** &gt; **[!UICONTROL at.js 설정 편집]** 페이지에서 확인할 수 있습니다.
