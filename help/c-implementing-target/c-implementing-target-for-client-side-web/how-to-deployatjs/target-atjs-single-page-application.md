...
description: SPA(단일 페이지 응용 프로그램)에 Adobe [!DNL Target] at.js 2.x to implement [!DNL Target] 을 사용하는 방법을 알아봅니다.
title: 단일 페이지 애플리케이션(SPA)에 대해 [!DNL Target] 을 구현할 수 있습니까?
feature: 서버측 구현
role: Developer
...
1. at.js 2.x를 설치합니다.
   ...
   [!UICONTROL 관리 > 구현]에 있는 Adobe Target UI를 통해 at.js 2.x를 다운로드합니다. at.js 2.x는 [!DNL Adobe Experience Platform]에서 태그를 통해 배포할 수도 있습니다. 그러나 [!DNL Adobe Target] 확장은 현재 최신 상태가 아니며 지원되지 않습니다.
...
이러한 예에서 JavaScript 코드를 사용하지만 [Adobe Experience Platform](/help/c-implementing-target/c-implementing-target-for-client-side-web/how-to-deployatjs/cmp-implementing-target-using-adobe-launch.md)의 태그와 같은 태그 관리자를 사용하는 경우 이 모든 작업을 단순화할 수 있습니다.

위의 단계를 수행하는 경우 SPA에 강력한 A4T 솔루션이 있어야 합니다.

## 구현 모범 사례 {#bp}

at.js 2.x API를 사용하면 여러 가지 방법으로 [!DNL Target] 구현을 사용자 지정할 수 있지만, 이 프로세스 중에 올바른 작업 순서를 따라야 합니다.
...
| 단계 | Action | 세부 사항 |
| --- | --- | --- |
| 1 | VisitorAPI JS 로드 | 이 라이브러리는 방문자에게 ECID를 할당합니다. 이 ID는 나중에 웹 페이지의 다른 [!DNL Adobe] 솔루션에 의해 사용됩니다. |
| 2개 | at.js 2.x 로드 | at.js 2.x는 [!DNL Target] 요청 및 보기를 구현하는 데 사용하는 모든 필요한 API를 로드합니다. |
| 1 | [!DNL Target] 요청 실행 | 데이터 계층이 있는 경우 [!DNL Target] 요청을 실행하기 전에 [!DNL Target]에 보내야 하는 중요한 데이터를 로드하는 것이 좋습니다. 이렇게 하면 `targetPageParams` 을 사용하여 타깃팅에 사용할 데이터를 모두 전송할 수 있습니다. 이 API 호출에서 execute > pageLoad와 prefetch > 보기를 요청해야 합니다. `pageLoadEnabled` 및 `viewsEnabled`을 설정한 경우 execute > pageLoad 및 prefetch > 보기가 모두 2단계에서 자동으로 발생합니다. 그렇지 않은 경우 `getOffers()` API를 사용하여 이 요청을 수행해야 합니다. |
| 4 | 호출 `triggerView()` | 3단계에서 시작한 [!DNL Target] 요청은 페이지 로드 실행과 보기 둘 다에 대한 경험을 반환할 수 있으므로 [!DNL Target] 요청이 반환된 후 `triggerView()`이 호출되는지 확인하고 오퍼를 캐시에 적용을 완료합니다. 이 단계를 뷰당 한 번만 실행해야 합니다. |
| 5개 | [!DNL Analytics] 페이지 보기 비콘 호출 | 이 비콘은 데이터 결합을 위해 3단계 및 4단계와 연결된 SDID를 [!DNL Analytics]에 보냅니다. |
| 6 | 추가 `triggerView({"page": false})` 호출 | 이는 보기 변경이 발생하지 않고 페이지에서 특정 구성 요소를 다시 렌더링할 수 있는 SPA 프레임워크의 선택적 단계입니다. 이러한 경우 SPA 프레임워크에서 구성 요소를 다시 렌더링한 후 [!DNL Target] 경험이 다시 적용되도록 하려면 이 API를 호출하는 것이 중요합니다. 이 단계를 원하는 만큼 여러 번 실행하여 [!DNL Target] 경험이 SPA 보기에서 지속되도록 할 수 있습니다. |
...
| 2개 | `getOffers()` API를 호출하여 캐시를 업데이트합니다 | 이 보기 변경으로 현재 방문자에게 더 많은 [!DNL Target] 활동에 대한 자격이 있거나 활동에서 자격이 없는 경우 수행할 수 있는 선택 단계입니다. 이 시점에서 추가적인 타깃팅 기능을 활성화하기 위해 추가 데이터를 [!DNL Target]에 보내도록 선택할 수도 있습니다. |
| 1 | 호출 `triggerView()` | 2단계를 실행한 경우 [!DNL Target] 요청을 기다린 후 이 단계를 실행하기 전에 캐시에 오퍼를 적용해야 합니다. 이 단계를 뷰당 한 번만 실행해야 합니다. |
| 4 | 호출 `triggerView()` | 2단계를 실행하지 않은 경우 1단계를 완료하는 즉시 이 단계를 실행할 수 있습니다. 2단계 및 3단계를 실행한 경우 이 단계를 건너뛰어야 합니다. 이 단계를 뷰당 한 번만 실행해야 합니다. |
| 5개 | [!DNL Analytics] 페이지 보기 비콘 호출 | 이 비콘은 데이터 결합을 위해 2단계, 3 및 4와 연결된 SDID를 [!DNL Analytics]에 보냅니다. |
| 6 | 추가 `triggerView({"page": false})` 호출 | 이는 보기 변경이 발생하지 않고 페이지에서 특정 구성 요소를 다시 렌더링할 수 있는 SPA 프레임워크의 선택적 단계입니다. 이러한 경우 SPA 프레임워크에서 구성 요소를 다시 렌더링한 후 [!DNL Target] 경험이 다시 적용되도록 하려면 이 API를 호출하는 것이 중요합니다. 이 단계를 원하는 만큼 여러 번 실행하여 [!DNL Target] 경험이 SPA 보기에서 지속되도록 할 수 있습니다. |
...
자세한 내용은 [단일 페이지 애플리케이션(SPA)](https://helpx.adobe.com/target/kt/using/atjs2-single-page-application-technical-video-implement.html)에서 Adobe Target의 at.js 2.x 구현 을 참조하십시오.

### Adobe Target에서 SPA용 VEC 사용 ![자습서 배지](/help/assets/tutorial.png)
