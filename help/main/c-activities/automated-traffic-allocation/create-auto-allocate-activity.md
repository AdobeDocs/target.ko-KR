...
description: Adobe에서 VEC(시각적 경험 작성기)를 사용하는 방법을 알아봅니다 [!DNL Target] 자동 할당 A/B 테스트 활동을 직접 만들려면 [!DNL Target]-enabled 페이지.
title: 자동 할당 활동을 만들려면 어떻게 해야 합니까?
...
를 사용하십시오 [!UICONTROL 시각적 경험 작성기] (VEC)의 [!DNL Adobe Target] 를 클릭하여 [!UICONTROL 자동 할당] [!UICONTROL A/B 테스트] 활동에서 직접 [!DNL Target]-enabled 페이지 및 내에서 페이지의 일부 수정 [!DNL Target].

>[!NOTE]
>
>추가 [!UICONTROL 자동 할당] [!UICONTROL A/B 테스트] 활동(이 문서에서 설명한 내용), [!DNL Target] 에서는 두 가지 추가 유형을 제공합니다 [!UICONTROL A/B 테스트] 활동: [!UICONTROL 수동(기본값)] 및 [!UICONTROL 자동 Target].
>
