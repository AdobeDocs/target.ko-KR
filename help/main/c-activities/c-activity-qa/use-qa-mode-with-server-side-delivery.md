...
활동 QA의 표준 구현에서는 `qa_mode` 매개 변수를 통해 `pageUrl` 매개 변수를 전달할 수 있습니다. 이 방법은 standard/ajax [!DNL Target] 호출에 편리합니다. 그러나 서버 간 호출의 경우 `pageUrl`을 사용할 수 없는 모바일 SDK 사례에는 적합한 방법이 아닙니다.
