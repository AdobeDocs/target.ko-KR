---
keywords: AB;A/B;AB...n;실수;함정;실수;함정;중요도;승자;통계적 차이;통계적;통계 전력;트래픽 할당;할당
description: Adobe에서 A/B 테스트를 수행할 때 회사에서 수행하는 가장 일반적인 위험 및 실수를 방지하는 방법을 알아봅니다 [!DNL Target] 및 기타 테스트 솔루션.
...
# 10가지 일반적인 A/B 테스트 위험 및 이를 피하는 방법

의 A/B 테스트 [!DNL Adobe Target] 는 대부분의 디지털 마케팅 최적화 프로그램의 중추를 구성하며 마케터가 방문자와 고객에게 최적화된 타깃팅된 경험을 제공할 수 있도록 합니다. 이 섹션에서는 A/B 테스트를 수행할 때 기업이 희생하는 가장 중요한 위험 요소 중 10가지를 간략하게 설명합니다. 또한 여기에는 회사가 테스트 노력을 통해 더 많은 ROI를 달성하고 보고된 A/B 테스트 결과를 더욱 신뢰할 수 있도록 이러한 위험을 방지하는 방법도 포함되어 있습니다.
...
## 위험 2: 통계적으로 유의미한 차이가 없는 여러 오퍼 테스트에 대한 승자 선언 {#section_FA83977C71DB4F69B3D438AF850EF3B6}
...
## 위험 3: 통계적 검정력의 효과 무시 {#section_0D517079B7D547CCAA75F80981CBE12A}
...
## 4단계: 편측 검정 사용 {#section_8BB136D1DD6341FA9772F4C31E9AA37C}
...
## 위험 6: 테스트 조기 중지 {#section_DF01A97275E44CA5859D825E0DE2F49F}
...
## 위험 7: 테스트 기간 동안의 트래픽 할당 변경 {#allocation}

데이터가 정규화될 때까지 테스트 결과를 왜곡할 수 있으므로 테스트 기간 동안 트래픽 할당 비율을 변경하지 않는 것이 좋습니다.
예를 들어 트래픽의 80%가 경험 A(제어)에 할당되고 트래픽의 20%가 경험 B에 할당되는 A/B 테스트 활동이 있다고 가정해 봅시다. 테스트 기간 동안 각 경험에 대한 할당을 50%로 변경합니다. 며칠 후 트래픽 할당을 100%로 경험 B로 변경합니다.

이 시나리오에서는 사용자에게 경험에 어떻게 지정됩니까?

경험 B에 대해 할당 분할을 100%로 수동으로 변경하는 경우, 원래 경험 A(제어)에 할당된 방문자는 처음에 지정된 경험 A에 유지됩니다. 트래픽 할당의 변경은 새 참여자에게만 영향을 줍니다.

백분율을 변경하거나 각 경험으로의 방문자 흐름에 큰 영향을 미치려면 새 활동을 만들거나 활동을 복사한 다음 트래픽 할당 비율을 편집하는 것이 좋습니다.

테스트 기간 동안 서로 다른 경험에 대한 백분율을 변경하는 경우 데이터가 정규화하는 데 며칠이 걸립니다. 특히 많은 구매자가 재방문자를 보내는 경우에 해당됩니다.
다른 예로, A/B 테스트의 트래픽 할당이 50/50으로 분할된 다음 분할을 80/20으로 변경하면 변경 후 처음 며칠 동안 결과가 기울어진 것으로 보일 수 있습니다. 평균 전환 시간이 높은 경우, 이는 구매에 몇 시간 또는 며칠이 소요됨을 의미합니다. 이러한 지연된 전환은 보고서에 영향을 줄 수 있습니다. 따라서 숫자가 50%에서 80%로 증가했으며 평균 전환 시간은 2일인 첫 번째 경험에서 현재 인구의 80%가 경험으로 들어가고 있지만 50%의 방문자만 테스트 첫 날에 전환됩니다. 이렇게 하면 전환율이 하락한 것처럼 보이지만, 이 80%의 방문자가 이틀 동안 전환한 후 다시 정규화됩니다.
...
## 위험 9: 고려 기간에서 차이점을 고려하지 않음 {#section_B166731B5BEE4E578816E351ECDEA992}
...
## 위험 10: 비즈니스 목표를 반영하지 않는 지표 사용 {#section_F0CD6DC7993B4A6F9BEEBB31CD1D9BEE}
...
## 결론: 위험 요소를 인식하고 단계별로 단계별로 안내하여 A/B 테스트의 성공 {#section_54D33248163A481EBD4421A786FE2B15}
