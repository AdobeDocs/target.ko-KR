...
>[!TIP]
>
>[!DNL Target]은(는) [!UICONTROL 수동]&#x200B;(기본값) [!UICONTROL A/B 테스트] 활동(이 문서에서 설명됨) 외에도 두 가지 유형의 [!UICONTROL A/B 테스트] 활동을 추가로 제공합니다. [!UICONTROL 자동 할당] 및 [!UICONTROL 자동 타겟]. 자세한 내용은 아래 [A/B 테스트 활동 유형](#types)을 참조하십시오.
...
수동 [!UICONTROL A/B 테스트] 활동 외에 [!DNL Target]에서는 두 가지 유형의 [!UICONTROL A/B 테스트] 활동을 추가로 제공합니다. [!UICONTROL 자동 할당] 및 [!UICONTROL 자동 타겟].

| 활동 유형 | 설명 |
| --- | --- |
| [!UICONTROL 수동 A/B 테스트] | 사전 지정된 테스트 기간 동안 전환을 최고로 향상시킨 경험을 확인하기 위해 둘 이상의 경험을 비교합니다.<P>이 섹션에서는 수동 [!UICONTROL A/B 테스트] 활동을 설정하는 방법에 대해 설명하지만 다른 유형의 [!UICONTROL A/B 테스트] 활동에 대한 단계는 유사합니다. |
...
세 가지 유형의 [!UICONTROL A/B 테스트] 활동을 만드는 단계는 유사합니다. [!UICONTROL 자동 할당] 또는 [!UICONTROL 자동 타겟] 활동을 만들려면:

1. [A/B 테스트 활동 만들기](/help/main/c-activities/t-test-ab/t-test-create-ab/test-create-ab.md)부터 시작합니다.
1. [!UICONTROL 타깃팅] 페이지로 이동하면 [!UICONTROL 트래픽 할당] 컨트롤을 클릭한 다음 아래와 같이 오른쪽 창에서 원하는 트래픽 할당 방법을 선택합니다.
