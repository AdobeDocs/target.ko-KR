...
해결책은 [!UICONTROL 자동 할당]. [!UICONTROL 자동 할당은 이러한 비용과 우승 경험을 알아내는 데 드는 오버헤드를 줄여줍니다. ] [!UICONTROL 자동 할당 기능에서는 모든 경험의 목표 지표 성과를 모니터링하고, 성과가 좋은 경험에는 비례하여 더 많은 새 참여자를 보냅니다. ] 다른 경험을 탐색하도록 충분한 트래픽이 예약되어 있습니다. 활동이 여전히 실행 중인 동안에도 활동의 이점을 확인할 수 있습니다. 최적화는 학습과 동시에 발생합니다.
...
이 문서에서는 샘플 크기를 결정할 때 균형 조정되어야 하는 요인들을 설명하고 적절한 샘플 크기를 추정하는 계산기를 소개합니다. A/B 테스트가 시작되기 전에 샘플 크기 계산기(위에 제공된 링크)를 사용하여 샘플 크기를 계산하면 통계적 표준을 준수하는 고품질 A/B 테스트를 항상 실행할 수 있습니다.
...
![probability_distribution 이미지](assets/probability_distributions.png)
...
![probability_distribution2 이미지](assets/probability_distributions2.png)
...
이 예에서는 5%의 상승도를 감지할 수 있다고 결정할 수 있습니다(대체 오퍼의 전환율(100%+5%)에 해당합니다&#42;5% = 5.25%) 100개의 테스트 중 80개에서 적절한 경우에는 각 오퍼에 대해 100,000명의 방문자 샘플 크기가 필요합니다. 사이트에 하루에 20,000명의 방문자가 있고 두 개의 오퍼를 테스트하는 중이라면, 테스트를 2회 동안 실행할 수 있어야 합니다&#42;100,000/20,000 = 10일 전에 대체 오퍼가 통제 오퍼보다 통계적으로 유의미하게 우수한지 여부를 결정할 수 있습니다.
...
RPV(방문당 매출)를 지표로 사용할 때 RPV가 주문당 매출 및 전환율의 제품이므로 분산의 추가 소스가 추가됩니다(RPV = 매출 / #visitors = (주문당 수입) &#42; #orders) / # 방문자 = 주문당 매출 &#42; (#visitors) &#42; CTR) / #visitors = 주문당 매출 &#42; CTR), 각각 자체 분산이 있습니다. 전환율의 분산은 수학 모델을 사용하여 직접 추정할 수 있지만 주문당 수익의 분산은 활동에 따라 다릅니다. 따라서, 과거 활동에서 이러한 분산에 대한 지식을 사용하거나 며칠 동안 A/B 테스트를 실행하여 매출액의 분산을 예측합니다. 분산은 CSV 다운로드 파일에 있는 판매 합계, 판매 제곱 합계 및 방문자 수 값에서 계산됩니다. 설정이 완료되면 스프레드시트를 사용하여 테스트를 완료하는 데 필요한 시간을 계산합니다.
...
일반적으로 RPV를 지표로 사용하려면 동일한 측정 수준 향상도에 대해 동일한 수준의 통계적 신뢰도를 달성하려면 20~30%가 더 필요합니다. 이것은 RPV가 전환당 다른 주문 크기의 분산이 추가되었기 때문입니다. 이는 최종 비즈니스 결정을 기반으로 할 지표로 직접 전환율과 RPV 중 하나를 선택할 때 고려해야 합니다.
