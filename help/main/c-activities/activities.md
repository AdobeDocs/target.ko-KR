...
| [!UICONTROL Show filters] 아이콘<P>![필터 표시 아이콘](/help/main/assets/icons/Filter.svg) | [!UICONTROL Type], [!UICONTROL Status], [!UICONTROL Reporting Source], [!UICONTROL Experience Composer], [!UICONTROL Metrics Type], [!UICONTROL Activity Source] 및 [!UICONTROL Properties]별로 활동을 필터링하려면 목록 맨 위에 있는 **[!UICONTROL Show Filters]** 아이콘을 클릭하여 필터에 액세스하십시오.<P>자세한 내용은 아래의 [활동 목록에 필터 적용](#filters)을 참조하세요. |
| 필드 검색 | 활동을 빠르게 찾거나 [!UICONTROL Activity] 목록에 표시되는 활동 수를 줄이십시오. [!UICONTROL Activity Name], [!UICONTROL URL] 또는 [!UICONTROL ID]별로 검색할 수 있습니다. |
