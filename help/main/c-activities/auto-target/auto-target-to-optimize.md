...
>
>* [!UICONTROL Target 분석] (A4T)에서 [!UICONTROL 자동 Target] 활동. 자세한 내용은 [자동 할당 및 자동 타겟 활동에 대한 A4T 지원](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)을 참조하십시오.


## 자동 Target을 사용한 실제 성공 사례 {#success}

최근 한 주요 의류 소매업체에서 [!UICONTROL 자동 Target] 활동은 10개의 카테고리 기반 경험(및 무작위 지정 제어)을 사용하여 각 방문자에게 올바른 컨텐츠를 전달합니다. &quot;[!UICONTROL 장바구니에 추가]&quot;이(가) 기본 최적화 지표로 선택되었습니다. 타깃팅된 경험의 평균 상승도는 29.09%였습니다. 작성 후 [!UICONTROL 자동 Target] 모델, 활동이 90% 개인화된 경험으로 설정되었습니다.

불과 10일 만에 170만 달러 이상의 리프트가 탄생했다.

사용 방법을 배우려면 계속 읽기 [!UICONTROL 자동 Target] 조직의 상승도 및 매출을 늘리려면 다음을 수행하십시오.
...
A/B 활동 흐름 내의 [!UICONTROL 자동 타겟] 선택 사항을 사용하면 한 번의 클릭으로 마케터가 정의한 일련의 경험들을 기반으로 개인화하는 기계 학습을 활용할 수 있습니다. [!UICONTROL 자동 Target] 는 기존 A/B 테스트나 [!UICONTROL 자동 할당]: 각 방문자에 대해 표시할 경험을 결정합니다. 목표가 단일 승자를 찾는 것인 A/B 활동과는 달리, [!UICONTROL 자동 타겟]은 주어진 방문자에 대해(방법 프로필과 기타 상황 정보를 기반으로) 최고의 경험을 자동으로 결정하여 높은 수준으로 개인화된 경험을 제공합니다.

와 유사함 [!UICONTROL Automated Personalization], [!UICONTROL 자동 Target] 사용 [Random Forest 알고리즘](/help/main/c-activities/t-automated-personalization/algo-random-forest.md)선도적인 데이터 과학 앙상블 방법인 를 사용하여 방문자에게 표시할 최고의 경험을 결정합니다. [!UICONTROL 자동 타겟]은 방문자의 행동 변화에 적응할 수 있으므로 상승도를 제공하도록 지속적으로 실행할 수 있습니다. 이를 때로 &quot;항시적&quot; 모드라고도 합니다.
...
* 에서는 특정 활동을 전환할 수 없습니다 [!UICONTROL 자동 Target] to [!UICONTROL Automated Personalization], 또는 그 반대로.
* 전환할 수 없습니다 [!UICONTROL 수동] 트래픽 할당(기존 [!UICONTROL A/B 테스트]) [!UICONTROL 자동 Target]활동을 초안으로 저장한 후 그 반대로 할 수 있습니다.
...
|--- |--- |--- |
| **개인화 알고리즘 평가(50/50)**: 목표가 알고리즘을 테스트하는 것이면 제어 및 타깃팅된 알고리즘 간에 50/50%로 분할된 방문자를 사용합니다. 이 분할은 가장 정확한 상승도 추정치를 제공합니다. 임의 경험을 제어로 사용하는 것이 좋습니다. | 50% 통제/50% 개인화 경험 분할 | <ul><li>통제 경험과 개인화된 경험 간 상승도의 정확성을 극대화합니다.</li><li>개인화된 경험을 가진 방문자가 상대적으로 적음</li></ul> |
...
를 조정하려면 [!UICONTROL 제어] 백분율에서 아이콘을 클릭합니다 [!UICONTROL 할당] 열. 통제군은 10% 미만으로 줄일 수 없습니다.
...
다음과 같은 몇 가지 시나리오를 사용할 수 있습니다 [!UICONTROL 자동 Target] over [!UICONTROL Automated Personalization]:

* 한 경험을 형성하기 위해 자동으로 결합하게 되는 개별 오퍼들이 아닌 전체 경험을 정의하려는 경우.
* 전체 [!UICONTROL 시각적 경험 작성기] (VEC) 기능은에서 지원하지 않습니다. [!UICONTROL 자동 개인화]: 사용자 지정 코드 편집기, 여러 경험 대상 등.
* 다양한 경험에서 내 페이지의 구조를 변경하려는 경우. 예를 들어, 홈 페이지에서 요소를 다시 배열하려면 [!UICONTROL 자동 Target] 보다 적합한 [!UICONTROL Automated Personalization].

## 기능 [!UICONTROL 자동 Target] 와(과) [!UICONTROL Automated Personalization]? {#section_2A601F482F9A44E38D4B694668711319}

### 알고리즘이 각 방문에 대해 유용한 결과를 내놓도록 최적화됩니다.

* 이 알고리즘은 최상의 경험을 제공하기 위해 방문자의 전환 성향(또는 전환으로 인한 예상 매출액)을 예측합니다.
...
* Multi-arm bandit 은 모델이 항상 적은 양의 트래픽을 소비하며 활동 학습의 수명 동안 계속 학습하고 이전에 학습한 트렌드를 과도하게 이용하는 것을 방지하기 위해 노력하고 있음을 보장합니다.
* 기본 모델은 최신 방문자 행동 데이터를 사용하여 24시간마다 다시 빌드되므로 [!DNL Target] 는 항상 변화하는 방문자 환경 설정을 사용합니다.
...
* 이 지표는 전환 기반 또는 매출 기반(좀 더 구체적으로 설명됨)일 수 있습니다 [!UICONTROL 방문당 수입]).

### [!DNL Target] 방문자에 대한 정보를 자동으로 수집하여 개인화 모델을 구축합니다.
...
### [!DNL Target] 자동으로 모두 사용 [!DNL Adobe Experience Cloud] 공유 대상을 사용하여 개인화 모델을 구축합니다.
...
예를 들어 [!UICONTROL 자동 개인화] 두 개의 위치에 위치당 두 개의 오퍼를 만든 활동에는 활동에 총 4개(2 = 4)의 경험이 포함되어 있습니다(제외 없음). [!UICONTROL 자동 타겟]을 사용하여 경험 1을 위치 1에서 오퍼 1을 포함하고, 위치 2에서 오퍼 2를 포함하도록 설정하고, 경험 2를 위치 1에서 오퍼 1을 포함하고, 위치 2에서 오퍼 2를 포함하도록 설정할 수 있을 것입니다. [!UICONTROL 자동 타겟]을 사용하면 한 경험 내에 여러 가지 변경이 생기도록 선택할 수 있으므로 활동에 있는 총경험 수를 줄일 수 있습니다.
...
* 왜냐면 [!UICONTROL 자동 Target] 는 A/B 활동 워크플로우에 포함되어 있습니다. [!UICONTROL 자동 Target] 보다 성숙하고 완전한 혜택으로 얻을 수 있는 이점 [!UICONTROL 시각적 경험 작성기] (VEC). 를 사용할 수도 있습니다 [QA 링크](/help/main/c-activities/c-activity-qa/activity-qa.md) with [!UICONTROL 자동 Target].

### 자동 타겟에서는 광범위한 온라인 테스트 프레임워크를 제공합니다.

* Multi-arm bandit 은 [!DNL Adobe] 데이터 과학자 및 연구자들이 실제 환경 개선에 따른 이점을 이해할 수 있습니다.
* 앞으로 이 시험대는 우리가 문을 열 수 있게 해줄 것이다 [!DNL Adobe] 데이터 기반 클라이언트에게 시스템 학습 플랫폼 제공 [!DNL Target] 모델.
