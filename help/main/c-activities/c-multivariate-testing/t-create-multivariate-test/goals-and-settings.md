...
description: Adobe에서 목표 및 설정 페이지를 사용하는 방법을 알아봅니다 [!DNL Target] mvt(다변량 테스트) 활동의 목표에 대한 정보를 지정하려면 를 선택합니다.
...
Adobe Target의 목표 및 설정 페이지에서 목표 정보를 입력할 수 있습니다 [!UICONTROL 다변량] (MVT) 테스트.
...
이 옵션이 S에서 활성화되지 않은 경우[!UICONTROL 관리] > [!UICONTROL 보고] (기본값) 우선순위를 지정합니다. 낮음, 중간 또는 높음.
