...
description: 사용 방법 알아보기 [!UICONTROL 목표 및 설정] 페이지 위치 [!DNL Adobe Target] 의 목표에 대한 정보를 지정하려면 [!UICONTROL 다변량 테스트] (MVT) 활동.
title: 에서 목표 및 설정을 지정하는 방법 [!UICONTROL 다변량 테스트] (MVT) 활동
...
다음 [!UICONTROL 목표 및 설정] 페이지 위치 [!DNL Adobe Target] 은 (는) 의 목표에 대한 정보를 입력하는 곳입니다. [!UICONTROL 다변량 테스트] (MVT) 활동.
...
이 옵션이에서 활성화되지 않은 경우 [!UICONTROL 관리] > [!UICONTROL 보고] (기본값), 우선 순위를 지정합니다. [!UICONTROL 낮음], [!UICONTROL Medium], 또는 [!UICONTROL 높음].
...
이전 버전에서 만든 활동의 경우 [!DNL Target], [!UICONTROL 낮음] 우선 순위가 0으로 전환됩니다. [!UICONTROL Medium] 우선 순위는 5로 변환되며, [!UICONTROL 높음] 우선 순위는 10으로 변환됩니다. 필요에 따라 이러한 값을 조정할 수 있습니다.
...
다음에서 데이터를 수집할지 여부 지정 [!DNL Adobe Target] 또는 부터 [!DNL Adobe Analytics]. 다음을 참조하십시오 [Target용 보고 소스로서의 Adobe Analytics](/help/main/c-integrating-target-with-mac/a4t/a4t.md) (A4T) 보고 솔루션과 각 솔루션의 이점 간의 차이점에 대해 알아봅니다.

선택 시 [!DNL Analytics] 을(를) 위한 보고 소스로 사용 [!DNL Target], 다음을 선택합니다. [!DNL Analytics] 받을 보고서 세트 [!DNL Target] 활동 데이터. 이렇게 하려면 먼저 다음 중 하나를 선택합니다 [!DNL Analytics] 계정이 연결된 회사 를 선택한 다음 활동에 대한 보고서 세트를 선택합니다. 연결할 수 있도록 프로비저닝된 보고서 세트만 [!DNL Target] 선택할 수 있습니다. 예상하는 보고서 세트가 표시되지 않으면 먼저 로그아웃했다가 다음에 다시 로그인하십시오 [!DNL Adobe Experience Cloud] 다시 시도하십시오. 보고서 세트가 여전히 목록에서 누락된 경우 [고객 지원 센터](/help/main/cmp-resources-and-contact-information.md#reference_ACA3391A00EF467B87930A450050077C).
...
고급 설정은 다음에 사용할 수 있습니다. [!UICONTROL 다변량 테스트] 목표 지표.
...
* [!UICONTROL 카운트가 증가되는 방식을 지정하려면 증분 카운트 및 사용자를 활동에 유지를 선택합니다.]
* [!UICONTROL 사용자가 활동에 다시 입장하면 표시되는 경험을 지정하려면 증분 카운트, 사용자 해제 및 재입력 허용을 선택합니다.]
* [!UICONTROL 활동 콘텐츠 대신 사용자에게 표시되는 콘텐츠를 지정하려면 증분 카운트, 재입력에서 사용자 및 막대 해제를 선택합니다.]
