...
# [!DNL Target] 보고 위치 [!DNL Adobe Customer Journey Analytics]
...
*마케터는 다음에 대한 단일 보고 소스를 가질 수 있습니다. [[!DNL Adobe Journey Optimizer]](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/cja-ajo){target=_blank} 및 [!DNL Target]. 두 개인화 제품 모두 [!DNL Customer Journey Analytics]에 연결하여 웹 개인화를 보다 전체적으로 살펴볼 수 있습니다.

## 고려 사항

를 사용하기 전에 다음 정보를 고려하십시오. [!DNL Customer Journey Analytics] 및 [!DNL Target] 통합:

>[!IMPORTANT]
>
>이 통합은 와 동일하지 않습니다. [[!UICONTROL Adobe Analytics for Target]](/help/main/c-integrating-target-with-mac/a4t/a4t.md) (A4T). 구현 및 지원되는 활동 유형이 다릅니다. 용으로 이 통합을 사용하기 전에 이 문서를 완전히 읽어야 합니다. [!DNL Target] 활동.
...
* 만들려면 [!DNL Target] 활동(포함) [!DNL Customer Journey Analytics] 보고 시 &quot;[!UICONTROL Approver]&quot; 또는 &#39;[!UICONTROL Editor]의 &quot;역할 [!DNL Target].
...
* 모든 기존 항목 [!DNL Target] 활동은 계속 사용합니다. [!DNL Target] 데이터 수집 및 은 이 통합을 활성화해도 영향을 받지 않습니다.
* 이 통합을 사용하려면 기본 구현 방법이 다음과 같습니다 [[!DNL Adobe Experience Platform]](https://experienceleague.adobe.com/en/docs/experience-platform){target=_blank} and [!DNL Target] implemented through the [[!DNL Adobe Experience Platform Web SDK]](https://experienceleague.adobe.com/en/docs/target-dev/developer/client-side/aep-web-sdk){target=_blank}.

  현재 구현된 [!DNL Adobe Experience Platform Web SDK]를 보유하고 있지 않다면 [[!DNL Adobe Analytics] 소스 연결](https://experienceleague.adobe.com/en/docs/experience-platform/sources/ui-tutorials/create/adobe-applications/analytics)을 만들어 데이터를 [!DNL Adobe Experience Platform]으로 가져오는 방법도 고려해 볼 수 있습니다.

  >[!NOTE]
  >
  >를 사용하는 경우 [!DNL Adobe Analytics] 소스 연결입니다. 두 위치에 모두 보고서가 있습니다. [!DNL Adobe Analytics] 및 [!DNL Customer Journey Analytics]. 그러나 이 두 가지 솔루션 간에 알고리즘이 다르기 때문에 결과는 일치하지 않을 수 있습니다.
...
1. 위치 [!DNL Customer Journey Analytics], **[!UICONTROL Connections]** 페이지, 클릭 **[!UICONTROL Create a new connection]**.

   ![에서 새 연결 링크 만들기 [!DNL Customer Journey Analytics]](/help/main/c-integrating-target-with-mac/cja4t/assets/create-connection.png)
...
1. 추가 **[!UICONTROL Adobe Target Classification Events]** 데이터 세트를 조회한 다음 **[!UICONTROL Next]**.
...
1. 이벤트 데이터 세트를 구성하십시오.
   ...
   자세한 내용은 [데이터 세트 추가 및 구성](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-connections/create-connection#add-dataset){target=_blank} 위치: *연결 만들기* 다음에서 *[!DNL Adobe Customer Journey Analytics]안내서*.

1. 를 사용하여 조회 데이터 세트 구성 [!UICONTROL Key] 필드를 &quot;key&quot;로 사용하고 [!UICONTROL Matching] 다음 경로가 있는 키 필드:
...
1. 클릭 **[!UICONTROL Add datasets]**&#x200B;을 클릭한 다음 을 클릭합니다 **[!UICONTROL Save]** 다음 화면에서 연결을 완료합니다.
...
1. 데이터 보기를 설정하고 위에서 만든 연결을 가리키는지 확인합니다.
   ...
   자세한 내용은 [데이터 보기 만들기 또는 편집](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-dataviews/create-dataview){target=_blank} 다음에서 *[!DNL Adobe Customer Journey Analytics]안내서*.
...
1. 사용 [!DNL Target] 의 차원 [!UICONTROL Experimentation] 패널, 다음 컨텍스트 레이블을 설정합니다.

   * 대상 [!UICONTROL Activity Name], &quot;실험&quot;을 사용합니다.
   * [!UICONTROL Experience Name], &quot;실험 변형&quot;을 사용합니다.

   ![실험 패널의 컨텍스트 레이블](/help/main/c-integrating-target-with-mac/cja4t/assets/context-labels.png){width="600" zoomable="yes"}

1. 다른 필드 설정을 완료한 다음 **[!UICONTROL Save and continue]** 완료 시.
