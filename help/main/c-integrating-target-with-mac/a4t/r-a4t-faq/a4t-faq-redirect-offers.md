...
at.js 버전 1.6.3 이상을 사용하는 경우, 두 페이지에서 페이지 보기를 카운트하는 것은 문제가 되지 않습니다. 이 경합 조건은 이전 버전을 사용하는 고객에게만 영향을 줍니다. Target 팀에서는 at.js의 현재 버전과 바로 전 버전, 이렇게 두 버전을 유지 관리합니다. [지원되는 버전](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/target-atjs-versions.md)을 실행 중인지 확인하려면 at.js를 필요에 따라 업그레이드하십시오.
...
IT 팀과 협력하여 이러한 매개 변수( `adobe_mc_sdid` 및 `adobe_mc_ref`) 허용 목록에추가된.
...
방문자가 링크를 클릭한다고 가정합니다 [!DNL `www.google.com`] 홈 페이지(`www.mysite.com/index.html`)에서 리디렉션 활동이 라이브 상태인 다음 새 페이지로 리디렉션됩니다(`www.mysite.com/index2.html`).
...
예, Platform Web SDK를 통해 A4T가 지원됩니다 [리디렉션 오퍼](/help/main/c-experiences/c-manage-content/offer-redirect.md).
