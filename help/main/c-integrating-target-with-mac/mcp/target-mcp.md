...
[!DNL Adobe Target] MCP 서버는 활동, 대상, 오퍼 및 구현 구성에 대한 읽기 및 쓰기 액세스를 모두 제공합니다. 통합을 통해 다음과 같은 작업을 수행할 수 있습니다.
...
예. MCP 서버는 A/B 및 경험 타기팅 활동 만들기, 활동 구성 업데이트, 트래픽 분할 관리, 변형 추가 또는 제거, 활동 활성화, 일시 중지 및 비활성화를 포함한 쓰기 작업을 지원합니다. 쓰기 작업에는 편집자 역할 이상이 필요하고, 활성화 및 비활성화에는 승인자 역할이 필요합니다.
+++
...
쓰기 도구에는 안전 주석과 확인 게이트가 포함되어 있으므로 명시적인 사용자 확인 없이 상태 변경 작업이 실행되지 않습니다. Adobe에서는 프로덕션에서 쓰기 도구를 사용하기 전에 샌드박스 환경에서 통합을 테스트하고, 모든 MCP에서 시작한 작업을 주의 깊게 검토한 후 확인하는 것을 권장합니다.
+++

+++[!DNL Adobe Target]에서 어떤 권한이 필요합니까?

**관찰자** 역할 이상은 모든 읽기 전용 도구에 대한 액세스 권한을 부여합니다. 쓰기 도구(만들기, 업데이트)에는 **Editor** 이상의 역할이 필요합니다. 상태 변경 도구(활성화, 비활성화)에는 **승인자** 역할이 필요합니다. 현재 액세스 수준을 잘 모를 경우 [!DNL Adobe Target] 관리자에게 문의하십시오.
+++
