...
1. **[!UICONTROL 알고리즘 유형]** 및 **[!UICONTROL 알고리즘]** 선택:
   ...
   >[!NOTE]
   >
   >**[!UICONTROL 비슷한 특성을 가진 항목]**/ **[!UICONTROL 미디어]**&#x200B;를 선택하면 [콘텐츠 유사성 규칙](#similarity)을 설정할 수 있는 옵션이 제공됩니다.

1. 필요에 따라 **항목 특성** 및 **일치하는 프로필 특성**, **권장 사항 키**, **필터링 키** 및/또는 **분석 지표**&#x200B;를 선택하여 알고리즘을 구성하십시오.

나머지 알고리즘 구성 옵션은 선택한 알고리즘에 따라 달라집니다. 알고리즘 구성을 완료하려면 [!UICONTROL 권장 사항 키], [!UICONTROL 필터링 키], [!UICONTROL 동시 발생 기준], [!UICONTROL 분석 지표] 및/또는 [!UICONTROL 항목 특성] 및 [!UICONTROL 일치하는 프로필 특성]을 선택하세요.

[!UICONTROL 프로필 특성별 가장 많이 본 항목] 또는 [!UICONTROL 프로필 특성별 최상위 판매자]를 선택하는 경우 **[!UICONTROL 프로필 특성]**&#x200B;을(를) 선택하여 인터랙션 데이터를 그룹화합니다. 프로필 특성은 지역 또는 국가와 같은 방문자 특성을 캡처하는 프로필 스크립트에서 가져오며, 스크립트 이름에는 `recsAttribute` 접두사가 있어야 하므로 `user.recsAttribute<Name>`(예: `user.recsAttributeRegion`)으로 저장됩니다. 배달 시 [!DNL Target]은(는) 각 방문자에게 해당 특성의 고유 값에 해당하는 등급 목록을 제공합니다.

[!UICONTROL 권장 사항 키]를 선택하는 방법에 대한 자세한 내용은 [권장 사항 키를 기반으로 권장 사항 만들기](/help/main/c-recommendations/c-algorithms/base-the-recommendation-on-a-recommendation-key.md)를 참조하십시오.

## [!UICONTROL 콘텐츠 백업] {#content}

[!UICONTROL 콘텐츠 백업] 규칙은 권장 항목 수가 [권장 사항 디자인](/help/main/c-recommendations/c-design-overview/design-overview.md)을 채우지 못할 경우 발생하는 결과를 결정합니다. [!DNL Recommendations] 기준에서 디자인에 필요한 권장 사항보다 적은 수의 권장 사항을 반환할 수 있습니다. 예를 들어 디자인에 4개 항목에 대한 슬롯이 있지만 기준에 따라 2개 항목만 권장되는 경우 나머지 슬롯을 비워두거나, 백업 권장 사항을 사용하여 추가 슬롯을 채우거나, 권장 사항을 표시하지 않도록 선택할 수 있습니다.

1. (선택 사항) **[!UICONTROL 부분 디자인 렌더링]** 토글을 &quot;켜짐&quot; 위치로 밉니다.
...
1. (선택 사항) **[!UICONTROL 백업 컨텐츠 표시]** 토글을 &quot;켜짐&quot; 위치로 밉니다.
   ...
   기준에 따라 두 개의 항목만 추천된다고 가정해 봅시다. [!UICONTROL 부분 디자인 렌더링] 옵션을 사용하면 처음 두 슬롯은 채워지지만 나머지 두 슬롯은 비어 있습니다. 그러나 [!UICONTROL 백업 권장 사항 표시] 옵션을 사용하면 처음 두 슬롯은 지정된 조건에 따라 채워지고 나머지 두 슬롯은 백업 권장 사항에 따라 채워집니다.

   다음 매트릭스는 [!UICONTROL 부분 디자인 렌더링] 및 [!UICONTROL 콘텐츠 백업] 옵션을 사용할 때 관찰할 결과를 보여 줍니다.
...
1. (조건부) 이전 단계에서 **[!UICONTROL 백업 컨텐츠 표시]**&#x200B;를 선택한 경우 **[!UICONTROL 백업 권장 사항에 포함 규칙 적용]**&#x200B;을 활성화할 수 있습니다.

   포함 규칙은 권장 사항에 포함되는 항목을 결정합니다. 사용 가능한 옵션은 수직 시장에 따라 다릅니다.

   자세한 내용은 아래의 [포함 규칙 지정](#inclusion)을 참조하십시오.

## [!UICONTROL 데이터 Source] {#data-source}

1. 원하는 **[!UICONTROL 동작 데이터 Source]**&#x200B;을 선택합니다. [!UICONTROL Adobe Target] 또는 [!UICONTROL Analytics].

   >[!NOTE]
   >
   >[!UICONTROL 동작 데이터 Source] 섹션은 구현에서 [Analytics for Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)(A4T)을 사용하는 경우에만 표시됩니다.
...
1. 표시할 권장 사항을 결정할 때 사용할 사용 가능한 내역 사용자 동작 데이터의 시간 범위를 결정하려면 **[!UICONTROL 전환 확인 기간]**&#x200B;을 설정하십시오. 이 옵션은 [!UICONTROL 유사한 특성을 가진 항목] 및 [!UICONTROL 사용자 지정 알고리즘]을 제외한 모든 알고리즘에 사용할 수 있습니다.
   ...
   | --- | --- | --- |
   | 6시간 | 알고리즘은 3~6시간마다 실행됩니다 | 선택한 [!UICONTROL 동작 데이터 Source]이(가) [!DNL Adobe Target]인 경우 [!UICONTROL 인기도 기반] 알고리즘 |
   ...
   | 2주 | 알고리즘은 24-48시간마다 실행됩니다 | <ul><li>[!UICONTROL 인기도 기반] 알고리즘</li><li>[!UICONTROL 항목 기반] 알고리즘</li><li>모든 [!UICONTROL 사용자 기반] 알고리즘</li><li>[!UICONTROL 장바구니 기반] 알고리즘</li></ul> |
...
>[!NOTE]
>
>[!UICONTROL 알고리즘 유형] 및 [!UICONTROL 알고리즘]&#x200B;(으)로 **[!UICONTROL 유사한 특성의]**/ **[!UICONTROL 미디어]**&#x200B;를 선택한 경우 콘텐츠 유사성 규칙을 설정할 수 있습니다.
...
**[!UICONTROL 유사한 특성을 가진 항목 기반]**/ **[!UICONTROL 미디어]**&#x200B;를 선택하면 권장 사항을 결정할 때 특정 항목 특성의 중요도를 늘리거나 줄이는 규칙을 만들 수 있습니다. 서적과 같은 항목의 경우 *장르*, *저자*, *시리즈*&#x200B;와 같은 속성의 중요도를 높여 비슷한 서적을 권장할 수 있습니다.
...
1. (조건부) **[!UICONTROL 최근에 구매한 항목을 추천할 수 있도록 허용하시겠습니까?]** &quot;켜짐&quot; 위치로 전환합니다.
