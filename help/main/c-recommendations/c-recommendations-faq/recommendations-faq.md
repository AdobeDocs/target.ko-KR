...
예. API를 통해 만든 [!UICONTROL 권장 사항] 개체([!UICONTROL 기준], [!UICONTROL 디자인], [!UICONTROL 컬렉션], [!UICONTROL 제외])는 UI에서 사용할 수 있으며 API 또는 [!DNL Target] UI를 통해 편집할 수 있습니다.

## [!DNL Target] API를 사용하여 [!DNL Target] UI에서 만든 시각적 오퍼를 관리할 수 있습니까?

아니오. [!DNL Target] UI에서 만들어진 시각적 오퍼가 있는 [!DNL Recommendations] 활동은 [!DNL Target] API를 사용하여 관리할 수 없습니다. 이러한 활동은 [!UICONTROL 활동] 목록에 표시되지만 GET/PUT을 사용하여 읽거나 업데이트할 수 없습니다.
...
현재, 고객이 속성 유형을 변경할 수 있는 기능이 없습니다. 변경하려면 문자열에서 숫자로 유형을 변경해야 하는 속성을 참조하는 [고객 문제를 여십시오](/help/main/cmp-resources-and-contact-information.md#reference_ACA3391A00EF467B87930A450050077C).
...
| 소스 | 세부 사항 |
| --- | --- |
| mbox 또는 API를 통해 업데이트된 아이템 속성 | <ul><li>권장 사항이 15분 안에 업데이트됩니다.</li><li>기존 권장 사항 및 항목 속성은 업데이트를 사용할 수 있을 때까지 표시됩니다.</li><li>카탈로그 색인 다음에는 카탈로그 검색이 없데이트됩니다(3~8시간).</li></ul> |
...
| 항목이 피드를 통해 카탈로그에 추가됨 | <ul><li>피드가 수집된 후에는 권장 사항이 업데이트됩니다(2~8시간). 후속 알고리즘 실행은 1~2일 알고리즘의 경우 12시간마다 예약되며, 7일 이상 알고리즘의 경우에는 24시간마다 예약됩니다. 추천가 총 2-32시간 내에 업데이트됩니다.</li><li>추가된 항목이 요청된 키가 아닌 경우에는 업데이트를 사용할 수 있을 때까지 기존 권장 사항이 표시됩니다.</li><li>추가된 항목이 요청된 키가 아닌 경우에는 업데이트를 사용할 수 있을 때까지 백업 권장 사항이 표시됩니다.</li><li>피드 수집(2~8시간) 및 카탈로그 색인(3~8시간) 후에는 카탈로그 검색이 업데이트됩니다. 카탈로그 검색이 총 5-16시간 내에 업데이트됩니다.</li></ul> |
...
* 이전에는 포함되었지만 지금은 제외되는 경우에는 &quot;항목 속성이 업데이트됨..&quot;에 따라 항목이 제외됩니다. 피드 소스에 따라 위에서 논의된 타임라인(mbox/API를 통해 15분 또는 피드를 통해 12~24시간).
...
## 내 [!UICONTROL 권장 사항] 활동, 오퍼, 프로모션 또는 기준 설정의 구성 변경이 내 사이트에 반영되려면 얼마나 걸립니까?
...
>[!NOTE]
>
>이 설정은 [!UICONTROL 시각적 경험 작성기]&#x200B;(VEC)에서 만들어진 활동에만 적용됩니다. 이 설정은 양식 기반 경험 작성기에서 만들어진 활동에는 적용되지 않습니다([!DNL Target]에는 위치 컨텍스트가 없습니다).
...
[!UICONTROL 호환되지 않는 기준 필터링] 설정이 사용되지 않는 경우 [!DNL Target]이(가) 알고리즘 선택기에서 알고리즘을 필터링하지 않고 모든 알고리즘이 표시됩니다.

[!UICONTROL 호환되지 않는 기준 필터링] 설정이 사용되는 경우 VEC 활동에서 [!DNL Target]이(가) 선택한 위치로부터 entityId와 범주 ID를 읽은 다음 `currentItem|currentCategory`을(를) 기반으로 알고리즘을 표시합니다(각각의 값이 해당 위치에 있는 경우). 따라서 기본적으로 알고리즘 선택기에는 선택한 위치를 위한 호환되는 알고리즘만 표시됩니다.
...
다음 목록에는 [!DNL Target]이(가) [!UICONTROL 호환] 확인란을 표시하지 않는 특수 사례가 포함되어 있습니다.
...
표준 속성 가중치는 모두는 아니지만 대부분의 기준 유형(콘텐츠 유사성뿐만 아니라)에 적용됩니다. 이러한 유형의 가중치는 특정 속성 값에 더 많은 가중치를 부여합니다. 다음 예에서 Nike 제품은 출력 권장 사항에서 문제가 생깁니다.
...
단일 호출에서 50,000개 제품 전송을 시도할 수 있습니다. 실패할 경우 여러 배치로 분할할 수 있습니다. Adobe는 고객이 호출을 5,000개 또는 10,000개 단위의 제품 배치로 분할하여 시스템 부하로 인한 시간 초과 가능성을 줄일 것을 권장합니다.
...
## [!DNL Recommendations] 활동에서 [!UICONTROL 데이터 다운로드] 작업이 실패한 이유는 무엇입니까? {#download-data-error}

[!DNL Recommendations] 활동의 [!UICONTROL 활동 개요] 페이지에서 **[!UICONTROL 데이터 다운로드]**&#x200B;를 클릭하면 [!DNL Target] 사용자 인터페이스에 `Error while fetching recommendation data file.` 오류가 표시될 수 있습니다

일반적으로 활동에 매우 큰 결과 세트가 있는 경우 발생합니다. 생성된 CSV가 단일 다운로드에서 사용자 인터페이스를 통해 반환할 수 있는 응답 크기를 초과합니다. 권장 사항 데이터 자체는 그대로 유지되므로 브라우저 내 다운로드 경로에서만 해당 크기의 파일을 전달할 수 없습니다.
...
다음은 [!UICONTROL 권장 사항] 활동에 대한 알려진 문제입니다.
