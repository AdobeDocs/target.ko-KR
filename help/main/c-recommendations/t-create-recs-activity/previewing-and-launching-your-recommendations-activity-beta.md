...
기준 상태가 나열된 [!UICONTROL 활동] 개요 다이어그램에서 알고리즘의 실행이 완료되었는지 여부를 확인할 수 있습니다. 다음 그림은 [!DNL Recommendations] 활동의 [!UICONTROL 개요] 페이지에서 활동 다이어그램의 상태를 보여 줍니다.
...
* **지속적인 알고리즘 실행**: 데이터는 매일 [!DNL Analytics]에서 [!DNL Target Recommendations]으로 이동합니다. 예를 들어 [!UICONTROL 관심도 보기] 추천의 경우 사용자가 제품을 볼 때 제품-보기 추적 호출이 실시간에 가깝게 [!DNL Analytics]에 전달됩니다. [!DNL Analytics] 데이터가 다음 날 일찍 [!DNL Target]&#x200B;(으)로 푸시되고 [!DNL Target]이(가) 12시간 이내에 알고리즘을 실행합니다.

>[!NOTE]
>
>[!UICONTROL 최근에 본 항목]은(는) 오프라인 알고리즘을 실행할 필요가 없으며 결과를 즉시 사용할 수 있습니다. mbox 데이터를 기반으로 하는 [!UICONTROL 가장 많이 본 항목] 및 [!UICONTROL 가장 많이 본 항목] 알고리즘은 일반적으로 필요한 계산이 단순하기 때문에 매우 빠르게 결과를 생성합니다. 이러한 기능은 디자인 변경을 미리 보거나 동작 데이터가 올바르게 수집되고 있는지 확인하려는 경우에 유용합니다.

## QA 링크를 사용하여 권장 사항 미리 보기

알고리즘에서 결과가 준비되면 [!DNL Adobe Target]의 [QA 링크](/help/main/c-activities/c-activity-qa/activity-qa.md) 기능을 사용하여 해당 결과를 미리 볼 수 있습니다. QA 링크는 [!UICONTROL 활동] 개요 페이지의 [!UICONTROL 활동 위치] 섹션에서 사용할 수 있습니다.
...
>
>* QA 모드에서는 사이트 탐색이 프로필의 [!UICONTROL 최근에 본 항목] 또는 [!UICONTROL 최근에 구매한 항목]에 영향을 주지 않습니다. 이 동작은 의도하지 않은 생산 행동 데이터의 오염을 피하기 위해 설계에 의해 발생합니다. [!UICONTROL 최근에 본 항목] 또는 [!UICONTROL 사용자 기반 권장 사항] 기준의 결과를 미리 보려면 먼저 QA 모드 외부에서 사이트를 찾은 다음 동일한 세션을 사용하여 QA 모드 링크를 엽니다.

## CSV 다운로드를 사용하여 권장 사항 미리보기

경우에 따라 권장되는 특정 항목을 감사할 수 있습니다. 이 기능은 특히 [!UICONTROL 이 항목을 본 사람, 특정 항목을 본 사람]과 같은 알고리즘을 사용할 때 유용합니다. 여기서 다른 항목 세트는 현재 사용자가 보고 있는 항목에 따라 권장되며 카탈로그에 수천 또는 수백만 개의 다른 항목이 있을 수 있습니다.

활동의 하나 이상의 알고리즘에 대해 [!UICONTROL 결과 준비] 상태가 표시될 때까지 결과를 다운로드할 수 없습니다.

미리 볼 결과를 다운로드하려면 활동 개요 페이지의 오른쪽 상단에 있는 메뉴 아이콘을 클릭한 다음 **[!UICONTROL 데이터 다운로드]**&#x200B;를 클릭합니다.
...
키 값을 기반으로 하는 다른 알고리즘 유형(예: [!UICONTROL 이 항목을 본 사람, 특정 항목을 본 사람])의 경우 키 값(즉, &quot;이&quot; 항목)은 가장 왼쪽 열에 나열되고 권장 항목(즉, &quot;저&quot; 항목)은 Recommendation_X 열에 왼쪽에서 오른쪽으로 나열됩니다.

결과 집합이 매우 큰 활동의 경우 **[!UICONTROL 데이터 다운로드]** 옵션이 실패하고 오류 `Error while fetching recommendation data file.`이(가) 발생할 수 있습니다. 이 문제는 생성된 CSV가 사용자 인터페이스를 통해 한 번의 다운로드로 반환할 수 있는 응답 크기를 초과할 때 발생합니다.

>[!NOTE]
>
>[!UICONTROL 사용자 기반 권장 사항] 알고리즘을 포함하는 활동에는 결과를 다운로드할 수 없습니다. [!UICONTROL 최근에 본 항목] 권장 사항 논리를 사용하는 기준에는 결과 다운로드를 사용할 수 없습니다.

## 권장 사항 활동 활성화

[!UICONTROL 활동 개요] 탭에서 상태 드롭다운 화살표를 클릭한 다음, **[!UICONTROL 활성화]**&#x200B;를 선택합니다.

[!UICONTROL 권장 사항] 활동이 현재 [!UICONTROL 비활성] 상태인 경우 드롭다운 목록에 [!UICONTROL 비활성] 레이블이 지정됩니다.

몇 초에서 몇 분 후에 상태가 [!UICONTROL Live]&#x200B;(으)로 전환됩니다.

동일한 드롭다운 목록을 사용하여 활동을 비활성화하거나 보관할 수도 있습니다.

## 권장 사항 설정을 변경할 때 중단 방지

라이브 활동에서 [!DNL Recommendations]개의 컬렉션, 기준, 프로모션 또는 디자인 설정을 변경하면 알고리즘 결과가 잘못되고 알고리즘 상태가 [!UICONTROL 결과가 준비되지 않음]&#x200B;(으)로 변경될 수 있습니다.
