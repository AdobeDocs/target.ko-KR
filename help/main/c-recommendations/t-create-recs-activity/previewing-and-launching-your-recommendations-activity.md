...
[권장 사항 오퍼](/help/main/c-recommendations/recommendations-as-an-offer.md)가 포함된 [!UICONTROL Recommendations], [!UICONTROL A/B Test] 또는 [!UICONTROL Experience Targeting]&#x200B;(XT) 활동을 만든 후에는 활동을 시작하기 전에 결과를 사용할 수 있도록 권장 사항을 미리 보는 것이 좋습니다. [!DNL Target Recommendations]은(는) 권장 사항을 미리 보는 여러 가지 방법을 제공합니다.
...
결과 집합이 매우 큰 활동의 경우 **[!UICONTROL Download data]** 옵션이 실패하고 오류 `Error while fetching recommendation data file.`이(가) 발생할 수 있습니다. 이 문제는 생성된 CSV가 단일 다운로드에서 사용자 인터페이스를 통해 반환할 수 있는 응답 크기를 초과할 때 발생합니다.
