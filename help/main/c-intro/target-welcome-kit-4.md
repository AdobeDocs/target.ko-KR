...
많은 사람들이 [!DNL Target] 사용자, Adobe는 고객의 [!DNL Target] 솔루션. 이 장에 포함시킨 여러 가지 팁에 나와 있는 내용을 요약했습니다. 이러한 아이디어를 당장 모두 사용할 준비가 되지 않았더라도 이 목록을 계속 유지하십시오. 솔루션을 통해 더 많은 경험을 얻을 수 있고 프로그램이 더 발달할수록 이러한 팁을 통해 더 많은 정보를 얻을 수 있습니다 [!DNL Target].
...
자세한 내용은 [데이터를 Target에 가져오는 방법](https://developer.adobe.com/target/before-implement/methods-to-get-data-into-target/methods-to-get-data-into-target/)자세한 내용은 {target=_blank}.

## 팁 2: [!DNL Target] 대상을 다른 Adobe Experience Cloud 대상과 혼합하여 보다 심층적으로 개인화할 수 있습니다.

다른 [!DNL Adobe Experience Cloud] 솔루션을 이용하는 대상을 혼합하면 고객에 대한 이해도가 훨씬 더 넓어질 뿐만 아니라 보다 심층적으로 개인화할 수 있습니다. 예를 들어 [!DNL Target] 은 실시간 대상 데이터를 제공하지만, [!DNL Adobe Analytics] 는 과거 대상 데이터를 제공합니다. 두 가지 방법을 결합하면 고객의 행동이 일관되는 시점을 식별할 수 있고 언제 새로운 행동을 취할 기회가 있는지 알 수 있습니다. 활동을 만들 때 &quot;모든 방문자&quot; 옆에 있는 드롭다운 메뉴를 클릭하면 됩니다. 다음으로 최대 20개의 대상 상자를 선택하고 &quot;여러 대상 결합&quot;을 클릭한 다음 &quot;저장&quot;을 클릭합니다.

단계별 지침은 [여러 대상 결합](/help/main/c-target/combining-multiple-audiences.md) 을 참조하십시오.

**알아 둘 사항**: [!DNL Adobe Audience Manager] 대상은 [!DNL Target] 에서 자동으로 사용할 수 있습니다. 그러나 [!DNL Adobe Analytics] 대상을 공유하려면 약간의 수동 설정이 필요합니다. 에서 대상 작성 프로세스 중에 &quot;Experience Cloud 대상으로 지정&quot; 상자를 선택하면 됩니다 [!DNL Analytics]. Then from [!DNL Target], &quot;Experience Cloud 대상자 가져오기&quot;를 클릭합니다.

## 팁 3: 서드파티 도구에서 사용할 데이터를 [!DNL Target]에서 내보냅니다.

응답 토큰을 사용하면 관리자는 [!DNL Target] 에서 서드파티 도구로 데이터를 쉽게 가져올 수 있습니다. 이 기능은 설문 조사 도구에서 수집된 데이터에 사용자 데이터를 추가하려는 경우에 유용합니다. 예를 들어, 설문 조사에 경험 a &quot;9&quot;에 대해 점수를 매기고 다른 사람이 경험 a &quot;4&quot;에 대해 점수를 매긴 경우 데이터를 사용하여 경험 A를 본 사용자와 경험 B를 본 사용자를 확인할 수 있습니다. 응답 토큰을 사용하여 내보낼 수도 있습니다 [!DNL Target] 데이터를 내부 데이터 웨어하우스에 추가합니다. &quot;관리&quot;를 클릭한 다음 원하는 응답 토큰 옆에 있는 스위치를 켜짐 위치로 전환하면 됩니다. 그런 다음 활동을 생성합니다. 그런 다음 데이터를 서드파티 공급업체로 전송할 수 있습니다. [!DNL Target] 에서 디버깅 도구를 사용하여 데이터를 내보내고 있는지 확인할 수 있습니다.
...
**먼저 수행할 작업**: at.js 버전 1.1 이상을 사용하고 있는지 확인합니다. 이전 버전을 사용하는 경우에는 응답 토큰이 표시되지만 at.js는 응답 토큰을 사용할 수 없습니다.
...
**유용한 힌트**: 사용자를 추가할 때 팀 구성원의 이름이 이전에 조직에 추가되지 않은 경우(예: 서드파티 에이전시 직원의 경우) 이메일 주소와 암호를 입력하면 팀 작업 영역에 참여하기 위한 이메일 초대장이 트리거됩니다.
...
다시 타겟팅하지 않는 세그먼트를 타겟팅하는 경우(예: 예기치 않은 날씨 이벤트의 영향을 받는 고객) 일회용 대상을 만들면 대상 라이브러리에 불필요한 것들을 추가하지 않고 작업을 완료할 수 있습니다. 이렇게 하면 반복적으로 사용하는 대상을 더 쉽게 찾을 수 있습니다.
...
**유용한 팁**: 를 사용하십시오 [!DNL Adobe Target] [샘플 크기 계산기](/help/main/c-activities/t-test-ab/sample-size-determination.md#section_6B8725BD704C4AFE939EF2A6B6E834E6) 을 눌러 성공적인 테스트에 필요한 샘플 크기를 결정합니다.

## 팁 12: 더 간단한 테스트를 디자인하여 테스트를 만들고 구현할 수 있도록 하십시오.

테스트 설계 방법의 모든 측면을 고려한 후에는 계획이 매우 복잡해질 수 있습니다. 테스트 대상 비즈니스 위치 및 테스트 결과를 디자인, 코드, 실행 및 분석할 수 있는 그룹 기능을 기반으로 테스트가 너무 야심찬 것 같은지 확인합니다. 그렇다면 범위와 복잡성을 줄일 준비를 하십시오. 테스트를 전혀 수행하지 않는 것보다 작게 시작하는 것이 좋습니다. 테스트를 실행하지 않는 경우에는 영향력 있는 상승도를 제공할 수 없습니다. 팀의 열망과 실제의 현실과 자원 및 능력의 균형을 맞추는 것이 중요합니다.
...
![복잡한 테스트 세분화 그림](/help/main/c-intro/assets/break-complex-tasks.png)
...
가설이 사실로 입증되지 않았다면 테스트를 개선할 방법을 생각해 보십시오. 비록 실험된 경험들 중 어떤 것도 더 나은 성과를 내지 못했지만, 여러분의 실험은 시간 낭비가 아니라는 것을 기억하세요. 성공적인 테스트가 항상 매출 또는 전환율 증가를 의미하지는 않습니다. 만약 테스트가 실제로 당신의 가설을 뒷받침한다면, 당신은 일반적인 이론을 발전시키는 길로 가고 있습니다. 그러나 분명한 승산이 있다고 해서 거기서 멈추지 마세요. 너무 자주, 마케터들은 무엇이 성공으로 이어졌는지 제대로 이해하지 못한 채 한 번 테스트한 다음 그 결과에 의존하는 실수를 합니다. 대신 이러한 결과를 반복하여 선두 주자가 앞서고 있는 이유를 파악하십시오. 그래야 향후 캠페인에 사용할 수 있는 보다 심층적인 통찰력을 얻을 수 있습니다.

## 팁 16: 아이디어에 대한 테스트 및 개인화 활동을 비교하여 타기팅을 개선하십시오.

다른 위치에서 다른 테스트 내에서 서로 다른 대상의 전환 성과를 비교하면 회사의 최적화 전략을 세우고 집중하는데 도움이 될 수 있습니다. 테스트 비교를 통해 어떤 대상이 테스트에 가장 가치가 있고, 어떤 타기팅 경험을 받아야 하며, 어떤 유형의 경험이 반응을 이끌어낼 가능성이 가장 높은지를 확인하십시오.
...
데이터에 대해 어떻게 행동할 것인지 확실하지 않으면 테스트가 무의미합니다. 여기에는 주요 성공 지표, 승자를 추진하기 위해 필요한 사항, 테스트 결과에 대한 후속 조치, 대상 정보 활용 방법 등이 포함됩니다. 신속하고 성공적인 테스트를 위해서는 테스트에 참여하는 모든 그룹(개발자, 크리에이티브, 테스트 전문가 등)이 테스트 시작 전에 해당 역할을 알고 있어야 합니다.

## 팁 18: 테스트를 시작하기 전에 비즈니스에서 승자를 추진하는 것을 지원하는지 확인하십시오.

성공적인 최적화 조직은 테스트 개념을 믿고 테스트가 성공할 경험에 대한 전문적인 의견이 항상 진실되지는 않음을 이해합니다. 따라서 탄탄한 데이터 기반에 따라 승자를 결정하며, 승자가 기대치에 맞지 않거나 직관적이지 않은 것처럼 보이나 결과가 나온 후 우승 경험을 라이브로 푸시할 의지가 있습니다.

예를 들어 한 Adobe 헬스케어 서비스 고객은 최근 팀이 확실히 성공할 것으로 간주했던 영웅 배너가 실제로 전환에 부정적인 영향을 미쳤음을 보여줌으로써 테스트의 가치를 입증했습니다. 조직에서 아직 테스트를 완전히 채택하지 않은 경우 테스트 결과의 변경 사항을 점진적으로 적용할 수 있도록 먼저 단순하고 작은 범위 테스트를 수행하는 것이 좋습니다.

## 팁 19: 사이트가 변경될 때 문제가 발생하지 않도록 테스트를 시작했음을 모두에게 알리십시오.

QA 매개 변수를 사용하도록 활동을 설정할 때 얻을 수 있는 이점 중 하나는 해당 링크를 팀의 모든 사용자와 공유할 수 있다는 것입니다. 더 많은 사람들이 활동을 인식하고 테스트 변형을 히트할 때 사이트가 제대로 작동하지 않는다고 가정하지 않도록 합니다.

테스트를 마친 후에는 캠페인 시작, 테스트 결과 및 특히 학습한 교훈을 전달하면 테스트 결과에 대한 인식과 관심을 높이는 데 도움이 됩니다. 조직 내 모든 사람과 결과를 공유하면 가설을 재테스트하지 않고, 모든 사람에게 어떤 것이 작동하는지 교육하며, 발견사항을 바탕으로 하는 작업에 대한 자신의 아이디어를 근본적으로 해결하는 데에도 도움이 됩니다. 검색 결과와 주요 지식을 공유하기 위해 매번 사용하는 템플릿을 준비하는 것이 좋습니다.
...
하루 종일 다양한 세그먼트로 모바일 캠페인을 &quot;하루 분할&quot;하여 대상자에 도달하는 방법과 시기 및 일일 광고 비용을 보다 효율적으로 관리하는 방법을 이해합니다.

많은 마케터들은 특정 디바이스의 사용이 가장 많은 시간에 해당 목소리를 포착할 수 있는 충분한 예산을 할당하지 못해 많은 매출과 잠재 고객을 테이블 위에 남겨두는 실수를 범합니다.

예를 들어 태블릿 사용량은 일반적으로 저녁 시간에 급증하며, 많은 사용자가 TV를 보면서 탐색합니다. 이와는 대조적으로, 스마트폰 사용자는 일반적으로 이동 중에 콘텐츠에 액세스합니다. 최대 전환 시간은 업종마다 다르므로 고유 고객이 가장 행동에 옮길 가능성이 높은 경우를 이해하는 것이 중요합니다.
