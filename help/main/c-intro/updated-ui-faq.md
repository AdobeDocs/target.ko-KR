...
## 업데이트된 UI를 현재 모든 [!DNL Target] 고객, [!UICONTROL Standard] 및 [!UICONTROL Premium]에서 사용할 수 있습니까?
...
+++세부 사항
**[!UICONTROL 다시 정렬*]*: 기존 VEC에서 다시 정렬 옵션은 오버레이를 사용하여 사용자가 동일 그룹 내의 요소를 다시 배치할 수 있도록 했습니다. 움직임은 형제 요소 간의 순서를 바꾸는 것으로 한정되었다.

업데이트된 VEC에서 이 기능은 앞으로 이동 및 뒤로 이동 작업을 통해 간소화됩니다. 이러한 컨트롤은 겹침 순서에서 요소를 앞이나 뒤로 이동하여 레이아웃에서 요소의 위치를 가로와 세로 모두 조정합니다.

**크기 조정**: [!UICONTROL 크기 조정] 기능은 [!UICONTROL 크기] 섹션 아래의 [!UICONTROL 속성] 패널에 있습니다. 사용자는 요소의 너비와 높이를 직접 조정할 수 있습니다. 고급 설정에는 다음이 포함됩니다.
...
**이동**: [!UICONTROL 이동] 옵션은 [!UICONTROL 위치] 섹션 아래의 [!UICONTROL 속성] 패널에 있습니다. 이 옵션을 사용하면 다음과 같은 작업을 수행할 수 있습니다.

* 요소의 위치(예: 절대, 상대, 고정)를 설정합니다
* 계층화를 위한 z-색인 정의
* 위치 지정 유형 선택

업데이트된 [!UICONTROL 속성] 레일은 사용자 지정 인라인 스타일을 지원하므로 사전 설정 옵션이 레이아웃 요구 사항에 맞지 않을 때 유연성을 제공합니다.

**[!UICONTROL 숨기기]**: [!UICONTROL 숨기기] 기능은 [!UICONTROL 속성] 패널에 있습니다. 요소를 선택한 후 요소를 삭제하지 않고 보기에서 제거하려면 [!UICONTROL 요소 숨기기]를 클릭하십시오. 이 기능은 디자인 또는 미리 보기 중 가시성을 관리하는 데 유용합니다.

**[!UICONTROL 제거]**: [!UICONTROL 제거] 기능은 [!UICONTROL 속성] 패널을 통해 액세스할 수 있습니다. 요소를 선택한 후 요소 제거 를 클릭하여 페이지에서 요소를 삭제합니다. 이 작업은 레이아웃에서 요소를 영구적으로 제거합니다.

+++

## [!UICONTROL 디자인] 패널을 확대할 수 있도록 [!UICONTROL 구성 요소], [!UICONTROL 수정 사항] 및 [!UICONTROL 속성] 레일을 축소할 수 있습니까? {#collapse}

+++세부 사항

예. 이러한 레일을 축소하여 [!UICONTROL 디자인] 캔버스를 확장하면 보다 쉽게 편집할 수 있습니다. 방법은 다음과 같습니다.
...
[!UICONTROL 속성] 레일을 축소하고 [!UICONTROL 디자인] 캔버스를 확대하려면 레일의 오른쪽에 있는 [!UICONTROL 속성 표시/숨기기] 아이콘(![속성 아이콘](/help/main/assets/icons/Propertie.svg))을 클릭하여 [!UICONTROL 속성] 레일을 축소하거나 표시합니다.

+++

## [!UICONTROL 초안으로 저장] 및 [!UICONTROL 동기화 중] 상태를 계속 사용할 수 있습니까?

+++세부 사항
사용자 인터페이스에 대한 최신 업데이트를 통해 [!UICONTROL 초안으로 저장] 및 [!UICONTROL 동기화 중] 상태를 더 이상 사용할 수 없습니다. 자세한 내용은 *[!UICONTROL 활동 개요]*&#x200B;의 [활동 목록에 필터 적용](/help/main/c-activities/activities.md#filters) 아래의 상태를 참조하십시오.
...
+++세부 사항
[!UICONTROL 오퍼 라이브러리] UI에는 모든 오퍼에 대해 일관된 특성 정의가 필요합니다. 활동 전용(애드혹) 오퍼를 생성할 때 사용자는 오퍼 이름도 지정해야 합니다. 이 정보는 [!UICONTROL 양식 기반 경험 작성기]에 표시되므로 코드나 콘텐츠를 검토하지 않고도 오퍼를 보다 쉽게 식별할 수 있습니다.
...
+++세부 사항
[!UICONTROL 경험 조각] 미리 보기 링크는 선택한 조각에 해당하는 정보 아이콘(![정보 아이콘](/help/main/assets/icons/InfoOutline.svg) )을 클릭할 때 표시되는 [!UICONTROL 빠른 정보] 팝오버에서 사용할 수 있습니다.

+++

## 업데이트된 UI로 기존 활동을 편집할 때 [!UICONTROL 향상된 경험 작성기]를 비활성화해야 합니다. [!DNL Adobe]님이 다른 고객과 유사한 행동을 관찰했습니까?

+++세부 사항
예. [!DNL Adobe Experience Cloud] [!DNL Visual Editing Helper extension]을(를) 사용하는 경우 [!UICONTROL 고급 경험 작성기]&#x200B;(EEC) 을 비활성화해야 할 수 있습니다.
...
* **[!UICONTROL 권장 사항]**: [권장 사항 피드 처리 서버에서 사용하는 IP 주소](/help/main/c-recommendations/c-recommendations-faq/ip-addresses-marketing-cloud.md)를 참조하세요.
...
+++세부 사항
이제 환경은 고객이 사용한 마지막 환경으로 기본 설정됩니다. 환경을 전환하려면 [!UICONTROL 카탈로그 검색] UI의 오른쪽 위 모서리에 있는 [!UICONTROL 환경] 선택기를 사용하십시오.
