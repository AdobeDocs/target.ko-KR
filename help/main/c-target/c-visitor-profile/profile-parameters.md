...
1. **[!UICONTROL Create Script]** 아이콘을 클릭합니다.
   ...
   | 매개변수 유형 | 설명 |
   |--- |--- |
   | mbox | mbox를 만들 때 페이지 코드를 통해 직접 전달됩니다. [Target 개발자 안내서](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/global-mbox/pass-parameters-to-global-mbox.html?lang=ko){target=_blank}에서 *글로벌 Mbox에 매개 변수 전달*&#x200B;을 참조하십시오.<P>**참고**: [!DNL Target]에는 mbox 호출당 50개의 고유 프로필 속성 제한이 있습니다. [!DNL Target]에 50개가 넘는 프로필 특성을 전달해야 하는 경우 [!UICONTROL Profile Update API] 메서드를 사용하여 전달하십시오. 자세한 내용은 [Target 개발자 안내서](https://experienceleague.adobe.com/docs/target-dev/developer/api/profile-apis/profile-api-overview.html?lang=ko){target=_blank}에서 *프로필 업데이트*&#x200B;를 참조하십시오. |
...
프로필 스크립트는 [!UICONTROL Audiences] 인터페이스의 [!DNL Target] 탭에서 사용할 수 있습니다.
...
* **mboxTrace 디버깅 도구를 사용하여 프로필 스크립트를 디버그합니다.**
  ...
  이 메서드를 사용하려면 **[!UICONTROL Target]** 섹션에서 **[!UICONTROL Administration]** > **[!UICONTROL Implementation]** > **[!UICONTROL Generate Authorization Token]** > [!UICONTROL Debugger tools]을(를) 클릭하여 생성할 수 있는 인증 토큰이 필요합니다.
...
| `//` | 부울에 포함된 타겟의 모든 요소(Array source, Array target)가 소스에 포함되어 있는지 확인합니다.<br>`//`는 대상(정규 표현식에 해당함)에서 하위 문자열을 추출하여 `Array/*String*/ decode(String encoding, String regexp, String target)`로 디코딩합니다.<br>이 기능은 상수 문자열 값, 그룹화(`condition1 \|\| condition2) && condition3`) 및 정규식(`/[^a-z]$/.test(landing.referring.url)`) 사용도 지원합니다. |
