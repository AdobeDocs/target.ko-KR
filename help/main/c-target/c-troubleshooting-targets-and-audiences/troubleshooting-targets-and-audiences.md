...
활동을 만드는 동안 대상 URL 타깃팅을 적용하려면 **[!UICONTROL 경험]** 페이지(3단계 안내 워크플로의 1단계)에서 **[!UICONTROL 구성]** 아이콘(![구성 아이콘](/help/main/assets/icons/Setting.svg))을 클릭하고 **[!UICONTROL 페이지 전달]**&#x200B;을 클릭한 다음 원하는 URL을 지정하십시오.
...
[!UICONTROL 템플릿 규칙]을 통해 추가된 대상자 URL 타겟팅 및 URL 타겟팅은 URL 타겟팅으로 평가됩니다(아래 URL 타겟팅 참조).

### URL 타겟팅 {#url-targeting}

대상을 만드는 동안 URL 타깃팅을 적용하려면 **[!UICONTROL 사이트 페이지]**&#x200B;를 클릭하고 [!UICONTROL 대상 만들기] 창으로 끌어 놓은 다음 **[!UICONTROL 사이트 페이지]**&#x200B;를 클릭하고 첫 번째 드롭다운 목록([!UICONTROL 현재 페이지], [!UICONTROL 이전 페이지] 또는 [!UICONTROL 랜딩 페이지])에서 옵션을 선택한 다음 두 번째 드롭다운 목록에서 [!UICONTROL URL]을 선택하고 평가자를 지정한 다음 원하는 URL을 지정하십시오.
...
[!DNL Target]의 mbox 호출당 고유한 프로필 속성 제한은 50개입니다. [!DNL Target]에 50개가 넘는 프로필 특성을 전달해야 하는 경우 [!UICONTROL 프로필 업데이트] API 메서드를 사용하여 전달할 수 있습니다. 자세한 내용은 Adobe Target API 설명서의 [프로필 업데이트](https://developers.adobetarget.com/api/#authentication-tokens)를 참조하십시오.
