...
description: 경험 타깃팅 및 Adobe에 사용된 대상에 대한 FAQ 보기 [!DNL Target] 활동.
...
## 은 어떻게 합니까? [!DNL Target] 타깃팅에서 URL을 평가하시겠습니까? {#url}
...
![페이지 게재 URL](/help/main/c-target/c-troubleshooting-targets-and-audiences/assets/activity-url.png)

대상자 URL 타겟팅이 정확한 URL 일치를 찾습니다. URL이 일치하는 경우 Target은 추가 논리를 고려하지 않습니다. 위의 URL에서 활동이 실행되도록 설정된 경우 `www.example.com`, 대상 URL 타깃팅이 쿼리에 구애받지 않으므로 URL은 다음 URL과 일치합니다.
...
의 첫 번째 인스턴스 `Category` 매개 변수가 평가되고 두 번째 `Category` 매개 변수는 무시됩니다.
...
## API를 통해 생성된 대상에 대한 변경 사항이 [!DNL Target] UI? {#section_6BEB237CAC004A06A290F9644E5BF0FB}
