...
URL 타깃팅을 적용하려면 대상을 만드는 동안 규칙 추가 를 클릭하고 사이트 페이지 를 클릭한 다음, 첫 번째 드롭다운 목록(현재 페이지, 이전 페이지 또는 랜딩 페이지)에서 옵션을 선택하고, 두 번째 드롭다운 목록에서 URL을 선택하고, 평가기를 지정한 다음 원하는 URL을 지정합니다.
