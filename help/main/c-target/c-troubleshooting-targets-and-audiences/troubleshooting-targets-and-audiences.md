...
description: Adobe에 사용된 경험 타깃팅 및 대상에 대한 FAQ 보기 [!DNL Target] 활동.
...
## 어떻게 [!DNL Target] 타깃팅에서 URL을 평가하시겠습니까? {#url}

Target은 활동을 만들 때 대상 URL 타깃팅을 사용하는지 또는 대상을 만들 때 URL 타깃팅을 사용하는지에 따라 URL을 다르게 평가합니다.
...
활동을 만드는 동안 대상 URL 타깃팅을 적용하려면 경험 페이지(3단계 안내 워크플로우 중 1단계)에서 톱니바퀴 아이콘을 클릭하고 페이지 전달을 클릭한 다음, 원하는 URL을 지정하십시오.

![페이지 전달 URL](/help/main/c-target/c-troubleshooting-targets-and-audiences/assets/activity-url.png)

대상 URL 타깃팅은 정확한 URL 일치를 찾습니다. URL이 일치하는 경우 Target에서 추가 논리를 고려하지 않습니다. 활동이 실행되도록 설정된 경우 위의 URL에서 `www.example.com`를 입력하면 대상 URL 타깃팅이 질의와 다르므로 URL은 다음 URL에 대해 일치합니다.

* `www.example.com?query=something`
* `www.example.com?query=anything`
* `www.example.com?query=nothing&qa=true&stuff=random&product=shoes&height=superTall`

URL에 대한 대상 타깃팅 외에도 쿼리에 사용할 수 있는 특정 값을 지정할 수도 있습니다.

를 통해 추가된 대상 URL 타깃팅 및 URL 타깃팅 [!UICONTROL 템플릿 규칙] url 타깃팅으로 평가합니다(아래 URL 타깃팅 참조).

### URL 타깃팅

URL 타깃팅을 적용하려면 대상을 만드는 동안 을(를) 클릭합니다 [!UICONTROL 규칙 추가]를 클릭합니다. [!UICONTROL 사이트 페이지]첫 번째 드롭다운 목록에서 옵션을 선택합니다([!UICONTROL 현재 페이지], [!UICONTROL 이전 페이지], 또는 [!UICONTROL 랜딩 페이지]). [!UICONTROL URL] 두 번째 드롭다운 목록에서 평가기를 지정한 다음 원하는 URL을 지정합니다.
...
## 복잡한 URL 문자열을 만들 때 [!DNL Target] 전체 URL을 평가하시겠습니까?
...
의 첫 번째 인스턴스 `Category` 매개 변수가 평가되고 두 번째 매개 변수가 `Category` 매개 변수는 무시됩니다.

우수 사례는 다음과 같이, 단일 카테고리와 연결된 여러 값을 갖는 것입니다.
...
## 모든 프로필 매개 변수가 [!DNL Target] 사용자 인터페이스? {#section_3CD947D15C984EE9AD19550220E0E8BD}
...
## API를 통해 만든 대상에 대한 변경 사항이 [!DNL Target] UI? {#section_6BEB237CAC004A06A290F9644E5BF0FB}
