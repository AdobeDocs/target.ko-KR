...
* **모든 활동 뒤에 Insights 페이지 위치를 변경합니다.** 탐색에서 **모든 활동** 페이지 다음에 **인사이트 페이지** 위치가 나타나도록 업데이트되었습니다. 이제 모든 활동 페이지가 대상 애플리케이션의 기본 페이지입니다. (TGT-54672)
...
* **왼쪽 탐색을 접을 수 있게 만듭니다.** 이제 왼쪽 탐색을 접을 수 있으며 모든 탭에서 확장할 수 있습니다. 축소하면 아이콘에 도구 설명이 표시됩니다. 확장하면 아이콘과 텍스트가 모두 표시됩니다. 이 향상된 기능을 통해 사용자는 탐색 옵션에 대한 빠른 액세스를 유지하면서 필요할 때 더 많은 화면 공간을 사용할 수 있습니다. (TGT-54608)
...
* **활동에 이미지를 추가할 수 없습니다. 알 수 없는 오류입니다.** 이 수정 사항은 사용자가 활동에서 이미지를 추가하거나 바꾸려고 할 때 &quot;알 수 없는 오류&quot;가 발생하는 문제를 해결합니다. 이 문제는 Scene7 URL 동기화 및 인증 실패와 관련되어 있습니다. 이 수정 사항을 통해 시각적 경험 작성기에서 이미지를 올바르게 추가하고 바꿀 수 있습니다. (TGT-54137)
...
* 제거 후 **속성 토큰이 자동으로 다시 추가되지 않습니다.** 이 수정 사항은 사용자가 활동을 편집할 때 선택하지 않은 속성 토큰이 자동으로 다시 추가되는 문제를 해결합니다. 이전에는 사용자가 자동 감지 속성을 제거하고 여러 단계를 탐색하면 시스템에서 경고나 프롬프트 없이 제거된 속성을 다시 도입하여 예기치 않은 속성에서 활동이 활성화되었습니다. 업데이트는 속성 토큰이 제거되면 이후의 모든 편집 및 탐색 중에 제거된 상태로 유지되도록 합니다. (TGT-54040)
...
* **허용 목록에추가된 URL 관리 기능.** 허용된 원격 오퍼 URL 패턴을 관리하기 위해 새로운 &quot;허용 목록에추가된 URL&quot; 관리 기능이 [!UICONTROL Administration] 섹션에 추가되었습니다. 이 기능에는 표에 허용 목록에추가된 URL을 표시하고, URL을 검색 및 필터링하고, 모달 대화 상자를 통해 새 URL을 추가하고, 확인을 통해 URL을 삭제하는 기능이 포함됩니다. 이 기능을 통해 고객이 원격 오퍼에 사용할 수 있는 URL을 제어할 수 있으므로 보안과 규정 준수가 향상됩니다. (TGT-54486)
...
* **GraphQL에서 createdAt 및 modifiedAt에 의한 필터링을 지원합니다.** 및 `createdAt` 날짜 필드별로 활동을 필터링하는 `modifiedAt` 지원이 GraphQL API에 추가되었습니다. 이러한 향상된 기능을 통해 Target UI에서 정확한 필터링을 수행하고, 클라이언트측 데이터 처리를 줄이며, 필터링된 활동 데이터를 검색하기 위해 여러 API 호출이 필요하지 않습니다. (TGT-54499)
...
* **[!UICONTROL Reports] 화살표가 확장 가능한 열을 명확하게 나타내지 않는 문제를 해결했습니다.** 보고 테이블에서 업데이트된 UI에서 추가 열을 확장할 수 있음을 명확하게 표시하지 않는 문제를 해결했습니다. 열 헤더 근처의 [!UICONTROL Reports] 화살표에 사라진 도구 설명이 추가되어 고객이 더 많은 열을 사용할 수 있음을 이해할 수 있습니다. (TGT-53479)
...
### [!DNL Target Standard/Premium] 25.10.1 (2025년 10월 22일 목요일)
...
### [!DNL Target Standard/Premium] 25.8.3 (2025년 8월 21일 금요일)
...
### [!DNL Target Standard/Premium] 25.8.2 (2025년 8월 14일 금요일)
...
### [!DNL Target Standard/Premium] 25.8.1 (2025년 8월 7일 금요일)
...
### [!DNL Target Standard/Premium] 25.7.4 (2025년 8월 1일 토요일)
...
### [!DNL Target Standard/Premium] 25.4.5 (2025년 4월 25일 토요일)
...
### [!DNL Target Standard/Premium] 25.4.4 (2025년 4월 17일 금요일)
...
### [!DNL Target Standard/Premium] 25.4.3 (2025년 4월 11일 토요일)
...
### [!DNL Target Standard/Premium] 25.4.1 (2025년 4월 2일 목요일)
...
자세한 내용은 [Adobe Target 개발자 안내서](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/allowlist-edges.html){target=_blank}의 *Target 에지 노드*&#x200B;를 참조하십시오.
...
* 고객이 대상을 대상 이름의 첫 번째 문자(예: !London)로 느낌표( ! )를 사용하여 저장하려고 하면 경고 메시지가 표시됩니다. (TGT-43643)
...
<table id="table_6D99C48B72D24728BF623608053931D3"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Target 활동에 AEM에서 만든 경험 조각을 사용하면 AEM의 편의성과 기능을 Target의 강력한 AI(Automated Intelligence) 및 머신 러닝(ML) 기능을 결합하여 경험을 다양한 규모로 테스트 및 개인화할 수 있습니다.&amp;nbsp;&amp;nbsp; </p> <p>AEM에서는 모든 콘텐츠 및 에셋을 중앙 위치에 가져와서 개인화 전략을 실행합니다. AEM을 사용하면 코드를 작성하지 않고도 한 위치에서 데스크탑, 태블릿 및 휴대 디바이스의 콘텐츠를 쉽게 만들 수 있습니다. 모든 장치를 위해 페이지를 만들 필요가 없이, 콘텐츠를 사용하여 각 경험이 자동으로 조정됩니다. </p> <p> Target을 사용하면 행동 변수, 컨텍스트 변수 및 오프라인 변수를 통합하는 규칙 기반 및 AI 중심 머신 러닝 접근 방식들의 결합을 기반으로 다양한 규모의 개인화된 경험을 제공할 수 있습니다.Target을 사용하면 A/B와 다변량 활동을 쉽게 설정 및 실행하여 최상의 오퍼, 콘텐츠 및 경험을 결정할 수 있습니다. </p> <p>경험 조각은 콘텐츠/경험 작성자 및 관리자를 Target을 사용하여 비즈니스 결과를 이끄는 최적화 및 개인화 전문가에게 연결하기 위한 매우 큰 단계를 나타냅니다. </p> <p>자세한 내용은 <a href="/help/main/c-experiences/c-manage-content/aem-experience-fragments.md#topic_1E1E4EA01F074349B2CF8785387B5FE8" format="dita" scope="local"> AEM 경험 구성 요소 </a>를 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>보고서 </p> </td> 
   <td colname="col2"> 
