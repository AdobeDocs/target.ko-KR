...
* 기존 VEC에서 사용할 수 있는 기능에 맞게 업데이트된 [!UICONTROL Rearrange]&#x200B;(VEC) UI에 [!UICONTROL Visual Experience Composer] 옵션을 추가했습니다. (TGT-46957 및 TGT-52876)
...
* 업데이트된 UI에서 변경 내용을 만들 때 [!UICONTROL page load]에 [!DNL Target] 이벤트가 표시되지 않는 문제가 해결되었습니다. 업데이트는 보기에만 적용됩니다. (TGT-52604)
...
* 업데이트된 VEC에서 [!UICONTROL Automated Personalization]&#x200B;(AP) 활동의 기본 콘텐츠 처리가 이전 UI와 일치하지 않는 문제를 해결했습니다. 이제 명시적으로 추가된 그룹이 없을 때 시스템은 `optionGroup`을(를) 가진 &quot;기본 콘텐츠&quot;라는 기본 `optionGroupLocalId = 0`을(를) 자동으로 추가합니다. 이 그룹에는 기본 옵션(예: `optionLocalId: 0`)이 포함되어 있습니다. 기본 콘텐츠가 제거되면 해당 옵션 그룹도 제거됩니다. (TGT-52651)
* 이전에 제거한 경험에서 [!UICONTROL Multivariate Test]을(를) 다시 사용할 수 없도록 잘못 설정된 `experienceLocalId`(MVT) 활동의 문제를 해결했습니다. (TGT-52672)
...
* [!UICONTROL Form-Based Experience Composer] 드롭다운에서 특수 문자가 있는 mbox를 마우스로 가리키면 편집기가 비어 있고 &quot;Element&quot;에서 &quot;querySelector&quot;를 실행하지 못했습니다.&quot;가 트리거되는 [!UICONTROL Location]의 문제를 해결했습니다. 라는 오류가 표시됩니다. (TGT-52717)
...
* 업데이트된 [ UI 및 ](/help/main/c-intro/updated-ui-faq.md)(VEC)에 대한 일반적인 질문을 해결하기 위해 [!DNL Target]새 FAQ 문서[!UICONTROL Visual Experience Composer]을 추가했습니다.
* [!UICONTROL URL - does not contain]의 &quot;[!UICONTROL Page Delivery]&quot; 규칙이 작동하지 않아 콘텐츠를 차단해야 하는 경우에도 표시할 수 있는 문제가 해결되었습니다. (TGT-52754)
...
* [!UICONTROL Visual Experience Composer] 활동의 추가 페이지를 동일한 URL로 잘못 리디렉션할 수 있는 업데이트된 [!UICONTROL A/B Test]&#x200B;(VEC)의 문제를 해결했습니다. (TGT-51838)
...
* `optionLocalIds`에서 참조된 모든 `optionGroups`이(가) 옵션 배열에 있는지 확인하기 위해 유효성 검사를 추가했습니다. 잘못된 참조는 활동을 만드는 동안 자동으로 제거됩니다. (TGT-52687)
...
* 권장 사항이 있는 [!UICONTROL Download Recommendations data] (XT) 활동에 대해 &quot;[!UICONTROL Experience Targeting]&quot; 옵션이 누락되는 문제를 해결했습니다. (TGT-52730 및 TGT-52756)
...
* 잘못된 입력 오류로 인해 E[!DNL Target]이(가) 올바르게 표시되지 않는 업데이트된 [!UICONTROL xperience Fragments] UI의 문제를 해결했습니다. (TGT-52701)
...
* 기존 [!UICONTROL Reports] UI의 기능에 맞추기 위해 [!UICONTROL Report Settings] > ![( ](/help/main/assets/icons/Setting.svg)보고서 설정 아이콘[!DNL Target]) 아래에 QA 모드 트래픽 설정이 추가되었습니다. (TGT-52228 및 TGT-52329)
...
* [!UICONTROL Experience Fragments]의 [!UICONTROL Offers Library]에 대한 최근 업데이트를 활동 내에서 사용하려고 할 때 반영하지 않는 문제를 해결했습니다. (TGT-52659)
...
* AP 활동에서 콘텐츠를 관리할 때 [!UICONTROL Contains]에서 &quot;[!UICONTROL Filter Rules]&quot; 옵션이 누락되는 문제를 해결했습니다. (TGT-52384)
* [!UICONTROL Automated Personalization]의 내부 시스템에서 `optionLocalId` 값을 사용하여 기본 오퍼를 추적하고 보고하는 방법과 관련된 [!DNL Target]&#x200B;(AP) 활동의 보고 불일치를 수정했습니다.
...
* [!UICONTROL Visual Experience Composer] 활동에 대한 [!DNL Recommendations]&#x200B;(VEC)에서 시퀀스 기준이 올바르게 표시되지 않는 문제가 수정되었습니다. (TGT-52435)
...
[!DNL AI Assistant]에 [!DNL Adobe Target]의 시작을 알려드리겠습니다! 이 강력한 사용자 인터페이스 기능은 [!DNL Target] 개념을 쉽게 탐색하고 이해할 수 있도록 설계되었습니다. [!DNL Adobe Experience Cloud]을(를) 포함하여 [!DNL Target]의 여러 제품에서 사용할 수 있습니다. [!DNL AI Assistant]이(가) 사용자의 경험을 혁신하기 위해 여기에 있습니다.

[!DNL AI Assistant]의 [!UICONTROL Target]은(는) [!DNL Experience Platform]개의 응용 프로그램 및 서비스를 통해 워크플로를 가속화하는 데 사용할 수 있는 대화 도구입니다. [!DNL AI Assistant]을(를) 사용하여 전반적인 생산성을 높이고 제품 지식을 이해합니다.
...
* [!DNL Target] 및 [!UICONTROL Product Administrator] 권한이 있는 [!UICONTROL System Administrator] 사용자는 이제 [!UICONTROL Administration]의 역할에 관계없이 [!DNL Target] 페이지의 모든 설정을 편집할 수 있습니다. 이러한 권한이 없는 사용자는 이러한 설정에 대해 읽기 전용 액세스 권한을 갖습니다. 이 업데이트는 [관리 설정](/help/main/administrating-target/administrating-target.md)에 대한 액세스 제어를 강화합니다. (TGT-48179)
...
* [!UICONTROL Visual Experience Composer]페이지 배달[ URL이 슬래시(/)로 끝났을 때 ](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#settings)(VEC)이 올바른 페이지를 표시하지 못하는 문제를 해결했습니다. (TGT-52237)
...
* [!DNL Customer Journey Analytics] 활동 페이지에서 [!UICONTROL Overview]이(가) 올바르게 표시되지 않도록 차단하는 문제를 해결했습니다. (TGT-52359)
...
* [!UICONTROL Managed Content]이(가) 활성화되었음에도 불구하고 [!UICONTROL Reporting Audiences] 및 [!UICONTROL Show Only Selected]에 대한 대상을 선택한 후 대상 이름이 표시되지 않는 문제를 해결했습니다. (TGT-52393)
...
* [!UICONTROL Activity] 설정 페이지와 [!UICONTROL Reporting] 개요 페이지 간에 대상 목록이 일치하지 않는 문제를 해결했습니다. (TGT-52203)
* 잘못된 사용자 입력 오류로 인해 활동에 새 페이지를 추가할 수 없는 문제를 해결했습니다. (TGT-52263)
* 옵션이 변경되지 않은 경우 `OptionLocalIDs`이(가) 잘못 증가하는 문제를 해결했습니다. (TGT-52187)
* 옵션이 변경되지 않은 경우 `location` 및 `OptionLocalIDs`이(가) 잘못 증가하는 문제를 해결했습니다. (TGT-52188)
* 활동의 [!UICONTROL Overview] 페이지에서 위치가 올바르지 않은 문제를 해결했습니다. (TGT-52182)
* 잘못된 위치에 잘못된 선택기 경고가 표시되지 않던 문제를 수정했습니다. (TGT-52110)
* 다운로드한 보고 파일에 보고 UI에 있는 데이터가 올바르게 표시되도록 문제를 해결했습니다. (TGT-52068)
* 페이지 전달 규칙을 추가한 후 배치 작업이 실패하지 않도록 문제를 해결했습니다. (TGT-52097)
* [!DNL Target]이(가) 웹 사이트의 URL에서 모든 쿼리 매개 변수를 트리밍하는 문제를 해결했습니다. (TGT-52100)
* 고객이 기존 및 업데이트된 [!DNL Target] UI 모두에서 활동을 만들 수 없도록 하는 콘솔 오류가 해결되었습니다. (TGT-52181)
...
* 업데이트된 [!UICONTROL Visual Experience Composer]&#x200B;(VEC)에서 이동 경로가 편집기 하단에 항상 표시되지 않아 요소를 정확하게 선택할 수 없는 문제가 해결되었습니다. (TGT-52169)
...
* 고객이 모든 작업 영역에서 활동을 만들 수 없도록 차단하는 문제를 해결했습니다. (Tgt-52218)
...
2025년 4월 22일부터 [!UICONTROL Product] 작업 영역의 역할에 관계없이 [!UICONTROL Solutions] 및 [!UICONTROL Administration] 관리자만 [!DNL Target] 섹션의 설정을 업데이트할 수 있습니다. 이 권한이 없는 사용자는 [!UICONTROL Administration] 섹션에 대한 읽기 전용 액세스 권한을 갖게 됩니다.
...
* VEC([!UICONTROL All Visitors])에서 변경한 후 대상 설정이 &quot;[!UICONTROL Visual Experience Composer]&quot;(으)로 되돌아가는 문제가 해결되었습니다. (TGT-52132)
...
* 오퍼가 &quot;콘텐츠를 찾을 수 없음&quot;으로 표시되고 [!UICONTROL Offers]&#x200B;(AP) 활동의 [!UICONTROL Automated Personalization] 페이지에 표시되지 않는 문제가 해결되었습니다. (TGT-52150)
...
* 고객이 편집 후 [!UICONTROL Optimization Goal] 페이지의 [!UICONTROL Reporting Settings]에서 [!UICONTROL Goals & Settings]을(를) 다시 구성해야 하는 문제를 해결했습니다. (TGT-52071)
...
* 전체 경험 URL이 [!UICONTROL Browse mode]&#x200B;(VEC) 내의 [!UICONTROL Visual Experience Composer]에 표시되지 않는 문제를 해결했습니다. (TGT-52101)
* 활동을 편집하면 페이지 게재가 URL 끝에 &quot;/&quot;를 추가하여 유효하지 않게 되는 문제를 해결했습니다. (TGT-52114)
* [!UICONTROL Activity QA]의 [!UICONTROL Form-Based Experience Composer] 링크가 [!DNL Adobe Experience Cloud] 홈 페이지로 잘못 리디렉션되는 문제가 해결되었습니다. (TGT-52055)
...
* 기존 UI와 유사한 [ 대화 상자에서 ](/help/main/c-target/c-audiences/audiences.md#section_11B9C4A777E14D36BA1E925021945780)대상 정의 카드[!UICONTROL Activity QA]에 대한 액세스를 복원했습니다. (TGT-52056)
...
* [!DNL Target]을(를) 취소할 때 [!UICONTROL ClickTrack]이(가) 선택기를 잘못 저장하는 문제를 해결했습니다. (TGT-51937)
...
* `companyName` 지표에 대한 `reportSuite` 구성에서 [!DNL Analytics] 및 `pageviews`과(와) 일치하지 않는 문제를 해결했습니다. (TGT-51965)
...
* 동일한 [!UICONTROL Visual Experience Composer] 선택기를 여러 번 사용할 때 [!UICONTROL Click Tracking]이(가) 활성화된 [!UICONTROL ClickTrack]&#x200B;(VEC) 활동에서 &quot;잘못된 사용자 입력&quot; 오류가 해결되었습니다. (TGT-51921)
...
* 활동 만들기 프로세스를 취소([!UICONTROL Cancel] 대신 [!UICONTROL Add Audience] 선택)할 때 모든 상태 대상 변경 내용이 제거되던 문제를 해결했습니다. (TGT-51769 및 TGT-51770)
...
* 활동을 편집하는 동안 [!UICONTROL Cancel]을(를) 클릭하면 Target UI가 [!UICONTROL Activities List] 페이지 대신 [!UICONTROL Activity Details] 페이지로 이동하는 문제가 해결되었습니다. (TGT-51731)
* 고객이 [!UICONTROL Export Reports to CSV] 옵션을 통해 보고서를 다운로드할 수 없는 문제를 해결했습니다. (TGT-51708)
* [!DNL Target Standard] 고객이 [!UICONTROL Properties] 기능인 [!DNL Target Premium]을(를) 사용하는 것으로 잘못 표시되는 양식 기반 경험 작성기의 문제를 해결했습니다. (TGT-51678)
* 새 오퍼를 만들 때 [!DNL Adobe Experience Platform] 특성이 표시되지 않도록 차단하는 문제를 해결했습니다. (TGT-51665)
* [!DNL Recommendations] 인벤토리에 대한 모든 활성 필터를 빠른 검색으로 이동하여 UI를 [!UICONTROL Catalog Search] 레일이 아닌 [!UICONTROL Filter]과(와) 맞췄습니다. (TGT-50723)
...
>[!IMPORTANT]
>
>[!DNL Target] UI 버전 전환 수명 종료 계획에 대한 중요한 정보는 [[!DNL Target] UI 버전 전환 중단](/help/main/r-release-notes/release-notes.md#toggle)을 참조하십시오.
...
### Target Standard/Premium 25.1.1(2025년 1월 9일) {#ui-update-1}
...
수동 제안 렌더링의 경우 이 최적화를 활용하려면 [[!DNL Platform Web SDK implementation]](https://experienceleague.adobe.com/ko/docs/target-dev/developer/client-side/aep-web-sdk){target=_blank}을(를) 검토하여 경험을 수동으로 렌더링한 후 또는 `applyPropositions` 메서드(또는 해당 [!DNL Launch] 작업을 도우미로 사용)를 사용하여 경험을 렌더링할 때 알림을 보내고 있는지 확인하십시오.
...
### `Browser:iPad` 대상 특성의 `Browser:iPhone` 및 [!UICONTROL Browser]에 대한 업데이트(2024년 4월 30일)

| 업데이트 | 세부 사항 |
|--- |--- |
| 대상을 만들 때 사용된 [!UICONTROL Browser:iPad]브라우저 특성[!UICONTROL Browser:iPhone]에서 [ 및 ](/help/main/c-target/c-audiences/c-target-rules/browser.md)이(가) 업데이트되었습니다. | [!DNL Adobe Target]을(를) 사용하면 페이지를 방문할 때 특정 [브라우저 또는 브라우저 옵션](/help/main/c-target/c-audiences/c-target-rules/target-rules.md)을 사용하는 방문자를 포함하여 [여러 범주 특성 중 하나를 타깃팅할 수 있습니다](/help/main/c-target/c-audiences/c-target-rules/browser.md).<P>[!DNL Target] Standard/Premium 24.3.1(2024년 3월 4~6일)부터 Target UI를 사용하여 만든 기본 제공 대상(예: `Browser:iPad` 및 `Browser:iPhone`)이 [!DNL iPad], [!DNL iPhone] 및 `profile.mobile.deviceVendor`을(를) 사용하여 `profile.mobile.isMobilePhone` 및 `profile.mobile.isTablet`에 대한 적절한 타깃팅을 수행하도록 업데이트됩니다.<P>이 업데이트에는 고객 측에서 작업을 수행할 필요가 없습니다.<p><B>중요</b>: 고객이 프로필 스크립트(및 JavaScript 세그먼트)에서 [!DNL iPad] 및 [!DNL iPhone]에 대한 적절한 타깃팅을 수행하려면 **2024년 4월 30일**&#x200B;까지 고객이 수동으로 변경해야 합니다. 수동으로 변경해야 하는 대체 설정의 예는 [대상 특성  [!DNL iPad] 의  [!DNL iPhone] 및[!UICONTROL Browser]에 대한 업데이트](/help/main/c-target/c-audiences/c-target-rules/browser.md#updates)를 참조하십시오. |

### [!UICONTROL Visual Editing Helper] 확장(2024년 3월 14일)

이번 릴리스에는 [[!DNL Adobe Experience Cloud Editing Helper]](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)의 [!DNL Google Chrome] 확장에 대한 다음과 같은 개선 사항 및 수정 사항이 포함되어 있습니다.

* 고객 웹 사이트에서 작성을 수행할 때 iFrame 로드 메커니즘을 개선했습니다.
* VEC([!UICONTROL Visual Experience Composer])에서 작성을 수행하는 동안 확장에서 쿠키를 복제하는 문제가 해결되었습니다.
...
* [!UICONTROL Multivariate]&#x200B;(A4T) 보고로 구성된 [!UICONTROL Analytics for Target]&#x200B;(MVT) 활동이 제대로 표시되지 않는 문제를 해결했습니다. (TGT-47490)
...
* 매출 목표 지표가 있는 [!UICONTROL Analytics for Target]&#x200B;(A4T) 활동이 열 이름으로 &quot;매출&quot;을 표시하지 않았고 매출 지표가 보고에서 ($) 형식으로 표시되지 않았습니다. 이는 이미 해결된 표시용 문제였습니다. (TGT-46995)
* 보고 일자 간격이 올바르게 작동하지 않는 문제를 수정했습니다. (TGT-47396)
* 고객이 [!UICONTROL All Activities] 아이콘을 사용하여 활동을 활성화하거나 비활성화한 후 [!UICONTROL More Actions] 페이지에 잘못된 상태가 표시되는 문제를 해결했습니다. (TGT-47367)
* 단일 고객에 대해 [!UICONTROL Important Attributes] 보고서가 표시되지 않는 문제를 해결했습니다. (TGT-47272)
...
* [ 활동의 경험에 대해 ](/help/main/c-activities/c-activity-qa/activity-qa.md)중복 오퍼 허용 안 함[을(를) 지원하도록 ](/help/main/c-activities/t-automated-personalization/managing-exclusions.md)활동 QA[!UICONTROL Automated Personalization] 기능을 개선했습니다. (TGT-46627)
...
* 일부 고객의 [!UICONTROL Activities] 페이지에서 활동이 제대로 표시되지 않는 문제를 해결했습니다. (TGT-46830)
* [[!UICONTROL Analytics for Target]](/help/main/c-integrating-target-with-mac/a4t/a4t.md)(A4T)을(를) 보고 소스로 사용하는 활동에 영향을 주는 다음 문제를 해결했습니다.
   * 일부 고객이 보고 데이터를 조회할 수 없는 문제가 해결되었습니다. (TGT-46557)
   * 활동 보고 페이지의 [!UICONTROL View in Analytics] 링크가 제대로 작동하지 않는 문제가 해결되었습니다. (TGT-46731)
   * [!UICONTROL Lift] 및 [!UICONTROL Confidence]의 데이터가 [!DNL Target] UI에 제대로 표시되지 않는 문제를 해결했습니다. (TGT-46592, TGT-46554 및 TGT-46586)

### [!UICONTROL Activities] 페이지 사용자 인터페이스 새로 고침(2023년 10월 25일)

[!DNL Adobe Target] 사용자의 사용자 경험을 개선하기 위한 [!DNL Target] 팀의 지속적인 노력의 일환으로 이 릴리스는 [!UICONTROL Activities] UI의 [!DNL Target] 페이지를 새로 고칩니다. 이번 업데이트를 통해 새로운 개선 사항이 추가되었으며 이전에 일관되지 않았던 디자인 패턴이 통합되고 표준화되었습니다.
...
* 새 활동을 만들 때 [!UICONTROL Activities]&#x200B;(VEC)이 [!UICONTROL Visual Experience Composer]의 기본 설정으로 열리도록 새 `selectorCriteria` UI가 개선되었습니다. (TGT-46586)
* VEC를 사용할 때 일부 고객이 [!UICONTROL Composer] 모드에서 요소를 편집할 수 없는 문제를 해결했습니다. (TGT-46470)
* 사용자 정의 속성을 사용할 때 일반적으로 많이 사용되는 선택기를 지정하는 기능이 추가되었습니다. (TGT-46545)
* 보고서가 [!UICONTROL Auto-Target]에 올바르게 표시되더라도 경우에 따라 [!UICONTROL Analytics for Target]&#x200B;(A4T)을(를) 사용하는 [!DNL Target] 보고서가 [!DNL Adobe Analysis Workspace] UI에 표시되지 않는 문제를 해결했습니다. (TGT-46494)
...
* Lightning Web Components(Light DOM)를 지원하도록 [!UICONTROL Visual Experience Composer]&#x200B;(VEC)을 개선했습니다. (TGT-45422)
* VEC 액션이 잘못된 순서로 적용되는 문제를 수정했습니다. 경우에 따라 VEC가 일부 수정 사항을 비동기적으로 적용했으며 요소에 추가 수정 사항을 추가하면 해당 요소가 [!UICONTROL Insert] 작업 후에 표시되는 경우 오류가 발생합니다. 또한 앵커 링크를 클릭할 때 업데이트되는 VEC URL도 수정했습니다. (TGT-45983)
* 이제 그림자 DOM의 요소를 지원하는 VEC [!UICONTROL Overlay] 기능 문제를 해결했습니다. (TGT-45202 및 TGT-45262)
* VEC에서 SPA(단일 페이지 애플리케이션) 페이지를 연 다음 [!UICONTROL Browse] 모드로 이동하면 뒤로 및 앞으로 화살표가 올바르게 작동하지 않는 문제를 해결했습니다. (TGT-45956)
...
* [!UICONTROL Automated Personalization] 백엔드에서 활동을 시기 적절하게 동기화하고 미리 보기 링크에서 예상되는 경험을 전달하는 등 일부 고객의 [!DNL Target]&#x200B;(AP) 활동에 영향을 미치는 문제가 수정되었습니다. (TGT-46202)

### [!DNL Target] Standard/Premium 23.9.1 (2023년 9월 6~11일)

이번 릴리스에는 다음과 같은 개선 및 수정 사항이 포함되어 있습니다.

* [!DNL Target]&#x200B;(A4T)을(를) 보고 소스로 사용하는 [!DNL Adobe Analytics] 활동에 대한 [!UICONTROL Auto-Allocate] UI 및 [!UICONTROL Analytics for Target] UI에서 보고 데이터가 일치하지 않는 문제를 해결했습니다. (TGT-46112)
...
* [!UICONTROL Status] 목록 페이지의 &quot;[!UICONTROL Activity]&quot; 열에 표시된 대로 활동이 제대로 동기화되지 않는 문제가 해결되었습니다. (TGT-46010 및 TGT-44831)
* [!UICONTROL View in Analytics]&#x200B;(A4T)을(를) 보고 소스로 사용하는 활동의 [!UICONTROL Reports] 페이지에 &quot;[!UICONTROL Analytics for Target]&quot; 링크가 표시되지 않는 문제가 해결되었습니다. (TGT-45808)
* 소수 자릿수가 포함 숫자 대신 백분율로 표시되도록 테이블의 값 표시가 조정되었습니다. 예를 들어 .08대신 8%가 표시됩니다. (TGT-45548)
* 고객이 키보드 포커스를 사용하여 [!UICONTROL Goals & Settings]&#x200B;(XT) 활동에 대한 [!UICONTROL Experience Targeting] 페이지의 다음 요소로 이동할 수 없는 문제를 해결했습니다. (TGT-44526)
* 활동을 만드는 동안 &quot;[!UICONTROL Add audiences]&quot; 대화 상자를 연 후 키보드 포커스가 손실되는 문제를 해결했습니다. (TGT-44525)

### [!DNL Target] Standard/Premium 23.7.1 (7월 24~26일)

이번 릴리스에는 다음과 같은 개선 및 수정 사항이 포함되어 있습니다.

* 섀도 DOM 요소를 포함하도록 [VEC(](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path))에서 DOM 경로를 사용하여 요소 탐색[!UICONTROL Visual Experience Composer]할 때 검색이 개선되었습니다. (TGT-45262)
...
| 기능 | 세부 사항 |
|--- |--- |
| [!UICONTROL QA mode] 활동에 대한 [!UICONTROL Automated Personalization] | [!DNL Adobe Target] [!UICONTROL QA mode]을(를) 이제 [!UICONTROL Automated Personalization] 활동에 사용할 수 있으며 [!UICONTROL Preview links] 기능을 대체합니다.<P>자세한 내용은 [활동 QA](/help/main/c-activities/c-activity-qa/activity-qa.md)를 참조하십시오. |

* [ 활동에서 ](/help/main/c-activities/t-automated-personalization/managing-exclusions.md#concept_4EF78013F80E48EFA024AE0274C9F037)제외를 관리[!UICONTROL Automated Personalization]하는 동안 중복 기능을 허용하지 않도록 성능이 향상되었습니다(로드 시간 감소 포함).
...
* 이미지 이름에 GB 18030 문자가 포함된 경우 [!UICONTROL Create Design] 패널에 이미지가 표시되지 않는 문제를 해결했습니다. (TGT-44614)
* 경험의 텍스트/HTML에서 일부 GB 18030 기호 문자가 잘못 이스케이프되는 문제가 해결되었습니다. (TGT-44600)
* 분석 중에 [!UICONTROL Auto Personalization] 활동에 대한 보고서가 중지되는 문제를 해결했습니다. (TGT-44820)
* 활동 이름에 대괄호([!UICONTROL Activity] 또는[)가 포함된 경우 ] 페이지에서 활동을 검색할 수 없는 문제를 해결했습니다. (TGT-44777)
...
* URL에 쿼리 매개 변수를 추가하여 VEC([!UICONTROL Visual Experience Composer])에서 페이지를 로드할 수 없는 문제를 해결했습니다. (TGT-44873)
* [!DNL Target] UI 전반에 걸쳐 다양한 현지화 수정을 수행했습니다.

### [!DNL Target] [!UICONTROL Real-Time CDP Profile Attributes]과(와) 공유된 Real-Time CDP 프로필 특성(2023년 6월 13일)

이번 릴리스에는 다음과 같은 개선 사항이 포함됩니다.

| 기능 | 세부 사항 |
|--- |--- |
| [!DNL Target]과 공유되는 Real-Time CDP 프로필 속성 | [!UICONTROL Real-Time CDP Profile Attributes]은(는) HTML 및 JSON 오퍼에서 사용할 수 있도록 [!DNL Target]과(와) 공유할 수 있습니다.<P>자세한 내용은 [ [!DNL Target]](/help/main/c-integrating-target-with-mac/integrating-with-rtcdp.md#rtcdp-profile-attributes)과 Real-Time CDP 프로필 속성 공유를 참조하십시오. |
...
| 기능 | 세부 사항 |
|--- |--- |
| [!UICONTROL Auto-Allocate] 및 [!UICONTROL Auto-Target]에 최적화된 A4T 지표<p>(릴리스 일자: 2023년 3월 30일) | [!DNL Target]을(를) 사용하면 [!UICONTROL A4T] 및 [!UICONTROL Auto-Allocate] 활동에 [!UICONTROL Auto-Target]을(를) 사용할 때 이항 이벤트를 기반으로 하는 지표 또는 연속 이벤트를 기반으로 하는 지표를 선택할 수 있습니다.<P>다음은 유의해야 할 지원되는 지표에서의 변경 내용입니다.<ul><li>[!DNL Target]은 2023년 9월 9일까지의 기존 활동에 대한 이전 동작을 유지했습니다. 이 날짜 이후에는 기존 활동을 새 동작으로 강제 마이그레이션하기 위해 지원되지 않는 지표를 사용하는 활동이 중단됩니다.</li></ul>자세한 내용은 [ 및 [!UICONTROL Auto-Allocate] 활동에 대한 [!UICONTROL Auto-Target]A4T 지원](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md#supported)에서 &quot;지원되는 목표 지표&quot;를 참조하십시오.<br>이 기능과 함께 다음 튜토리얼이 업데이트되었습니다.<ul><li>[A4T 보고서 설정 [!DNL Analysis Workspace] 활동 [!UICONTROL Auto-Allocate]용](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-allocate-activities.html?lang=ko-KR){target=_blank}</li><li>[A4T 보고서 설정 [!DNL Analysis Workspace] 활동 [!UICONTROL Auto-Target]용](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-target-activities.html?lang=ko-KR){target=_blank}</li></ul> |
...
* 사용자가 [!UICONTROL Default URL] > [!UICONTROL Administration] > [!UICONTROL Visual Experience Composer]에서 [!UICONTROL Default URL]을(를) 제거할 수 있도록 UI가 개선되었습니다. 이 변경을 통해 고객은 기본 URL을 빈 문자열로 다시 변경할 수 있게 되었습니다(이전에는 초기 구성 이후 변경할 수 없었음). (TGT-44577)
* 고객이 기본 제공 대상자(예약된 이름이 있는 대상자)을 편집하거나 삭제할 수 없도록 하는 제한이 제거되었습니다. (TGT-44655)
* [!UICONTROL Done]결합된 대상[!DNL Target]을(를) 만들 때 [ UI에 표시되는 분할자를 로드하는 동안 &quot;](/help/main/c-target/combining-multiple-audiences.md)&quot; 옵션이 비활성화되었습니다. (TGT-44079)
* [!UICONTROL Language] 페이지 하단에 있는 [!UICONTROL Audiences] 링크를 수정하여 &quot;[!UICONTROL Account communication preferences]&quot; 페이지로 올바르게 연결되었습니다. (TGT-43562)
* [!UICONTROL A/B Test] > [!UICONTROL Adobe Analytics] > [!UICONTROL Administration]에서 [!UICONTROL Reporting] 옵션을 선택한 후 고객이 [!UICONTROL Reporting Experience Cloud Solution] 활동을 만들 수 없는 문제를 해결했습니다. (TGT-44844)
* 고객이 [!UICONTROL Multivariate Test]&#x200B;(VEC) 내에서 많은 경험이 있는 [!UICONTROL Visual Experience Composer] 활동의 마지막 경험을 볼 수 없는 문제를 해결했습니다. 때때로 VEC 하단의 [DOM 경로](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path)로 인해 고객이 마지막 경험을 보지 못하는 경우가 있었습니다. (TGT-44578)
* 페이지에 인증이 필요하거나 페이지에서 리디렉션을 호출하는 경우 VEC의 검색 URL이 일반 브라우저 세션에 표시되는 현재 페이지를 반영하지 않는 문제가 해결되었습니다. (TGT-44350)
* 고객이 [!UICONTROL Filter Incompatible Criteria] > [!UICONTROL Recommendations]에서 [!UICONTROL Settings] 설정을 변경할 수 없는 문제를 해결했습니다. (TGT-44398)
* 이름에 점이 있는 보고서 세트가 있는 [!DNL Recommendations]을(를) 사용할 때 POST 요청에서 [!UICONTROL Analytics Classifications]개의 피드를 만들지 못하는 문제가 해결되었습니다. (TGT-44598)
...
* [!UICONTROL Visual Experience Composer]&#x200B;(VEC)을(를) 사용하여 사용자 지정 웹 구성 요소를 작성하는 업데이트:
...
* 서버측 추천 피드에 영향을 미치는 잠재적인 보안 문제가 해결되었습니다. (TGT-43769)
...
* 권장 사항이 포함된 [!UICONTROL A/B Test] 및 [!UICONTROL Experience Targeting]&#x200B;(XT) 활동에서 &quot;500 오류&quot;가 발생하는 문제를 해결했습니다. 이 문제는 [!DNL Target]이 더 이상 사용되지 않는 [!DNL Target] UI 및 [!DNL Recommendations] 백엔드에서 기준 오브젝트를 제대로 삭제하지 못했을 때 발생했습니다. (TGT-44383)
* [!UICONTROL Offer Level] 활동에 대한 [!UICONTROL Automated Personalization] 보고서에 표시된 오퍼 이름에서 위치를 제거했습니다. 이렇게 변경하면 보고서를 보다 쉽게 읽을 수 있습니다. (TGT-44294)
* AP와 [!UICONTROL Auto-Target] UI의 [!UICONTROL Personalization Insights] [!UICONTROL Important Attributes] 및 [!DNL Target] 보고서에서 45일 및 90일 달력 옵션을 제거했습니다. 사용 패턴과 성능 향상을 위해 이들 날짜 범위는 더 이상 사용되지 않습니다. 현재 허용되는 범위인 15일, 30일 및 60일을 반영하도록 UI가 업데이트되었습니다. (TGT-39357)
...
* 고객이 A4T를 사용하는 [!DNL Target] 활동을 만들거나 편집하는 동안 최적화 기준을 선택할 수 있도록 [!UICONTROL Auto-Target] UI에 도구 설명을 추가했습니다.
...
| 기능 | 세부 사항 |
| --- | --- |
| [!DNL Adobe Experience Manager] (AEM) 경험 조각 | AEM 경험 조각 기능에 대한 업데이트는 다음과 같습니다.<ul><li>[!UICONTROL Offers] 목록에서 AEM 경험 조각을 유형(HTML 또는 JSON)별로 필터링하는 기능이 추가되었습니다. (TGT-43121)</li><li>고객이 지원되지 않는 VEC를 사용할 때 JSON [!UICONTROL Experience Fragment] 오퍼를 삽입할 수 있는 문제를 해결했습니다. JSON 오퍼는 [!UICONTROL Form-Based Experience] 작성기를 사용하는 경우에만 삽입할 수 있습니다. (TGT-43846)</li></ul>자세한 내용은 AEM [경험 조각](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)을 참조하십시오. |
| Google Chrome용 새 [!UICONTROL Visual Experience Composer] 확장 | Chrome 웹 스토어에서 Chrome용 새 [!DNL Adobe Target] [!UICONTROL Visual Experience Composer]&#x200B;(VEC) 확장을 사용할 수 있습니다.<br>2023년 1월부터는 Google이 Manifest V2를 사용하는 확장 기능을 허용하지 않으므로 현재 [!DNL Target] VEC Helper 확장 기능은 Google Chrome에서 작동을 중지합니다. 새해부터는 [!DNL Target]에서 웹 사이트를 계속 시각화하려면 새 확장 버전을 다운로드하십시오.<br>다음 링크에는 Chrome 웹 스토어의 두 가지 확장 기능이 표시됩니다.<ul><li>[새 확장](https://chrome.google.com/webstore/detail/adobe-experience-cloud-vi/kgmjjkfjacffaebgpkpcllakjifppnca){target=_blank}</li><li>[이전 확장](https://chrome.google.com/webstore/detail/adobe-target-vec-helper/ggjpideecfnbipkacplkhhaflkdjagak){target=_blank}</li></ul>추가 정보는 [Visual Editing Helper 확장 기능](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)을 참조하십시오. |

* 대상 규칙 정보가 [!UICONTROL Audiences Refinements] 정보 창에 제대로 표시되지 않는 문제를 해결했습니다. (TGT-43917)
...
* [!UICONTROL Modifications]에서 [!UICONTROL Experiences] 모드로 전환한 후 VEC에서 활동을 만들거나 편집할 때 일부 구성 요소가 [!UICONTROL Compose] 페이지의 [!UICONTROL Browse] 패널에 제대로 표시되지 않는 문제를 해결했습니다. (TGT-43300)
* 일부 고객이 [!UICONTROL A/B Test]을(를) 사용하는 [!UICONTROL Auto-Target] 활동을 보관할 수 없는 문제를 해결했습니다. (TGT-40978)
...
* at.js 2.10.0 이상 버전을 다운로드할 때 서드파티 쿠키의 설정을 허용하거나 비활성화하는 [!UICONTROL Cross-Domain] 옵션을 추가했습니다. (TGT-43674)
* [!DNL Recommendations] 피드 가져오기가 실패할 경우 고객에게 알리도록 [!DNL Target] UI의 알림을 업데이트했습니다. (TGT-35811)
* [!UICONTROL Decision Offers]이(가) [!UICONTROL Visual Experience Composer]&#x200B;(VEC) 내에서 제대로 작동하지 않는 문제를 해결했습니다. (TGT-43866)
* [!UICONTROL Clicked an Element]&#x200B;(MVT) 활동을 만드는 동안 [!UICONTROL Multivariate Testing] 전환 목표를 선택할 때 오류 메시지가 표시되는 문제를 해결했습니다. (TGT-43842)
* [!UICONTROL Impressions]&#x200B;(AP) 활동에 대해 다운로드한 CSV 보고서 파일에 [!UICONTROL Automated Personalization] 열이 표시되지 않는 문제를 해결했습니다. (TGT-43780)
* [!UICONTROL Form-Based Experience Composer]을(를) 사용할 때 경험을 복제한 후 고객이 HTML/JSON 오퍼를 편집할 수 없는 문제를 해결했습니다. (TGT-43633)
* 고객이 [!UICONTROL A/B Test] 활동을 기본이 아닌 작업 영역에서 다른 기본이 아닌 작업 영역으로 복사할 수 없는 문제를 해결했습니다. (TGT-41910)
* 권장 사항이 포함된 [!DNL Recommendations] 및 [!UICONTROL A/B Test]&#x200B;(XT) 활동에서 고객이 [!UICONTROL Experience Targeting]개의 개체(디자인, 기준, 컬렉션 등)를 올바르게 표시하고 더 이상 사용하지 않는 기준 개체를 [!DNL Target] UI 및 [!DNL Recommendations] 백엔드에서 삭제할 수 있도록 문제를 해결했습니다. (TGT-42331)
...
* 이번 릴리스에서는 [!UICONTROL Auto Personalization]&#x200B;(AP) 및 [!UICONTROL Auto-Target]&#x200B;(AT) 활동(이전에는 [!DNL Target]에서 반환하지 않음)에 대한 콘텐츠를 미리 가져올 수 있습니다. AP/AT 활동이 게재 경로에 있고 컨텐츠 전달에 동일한 위치를 사용하는 다른 AB/XT 활동보다 우선순위가 높은 경우 이 경우 미리 가져오기 호출의 경우(&quot;실행&quot; 흐름에 대한 변경 사항 없음) 최종 사용자가 볼 수 있는 경험이 변경될 수 있습니다.
...
### [!DNL Target Standard]개 대상 새로 고침(3월 28일)

이번 릴리스에는 다음과 같은 업데이트가 포함됩니다.

* 모든 [!UICONTROL Audiences] 고객에 대해 새 [!DNL Target Standard] UI가 활성화됩니다.

### Target Standard/Premium 고객 엔지니어링 수정 사항(2022년 3월 22일)

이번 유지 관리 릴리스에는 다음과 같은 개선 사항이 포함됩니다.

* [!DNL Analytics]&#x200B;(A4T)을(를) 사용하는 활동과 함께 `prefetch`을(를) 사용할 때 `pageLoad` 보기 및 [!UICONTROL Delivery API] 클릭 지표에 대한 [!UICONTROL Analytics as the reporting source] 페이로드 데이터를 반환하는 기능이 추가되었습니다. (TNT-43198)
* 일본에서 일반적으로 사용되는 브라우저 유형을 허용하도록 보트 필터링 사용자 에이전트 목록을 업데이트했습니다. (TNT-43867)

### Target Standard/Premium 22.2.1 (2022년 2월 1일)

이 유지 관리 릴리스에는 Target Standard/Premium 22.1.2 릴리스에서 발표된 새 [!UICONTROL Audiences] UI에 대한 다음과 같은 수정 사항 및 개선 사항이 포함되어 있으며 향후 6주 동안 모든 지역의 고객에게 출시될 예정입니다. 이들 수정 사항은 [!DNL Adobe Target Standard/Premium]에서 생성된 대상자에 대한 기능을 조정합니다.
...
* [!UICONTROL Delete], [!UICONTROL Audiences] 및 [!DNL Adobe Experience Platform]에서 가져온 대상에 대해 [!DNL Adobe Experience Cloud] 목록에 [!DNL Adobe Target Classic] 옵션을 추가했습니다. 또한 일괄 삭제 기능이 추가되었습니다. (TGT-42914)

### at.js 버전 2.8.1 (2022년 1월 28일)

* `pageLoad`이(가) [!UICONTROL On Device Decisioning]&#x200B;(ODD) 하이브리드 실행 모드에서 target-global-mbox에 매핑되지 않는 문제가 해결되었습니다.
...
* 일부 고객이 [!UICONTROL IPs to exclude from [!DNL Target] reporting data] > [!UICONTROL Administration]의 &quot;[!UICONTROL Reporting]&quot; 목록에서 IP 주소를 삭제하지 못하는 문제를 해결했습니다. (TGT-43384)
...
* 고객이 [!UICONTROL A4T] 활동 보고에서 [!DNL Analysis Workspace] 단추를 클릭하여 [!UICONTROL View in Analytics]에서 [!DNL Target] 패널을 열지 못하는 문제가 해결되었습니다. (TGT-42099, TGT-42100)
* [!UICONTROL Edit Design]을(를) 사용하여 [!UICONTROL A/B Test] 및 [!UICONTROL Experience Targeting]&#x200B;(XT) 활동을 편집하는 동안 [!UICONTROL Form-Based Experience Composer] 단추가 표시되지 않는 문제를 해결했습니다. (TGT-41980)
* 새 [!UICONTROL Compatible] 활동을 만드는 동안 [!UICONTROL Recommendations] 확인란이 기준 선택에 표시되지 않는 문제를 해결했습니다. (TGT-42053)
...
[!DNL Target]에 [!UICONTROL Audiences] [!DNL Adobe Experience Platform Web SDK]을(를) 사용할 때 다음과 같은 기능이 추가되었습니다.
...
* [!DNL Target] UI의 다양한 위치에 경고 아이콘, 팝오버 및 메시지를 추가하여 소스에서 대상자가 삭제되었고 [!DNL Target] 활동에서 더 이상 사용할 수 없음을 표시할 수 있습니다.
   ...
   * 활동 만들기 워크플로의 [!UICONTROL Experiences] 단계:
     ...
     ![[!UICONTROL Experiences] 페이지의 원본 메시지에서 삭제된 대상](assets/deleted-at-source-experiences.png)

   * 활동 만들기 워크플로의 [!UICONTROL Targeting] 단계:

     ![[!UICONTROL Targeting] 페이지의 원본 메시지에서 삭제된 대상](assets/deleted-at-source-targeting.png)

   * 활동 만들기 워크플로의 [!UICONTROL Goals & Settings] 단계:

     ![[!UICONTROL Goals & Settings] 페이지의 원본 메시지에서 삭제된 대상](assets/deleted-at-source-goals-settings.png)

   * 대상 세분화([!UICONTROL Replace Audience], 활동 만들기 워크플로의 [!UICONTROL Targeting] 단계):

* Combine Audiences 기능을 사용하려고 하고 소스에서 대상 중 하나가 삭제되면 [!UICONTROL Save]이(가) 비활성화됩니다.
...
| [!BADGE Premium]{type=Positive url="/help/main/c-intro/intro.md#premium newtab=true" tooltip="Target Premium에 포함된 내용을 확인합니다."} 권장 사항 기준 만들기 및 편집 | 목표 달성에 적합한 추천 항목 알고리즘 및 설정 선택을 단순화하기 위해 [!UICONTROL Recommendations Criteria] 만들기 및 편집 워크플로가 간소화되었습니다.<br>자세한 내용은 [기준 만들기](/help/main/c-recommendations/c-algorithms/create-new-algorithm.md)를 참조하십시오. |
...
* 일부 웹 브라우저의 서드파티 쿠키에 대한 새 보안 정책으로 인해 고객이 [!UICONTROL Visual Experience Composer]&#x200B;(VEC)에 로그인할 수 없는 문제가 해결되었습니다. [시각적 경험 작성기 및 고급 경험 작성기 관련 문제 해결](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/issues-related-to-the-visual-experience-composer-vec-and-enhanced-experience-composer-eec.md)의 “Google Chrome 버전 80 이상을 사용할 때 시각적 경험 작성기(VEC) 또는 향상된 경험 작성기(EEC) 페이지가 로드되지 않음”에서 이 문제를 논의합니다.
...
* VEC의 [!UICONTROL Experiences] 활동에 대한 [!DNL Recommendations] 페이지에서 &quot;기준 편집&quot; 단추가 제대로 표시되지 않는 문제를 해결했습니다. (TGT-39512)
* 테스트 작업 영역에 복제 및 복사하는 경우 활동을 동기화할 수 없는 문제가 해결되었습니다. (TGT-40686)
* VEC에서 &quot;[&quot;을(를) 사용할 때 ](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)경험 조각[!UICONTROL Insert After]을(를) 사용하여 선택기를 수정할 수 없는 문제를 해결했습니다. (TGT-41802)
* 오퍼의 빈 JSON 콘텐츠를 백엔드로 전송할 수 없는 문제가 해결되었습니다. 비어있더라도 [!DNL Target] 에서 JSON 오브젝트를 보냅니다. (TGT-41555)
* 보고서를 보는 동안 고객이 &quot;[!DNL Analytics]&quot;을(를) 클릭할 때 [!DNL Analysis Workspace] 대신 레거시 [!UICONTROL View in Analytics] 보고서가 열리는 문제가 해결되었습니다. (TGT-41867)
* 고객이 [!DNL Analytics] 활동에 대한 보고 소스(A4T)로 [!UICONTROL Automated Personalization]을(를) 선택하려고 할 때 표시된 UI 메시지에 추가 설명이 추가되었습니다. 메시지는 &quot;[!DNL Target]이(가) [!UICONTROL Automated Personalization] 활동에 대한 유일한 지원 소스임을 나타냅니다.&quot; (TGT-41954)
* 고객이 쉼표 대신 “새 줄”로 호스트 구분을 시도하는 경우 오류 메시지에 추가 설명이 추가되었습니다. (TGT-40671)
* 일부 활동의 &quot;[!UICONTROL Last Updated]&quot; 날짜가 스페인어 및 일본어 고객용 영어 UI와 다른 문제가 해결되었습니다(스페인어 및 일본어로 UI를 조회하는 경우). (TGT-38980)
...
* [!UICONTROL Auto Personalization]에서 만든 [!UICONTROL Form-Based Experience Composer] 활동에 대한 보고서가 보고서에서 삭제된 오퍼를 참조하도록 하는 문제를 해결했습니다. 이로 인해 “이 보고서에 대한 데이터를 검색하는 데 문제가 있습니다. 문제가 지속되면 Adobe 클라이언트 지원 센터에 문의하십시오.”라는 오류 메시지가 표시됩니다. (TGT-41028)
...
| 기능 | 세부 사항 |
| --- | --- |
| [!UICONTROL Analytics for Target]&#x200B;(A4T) | [!UICONTROL View in Analytics]을(를) 보고 소스로 사용(A4T)하는 활동에서 [!UICONTROL Reports] 페이지의 &quot;[!DNL Analytics]&quot; 링크를 클릭하면 이제 [!DNL Analysis Workspace]이(가) 열립니다. 이전에는 링크로 [!DNL Analytics] 보고서를 열 수 있었습니다. (TGT-36959) |
...
* [!UICONTROL Audiences] 페이지를 새로 고칠 때 기본 작업 영역이 다른 작업 영역으로 변경되는 문제를 해결했습니다. (TGT-38871)
* 오류 메시지를 유발하는 [!UICONTROL Administration] > [!UICONTROL Implementation]의 문제를 수정했습니다. 해당 오류 메시지 내용: &quot;전역 mbox가 동기화되지 않을 수 있습니다. 재저장을 해 보십시오.&quot;
...
| 기능 | 세부 사항 |
| --- | --- |
| [!UICONTROL Analytics for Target]&#x200B;(A4T)을(를) 사용하여 리디렉션 지원 | Platform Web SDK 가 이제 [A4T](/help/main/c-integrating-target-with-mac/a4t/a4t.md)를 사용할 때 [!DNL Target] 리디렉션을 지원합니다.<br>자세한 내용은 [Analytics for [!DNL Target] 구현](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)을 참조하십시오. |

### at.js 버전 2.5.0(2021년 5월 13일)

이 at.js의 릴리스에는 다음과 같은 개선 사항 및 변경 사항이 포함되어 있습니다.

* at.js에 대한 [온디바이스 의사 결정](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/on-device-decisioning/overview.html?lang=ko-KR){target=_blank} 지원
...
| 기능 | 세부 사항 |
| --- | --- |
| at.js<br>(발표될 날짜)에 대한 온디바이스 의사 결정 | 마케터와 개발자는 온디바이스 의사 결정을 통해 거의 0에 가까운 지연 시간에 사용자 브라우저에 대한 실험 및 개인화를 제공할 수 있습니다.<br>자세한 내용은 [at.js에 대한 온디바이스 의사 결정](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/on-device-decisioning/overview.html?lang=ko-KR){target=_blank}을 참조하십시오. |
...
* 대상을 [!UICONTROL All Visitors]&#x200B;(으)로 변경한 후 활동을 동기화할 수 없는 문제를 해결했습니다. (TGT-40259)
* [!UICONTROL Automated Personalization] 옵션이 활성화되어 있더라도 [!UICONTROL Disallow Duplicates] 활동의 다른 위치에서 사용하는 경우 오퍼가 중복되지 않는 문제를 해결했습니다. (TGT-39567)
* [!UICONTROL Administration] > [!UICONTROL Scene7 configuration] 페이지를 제대로 로드할 수 없는 문제를 해결했습니다. (TGT-39918)
...
* [!UICONTROL Personalization Insights]&#x200B;(AT) 및 [!UICONTROL Auto-Target]&#x200B;(AP) 활동에 대한 [!UICONTROL Automated Personalization]개의 보고서가 이제 매일 생성됩니다. 지난 15일, 30일 및 60일 동안 [!UICONTROL Automated Segments] 또는 [!UICONTROL Important Attributes]을(를) 제공하는 보고서를 선택할 수 있습니다. 다른 전환 확인 기간 설정을 매일 실행할 수 있도록 45일 및 90일 옵션이 제거되었습니다. (TGT-39472)
* 고객이 활동의 [!UICONTROL Edit Dependency] 페이지에서 [!UICONTROL Goals & Settings]을(를) 클릭할 때 현재 종속성이 표시되지 않는 문제를 해결했습니다. (TGT-39340)
...
* [!DNL Adobe Analytics] 활동에서 [!UICONTROL Analytics as the reporting source]&#x200B;(A4T)을(를) 사용할 때 [!UICONTROL Auto-Target] 지표를 선택할 때 경고가 추가되었습니다. [!UICONTROL Auto-Target] 모델은 이진(전환 기반) 지표로 작동하도록 최적화되었습니다. 매출과 같은 연속적인 지표를 선택하면 최적의 결과가 없을 수 있으며 [!UICONTROL Personalization Insights] 보고서가 정확하지 않을 수 있습니다. (TGT-38926)
* A4T를 사용하는 [!UICONTROL Auto-Target Summary] 활동에 대한 [!UICONTROL Auto-Target] 보고서에 상태 아이콘을 추가했습니다. 보고서에서 각 경험 옆에 있는 녹색 확인 아이콘은 해당 경험에 대해 개인화된 머신 러닝 모델이 생성되었음을 나타냅니다. 시계 아이콘은 모델을 만들 수 있는 충분한 트래픽이 제공되지 않았음을 나타냅니다. (TGT-38925)
* A4T 및 [!UICONTROL Automated Segments] 전환 지표를 사용하는 [!UICONTROL Important Attributes] 활동에 대한 [!UICONTROL Auto-Target] 및 [!DNL Analytics] 보고서가 생성되며 보고 소스로 [!DNL Target]을(를) 사용할 때와 동일하게 표시됩니다. (TGT-38931)
* [!UICONTROL Recommendations] [!UICONTROL Collections] 목록에 환경 필터링 옵션이 추가되었습니다. (TGT-38353)
* [!UICONTROL Recommendations] 컬렉션에서 잘못된 제품 수가 표시되는 문제를 해결했습니다. (TGT-39162)
* [!UICONTROL Last Updated] [!UICONTROL Recommendations]에 [!UICONTROL Catalog Search] 필터를 추가했습니다. (TGT-38340)
* 업계 카테고리를 변경한 후 [!UICONTROL Recommendations] 페이지가 중단되는 [!UICONTROL Create Sequence]의 문제를 해결했습니다. (TGT-38160)
* 사용자가 [!UICONTROL Automated Personalization]&#x200B;(AP) 활동에서 대상을 오퍼에서 제거할 수 없도록 하는 문제를 해결했습니다. (TGT-39058)
* 일부 고객의 [!UICONTROL Audience Info] 카드에 잘못된 시간대(시작 및 종료 날짜)가 표시되는 문제를 해결했습니다. (TGT-39150)
* 일부 고객이 [!UICONTROL Default Workspace]의 활동 목록을 볼 수 없도록 하는 문제를 해결했습니다. (TGT-38526)
...
* [!UICONTROL Average Lift Confidence Interval] 행에 대한 [!UICONTROL Confidence] 보고에서 [!DNL Auto-Target] 및 [!UICONTROL Total]이(가) 표시되지 않는 문제를 해결했습니다. 모든 개별 경험에 대해 측정이 올바르게 표시됩니다. (TGT-37301)
* 9월 15일 오후 2시 30분(PDT)부터 10월 6일 오전 9시 25분(PDT)까지 [!DNL Adobe Target Premium] 사용자의 [!UICONTROL Auto-Target] 보고에 영향을 미치는 문제를 해결했습니다. 영향을 받는 전환 지표에 대한 보고서(&quot;[!UICONTROL Viewed a page]&quot; 또는 &quot;[!UICONTROL Clicked on mbox]&quot; 옵션을 사용하여 구성됨)를 볼 때 전환 속도가 잘못 보고됩니다. 현재 알려진 전달 문제는 없습니다.
* [!UICONTROL Last Updated At] 테이블에 선택 가능한 [!UICONTROL Catalog Search] 열과 [!UICONTROL Last Updated At] 필터를 추가했습니다. 이렇게 하면 각 개별 항목을 열어 마지막으로 업데이트한 시기를 확인할 필요가 없고 항목이 마지막으로 업데이트된 날짜로 필터링할 수 있기 때문에 시간과 노력이 절약됩니다.
...
| 기능 | 세부 사항 |
| --- | --- |
| ![Premium 배지](/help/main/assets/premium.png) A4T(Analytics for Target)에서 자동 타겟 활동을 지원 | 이제 [!UICONTROL Auto-Target] 활동이 [Analytics for Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)을 지원합니다.<br>이 통합을 통해 [!UICONTROL Auto-Target] 앙상블 머신 러닝 알고리즘을 사용하여 각 방문자의 프로필, 동작 및 컨텍스트를 기반으로 최상의 경험을 선택할 수 있습니다.<br>A/B 테스트 및 경험 타깃팅 활동과 함께 사용할 수 있도록 이미 [A4T를 구현](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)했다면 모두 준비가 된 것입니다!<br>자세한 내용은 [자동 할당 및 자동 타겟 활동에 대한 A4T 지원](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)을 참조하십시오. |
...
* 조직을 전환한 후 새 [!UICONTROL Administration] 페이지를 로드할 때 오류가 발생하던 문제를 해결했습니다. (TGT-37730)
* [!UICONTROL Administration > Implementation] 페이지에 잘못된 클라이언트 코드가 표시되는 표시 문제를 해결했습니다. (TGT-37849)
* VEC를 성공적으로 로드한 후 사용자가 [!UICONTROL Visual Experience Composer]&#x200B;(VEC)의 편집 기능을 사용할 수 없게 되는 문제를 해결했습니다. (TGT-37162)
...
* 사용자가 조직을 전환한 후 경우에 따라 [!UICONTROL Administration > Implementation] 페이지에서 at.js를 다운로드할 수 없는 문제를 해결했습니다. (TGT-37668)
* 사용자가 다운로드 버튼을 여러 번 클릭할 경우 [!DNL Target] 이 여러 요청을 보내지 못하도록 로드하는 동안 at.js 다운로드 버튼이 비활성화되었습니다. (TGT-37633)
* 경험이 오랜 기간 동안 &quot;결과 가져오기&quot;를 표시하도록 했던 [!UICONTROL Experience Targeting]&#x200B;(XT) 활동의 문제를 해결했습니다. (TGT-37684)
...
* 특정 사용자가 [!UICONTROL Activity] 목록을 표시할 수 없도록 하는 문제를 해결했습니다. &quot;URL 제안을 가져올 수 없습니다.&quot;라는 오류 메시지가 표시됩니다. Adobe 백엔드 시스템의 이름(FirstName/r/n)에서 캐리지 리턴을 사용하는 사용자에게 오류가 발생했습니다. (TGT-37330)
* 작업 영역 이름([!UICONTROL Activity]에 지정됨)에 아포스트로피가 포함된 경우 사용자가 [!UICONTROL Adobe Admin Console for Enterprise] 페이지를 표시할 수 없는 문제를 해결했습니다. (TGT-37709)
* 보고서 세트가 이미 지정되었지만 최적화 및 전환 지표를 선택하는 동안 [!UICONTROL Auto-Allocate] 활동에서 사용자에게 보고서 세트를 선택하도록 잘못 알리는 오류 메시지가 있는 문제를 해결했습니다. (TGT-37689)
* [!UICONTROL Goals and Settings] 페이지로 이동한 다음 뒤로 이동하면 [!UICONTROL Targeting] 페이지의 지표가 비어 있는 문제가 해결되었습니다. (TGT-37691)
...
당사는 향상된 성능을 제공하고, 새로운 기능을 출시할 때 필요한 유지 관리 시간을 줄이고, 제품 전반에 걸쳐 사용자 경험을 개선할 수 있도록 새로운 기술 스택을 사용하여 전체 [!DNL Target] UI를 점차적으로 다시 작성하고 있습니다. 새로 고친 첫 번째 섹션은 [!UICONTROL Setup]&#x200B;(으)로 이름이 변경된 [!UICONTROL Administration] 섹션입니다.

이 새로 고침의 일환으로 [!UICONTROL Administration] 섹션의 페이지를 사용하여 다음과 같은 다양한 작업을 쉽게 수행할 수 있습니다.

* [!UICONTROL Implementation] 탭에서 최신 at.js 파일을 다운로드합니다(**[!UICONTROL Administration]** > **[!UICONTROL Implementation]**).
* at.js 설정을 사용자 지정하고 변경 내용을 쉽게 검토할 수 있습니다(**[!UICONTROL Administration]** > **[!UICONTROL Implementation]**).
* 기본 통화 및 시간대, 보고에서 제외할 IP 등 향상된 보고 설정을 수정합니다(**[!UICONTROL Administration]** > **[!UICONTROL Reporting]**).
* 개인 정보 보호를 위해 방문자 IP 주소 난독화(**[!UICONTROL Administration]** > **[!UICONTROL Implementation]**)
* Adobe Admin Console에서 관리하기 전에 작업 영역별 기존 사용자 목록과 해당 역할을 봅니다(**[!UICONTROL Administration]** > **[!UICONTROL Users]**).
* [!UICONTROL Administration] 섹션의 모든 테이블을 검색하고 필터링합니다.
...
* 확장 가능한 벡터 그래픽(SVG) 이미지를 사용하여 [!UICONTROL Insert After] > [!UICONTROL Image]이(가) 제대로 작동하지 않는 문제를 해결했습니다. (TGT-37242)
* 초안 활동의 삭제를 방해하는 [!UICONTROL Publisher] 역할이 있는 사용자의 문제를 해결했습니다. (TGT-37358)
* [!UICONTROL All My Workspaces]을(를) 선택할 때 사용자가 활동을 편집할 수 없도록 하는 문제를 해결했습니다. (TGT-37276)

### Target Standard/Premium 20.5.1(2020년 6월 17일)

| 기능/향상 | 설명 |
| --- | --- |
| [!UICONTROL Auto-Allocate]개 활동에 대한 A4T(Analytics for Target) 지원 | 이제 [!UICONTROL Auto-Allocate] 활동이 [Analytics for Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)을 지원합니다.<br>이 통합을 통해 [!UICONTROL Auto-Allocate] multi-armed bandit 기능을 사용하여 [!UICONTROL Adobe Analytics] 목표 지표 및/또는 [!UICONTROL Adobe Analytics] 보고 및 분석 기능을 사용하는 동안 트래픽을 승리 경험으로 유도할 수 있습니다.<br>A/B 테스트 및 경험 타깃팅 활동과 함께 사용할 수 있도록 이미 [A4T를 구현](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)했다면 모두 준비가 된 것입니다!<br>자세한 내용은 [자동 할당 및 자동 타겟 활동에 대한 A4T 지원](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)을 참조하십시오. |
| 자동 타겟 및 Automated Personalization 활동을 위 방법의 응답 토큰 | [ 및 ](/help/main/administrating-target/response-tokens.md) 활동에 두 개의 [!UICONTROL Auto-Target]응답 토큰[!UICONTROL Automated Personalization]이 추가되어 방문자가 &quot;제어&quot; 또는 &quot;대상&quot; 트래픽에 할당된 결과로 특정 경험을 받았는지 여부를 확인할 수 있습니다.<ul><li>`experience.trafficAllocationId`은(는) 방문자가 &quot;제어&quot; 트래픽에서 경험을 받은 경우 0을 반환하고, 방문자가 &quot;대상&quot; 트래픽 분배에서 경험을 받은 경우 1을 반환합니다.</li><li>`experience.trafficAllocationType`이(가) &quot;컨트롤&quot; 또는 &quot;대상&quot;을 반환합니다.</li></ul>제어와 타겟 트래픽에 대한 자세한 내용은 [Automated Personalization 또는 자동 타겟 활동의 제어 선택](/help/main/c-activities/t-automated-personalization/experience-as-control.md)을 참조하십시오. |
| [!UICONTROL Publisher] 역할 | 이 새 역할은 현재 [!UICONTROL Observer] 역할과 유사합니다(활동을 볼 수 있지만 만들거나 편집할 수는 없음). 그러나 [!UICONTROL Publisher] 역할에는 활동을 활성화할 수 있는 추가 권한이 있습니다.<br>자세한 내용은 다음 문서를 참조하십시오. <ul><li>**Target Standard 사용자**: *사용자*&#x200B;에서 [역할 및 권한 지정](/help/main/administrating-target/c-user-management/c-user-management/user-management.md#roles-permissions).</li><li>**Target Premium 사용자**: *기업 권한 구성*&#x200B;에서 [6단계: 역할 및 권한 지정](/help/main/administrating-target/c-user-management/property-channel/properties-overview.md#section_8C425E43E5DD4111BBFC734A2B7ABC80).</li></ul> |
| A4T는 [!DNL Analysis Workspace]<br>2020년 6월 25일에 지원 | [!UICONTROL Anaytics for Target]&#x200B;(A4T)이(가) 이제 [!DNL Analysis Workspace]에서 지원됩니다. [!UICONTROL Analytics for Target (A4T) panel]을(를) 통해 [!DNL Adobe Target]에서 [!DNL Analysis Workspace] 활동 및 경험을 분석할 수 있습니다.<br>자세한 내용은 [A4T 보고의](/help/main/c-integrating-target-with-mac/a4t/reporting.md) *Analytics의 보고서* 및 Analytics 툴 안내서의 [Analytics for Target(A4T)](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/panels/a4t-panel.html?lang=ko-KR) 패널을 *참조하십시오*. |

**개선 사항, 수정 사항 및 변경 사항**

* 방문자 지표가 &quot;UniqueVisitors&quot; 대신 활동의 정의에 저장되도록 한 문제를 해결했습니다. (TGT-37098)
* [!DNL Target] UI에서 세로 스크롤 막대가 [!UICONTROL Audiences] 페이지에서 올바르게 작동하지 않는 문제를 해결했습니다. (TGT-36968)
...
* Target UI에서 사용자가 단일 환경에서 콘텐츠를 교체한 후 다른 추천 오퍼 미리보기를 통해 편집된 콘텐츠를 표시하도록 하는 문제를 해결했습니다. (TGT-36053 및 TGT-36894)
* 일부 사용자가 추천 카탈로그에서 항목을 삭제할 수 없는 문제를 해결했습니다. (TGT-36455)
* 사용자가 다중 페이지 활동에 추천 기준을 저장할 수 없는 문제를 해결했습니다. (TGT-36249)
* 기준을 연속해서 편집할 때 동작 데이터 소스 라디오 버튼이 사라지는 문제를 해결했습니다. (TGT-36796)
* 추천알고리즘이 장시간 &quot;결과 가져오는 중&quot;을 표시하도록 만든 디스플레이 문제를 해결했습니다. (TGT-36550 및 TGT-36551)
...
* [!DNL Adobe Experience Cloud]에 로그인하면 새 헤더 탐색으로 이동합니다. 검은색 막대가 맨 위에 있는 이전 탐색과 매우 유사하지만 다음과 같은 개선 사항을 제공합니다.
   ...
   * 향상된 사용자 도움말: 검색 결과에는 [!DNL Target] 제품 설명서와 커뮤니티 포럼 및 더 많은 비디오 콘텐츠의 결과가 포함되어 있어 더 많은 콘텐츠에 쉽게 액세스하여 [!DNL Target]을 최대한 활용할 수 있습니다. 또한 문제를 보고하거나 아이디어를 공유하기 쉽도록 [!UICONTROL Help] 메뉴에 피드백 메커니즘을 추가했습니다.

   * NPS(Net Promoter Score) 피드백 기능이 향상되어 설문 조사 모달이 작업 흐름을 방해하지 않습니다.

   * [!DNL Target]에 대한 알림은 현재 헤더의 [!UICONTROL Notifications] 드롭다운에서 사용할 수 없습니다.
...
* 추천 오퍼는 A/B 테스트 및 경험 타겟팅 활동에서 DIV 외에 다른 DIV(예: P, UL, H1) 요소에 추가할 수 있습니다. (TGT-34333)
...
| 기능/향상 | 설명 |
| --- | --- |
| 시각적 경험 작성기(VEC) | **새 VEC 메뉴 옵션**: VEC에서 페이지 요소를 클릭하면 메뉴에 해당 요소 유형에 사용할 수 있는 옵션이 표시됩니다.<ul><li>이제 [!UICONTROL Styles > Background] 옵션을 사용하여 선택한 요소의 배경 이미지와 색상을 변경할 수 있습니다. (TGT-15001)</li></ul>다음 *시각적* 경험 [선택 사항](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)에서 스타일을확인합니다.<br>**클릭 추적 개선 사항**: VEC 및 SPA(단일 페이지 애플리케이션) VEC 내에서 클릭 추적을 구성하는 프로세스를 개선했습니다.<ul><li>클릭 추적에서 사용할 요소를 선택할 때 사용 가능한 모든 요소의 이름이 오른쪽의 수정 패널에 표시되므로 원하는 요소를 빠르고 쉽게 선택할 수 있습니다.</li><li>세 부분으로 구성된 안내 활동 워크플로의 [!UICONTROL Goals & Settings] 페이지에 클릭 추적에 대해 선택한 요소의 수를 나타내는 숫자가 표시됩니다. 이 숫자를 마우스로 가리키면 선택한 모든 요소의 이름이 표시됩니다. (TGT-33878)</li></ul>다음 [클릭 추적](/help/main/c-activities/r-success-metrics/click-tracking.md)을 참조하십시오. |
...
| 기능/향상 | 설명 |
| --- | --- |
| [!UICONTROL Visual Experience Composer] | [!UICONTROL Visual Experience Composer]&#x200B;(VEC)에는 작업을 보다 빠르고 효율적으로 수행할 수 있도록 다음과 같은 개선 사항이 포함되어 있습니다.<ul><li>이제 DOM 경로 기능은 클릭 추적을 설정할 때 사용할 수 있습니다.<br>자세한 내용은 [클릭 추적](/help/main/c-activities/r-success-metrics/click-tracking.md#considerations)을 참조하십시오.</li><li>스타일 패널을 사용하여 선택한 요소의 기존 스타일 값을 보거나 편집할 수 있습니다. 스타일을 추가할 수도 있습니다.<br>스타일 패널에 액세스하려면 VEC 내에서 페이지 요소를 클릭한 다음 [!UICONTROL Edit] > [!UICONTROL Styles]을(를) 클릭합니다.<br>스타일 패널이 VEC 오른쪽에 표시됩니다. 이 패널에는 선택한 요소를 편집하거나 선택한 요소에 추가할 수 있는 스타일 목록이 포함되어 있습니다. CSS(Cascading Style Sheet)를 사용하는 데 익숙하거나 개발자로부터 코드를 받은 경우 실시간 CSS 편집기를 사용하면 변경 사항을 보고 스타일을 추가할 수 있습니다.<br>자세한 내용은 [시각적 경험 작성기](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles) 선택 사항의 *스타일*&#x200B;을 참조하십시오.</li><li>이제 서식 있는 텍스트 편집기에서 중첩된 HTML5 요소를 지원합니다.<br>HTML5 사양에 따라 중첩할 태그를 새로 조합할 수 있습니다. 이전 버전의 서식 있는 텍스트 편집기에서는 HTML5 사양에 따라 허용되는 태그의 새로운 중첩을 지원하지 않았습니다. 따라서 VEC에서 선택한 중첩된 요소가 제대로 처리되지 않아 HTML이 원치 않게 변경되었습니다. (TGT-33618)<br>자세한 내용은 [시각적 경험 작성기](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#edit-text-html) 선택 사항의 *텍스트/HTML 편집*&#x200B;을 참조하십시오.</li> |

#### 개선 사항, 수정 및 변경 사항

* VEC를 사용하여 에셋을 삭제할 때 워크플로를 개선했습니다. 삭제된 자산은 이제 [!UICONTROL Offers library] 및 [!DNL Scene7]에서 제거됩니다(해당하는 경우). 삭제된 에셋은 더 이상 검색 결과에 표시되지 않습니다. (TGT-31981)
* 이제 에셋 폴더에 이미지가 있더라도(비어 있지 않은 폴더) 에셋 폴더를 삭제할 수 있습니다. (TGT-33265)

  이전에는 Target 이미지 오퍼 라이브러리([!UICONTROL Offers] > [!UICONTROL Image Offers])에서 비어 있지 않은 폴더를 삭제할 수 없었습니다. 그러면 &quot;폴더가 비어 있지 않습니다!&quot;가 표시됩니다. &quot;폴더가 비어 있지 않습니다.&quot; 알림이 표시됩니다.  이 기능을 사용하여 폴더 삭제를 수행하여 내부에 에셋 및 하위 폴더가 포함된 전체 폴더를 제거할 수 있는 기능을 추가합니다. 이 기능은 Adobe Experience Cloud Assets UI뿐만 아니라 Target UI에서 사용할 수 있습니다.
...
* 이제 컬렉션 선택기에서 [!UICONTROL Recommendations] 컬렉션을 선택한 후에는 [!UICONTROL Save] 단추를 클릭해야 합니다. 이 워크플로는 [!DNL Target] 내의 다른 워크플로와 일치합니다. (TGT-33205)
...
| ![ 및 ](/help/main/assets/premium.png)<br> 활동의 [!UICONTROL A/B Test]Premium 배지[!UICONTROL Experience Targeting]권장 사항 | 이제 [!UICONTROL A/B Test]&#x200B;([!UICONTROL Auto-Allocate] 및 [!UICONTROL Auto-Target] 포함)과 [!UICONTROL Experience Targeting]&#x200B;(XT) 활동 내에 권장 사항을 포함할 수 있습니다. 이에 따라 다음과 같이 완전히 새로운 기능을 사용할 수 있게 됩니다.<ul><li>동일한 활동에서 권장 사항 및 비권장 사항 콘텐츠를 테스트하고 타기팅할 수 있습니다.</li><li>권장 사항들의 순서를 포함하여 페이지에서의 권장 사항 배치를 쉽게 실험할 수 있습니다.</li><li>[!UICONTROL Auto-Allocate]을(를) 사용하여 트래픽을 가장 성과가 가장 좋은 권장 사항 경험에 자동으로 푸시합니다.</li><li>[!UICONTROL Auto-Target]을(를) 사용하여 방문자를 개별 프로필에 따라 맞춤 권장 사항 경험에 동적으로 지정할 수 있습니다.</li></ul>시작하려면 VEC를 사용하여 [!UICONTROL A/B Test] 또는 [!UICONTROL Experience Targeting] 활동을 만들고 [!UICONTROL Insert Before], [!UICONTROL Insert After] 또는 [!UICONTROL Replace With] 작업을 사용하여 권장 사항을 경험에 추가하십시오. (RECS-6166)<br>자세한 내용은 [오퍼로서의 추천](/help/main/c-recommendations/recommendations-as-an-offer.md)를 참조하십시오. |
...
11월 12일의 [!DNL Target] Standard/Premium 릴리스에는 백엔드 개선 사항, 수정 사항 및 변경 사항이 포함되어 있습니다. [!UICONTROL Personalization Insights] 보고서는 11월 14일에 사용할 수 있습니다.
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>2개의 전문 보고서는 <span class="wintitle"> Automated Personalization (AP)</span> 및 <span class="wintitle"> 자동 타겟 (AT)</span> 활동 사용자가 사용할 수 있습니다. </p> <p> 
     ...
     </ul> </p> <p><a href="/help/main/c-reports/c-personalization-insights-reports/personalization-insights-reports.md#concept_A897070E1EDC403EB84CFB7A6ECAD767" format="dita" scope="local">개의 Personalization 통찰력 보고서</a>를 참조하십시오. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* Adobe Audience Manager (AAM) 에서 삭제된 Target 활동에서 참조된 대상을 처리하는 방법을 개선했습니다. (TGT-23338)
   ...
   * AAM에서 대상이 삭제된 경우 [!UICONTROL Audience] 목록과 대상 선택기에 모두 경고 아이콘이 표시됩니다. 또한 UI의 도구 설명은 또한 대상이 AAM에서 삭제되었음을 나타냅니다.
...
* [!UICONTROL Administration] 페이지에서 Adobe Analytics을 보고 소스로 선택한 경우 특정 상황에서 사용자가 활동을 만들 수 없는 문제를 해결했습니다. 사용자에게 보고서 세트를 선택하는 옵션을 제공하지 않아도 &quot;보고서 세트를 선택하십시오&quot;라는 메시지가 표시됩니다. (TGT-31968)
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_9C39ACD865CE4167BDBAA093EDFD3B68"> 
      <li id="li_19710BA5965E4F858B128E1E9FF89471"> <p>이제 제외 그룹의 동일한 위치에서 여러 개의 오퍼를 사용할 수 있습니다. 제외가 많은 경우(1000개) 자동화된 개인화(AP) 활동을 작성하는 동안 콘텐츠 관리 대화 상자 및 미리보기 페이지가 더 빨리 로드되는 것을 관찰할 수 있습니다. (TGT-31329) <a href="/help/main/c-activities/t-automated-personalization/managing-exclusions.md#topic_30B4E4F89C914EB2B20B038C0299ED2E" format="dita" scope="local"> 제외 관리 </a>을(를) 참조하십시오. </p> </li> 
      <li id="li_542C66E2998541BC87D0A96F4672C665"> <p>이제 보고서 그룹별로 오퍼를 필터링 할 수 있습니다. (TGT-31643) <a href="/help/main/c-activities/t-automated-personalization/create-ap-activity.md#task_8AAF837796D74CF893CA2F88BA1491C9" format="dita" scope="local"> Automated Personalization 활동 만들기 </a>을(를) 참조하십시오. </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Automated Personalization (AP) 및 자동 타겟 (AT) 활동에 대한 전문 보고서에 액세스하십시오. </p> <p> 
     ...
     </ul> </p> <p> <p>참고: 이 기능은 곧 제공될 예정입니다. 이 기능을 사용할 준비가 되면 정확한 날짜를 공지하겠습니다. </p> </p> <p><a href="/help/main/c-reports/c-personalization-insights-reports/personalization-insights-reports.md#concept_A897070E1EDC403EB84CFB7A6ECAD767" format="dita" scope="local"> Personalization 통찰력 보고서 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>시각적 경험 작성기(VEC) </p> </td> 
   <td colname="col2"> <p> 
...
<table id="table_7E3513EABA4948DC92EADCCE0234A9FF"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>한 개의 프로필 속성을 정적 숫자가 아닌 다른 프로필 속성과 비교합니다. </p> <p><a href="/help/main/c-target/c-audiences/creating-a-profile-attribute-comparison-audience.md#concept_4C2124B79A5B4556A6C1D10C0F5E40A0" format="dita" scope="local"> 프로필 특성 비교 대상 만들기 </a>를 참조하십시오. </p> <p> (TGT-28406) </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Adobe Target 경험 템플릿은 일부 공통 마케터 사용 사례를 실행하기 위해 Target에서 사용할 구성 가능한 입력을 사용하는 사전 코딩된 오퍼 샘플입니다. 이러한 경험 템플릿은 시각적 경험 작성기 또는 양식 기반 경험 작성기를 통해 Adobe Target에서 몇 가지 공통적인 외부 사용 사례를 실행할 시작점으로 개발자 및 마케터에게 무료로 제공됩니다. 웹 페이지 또는 플랫폼 아키텍처와 제대로 통합하는 데 사용자 지정이 필요할 수 있습니다. </p> <p><a href="/help/main/c-experiences/c-visual-experience-composer/c-vec-code-editor/experience-templates.md#concept_109BBD7EABC04DD39E6B7B1687786652" format="dita" scope="local">개의 경험 템플릿 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>Target 기본 사항 웨비나 시리즈 </p> </td> 
...
* 이제 권장 사항 CSV 다운로드의 백업 행에는 &#42;(단일 별표) 대신 선행 &quot;&#42;&quot;(큰따옴표가 별표를 묶음)이(가) 있습니다.
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> 
     ...
     </ul> </p> <p>자세한 내용은 <a href="/help/main/c-reports/c-report-settings/report-settings.md#concept_3A80D5A394EC4B639DC715E06085BDB0" format="dita" scope="local"> 보고서 설정 </a>의 Target 사전 설정 섹션을 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>프로필 스크립트 </p> </td> 
   <td colname="col2"> <p> 
     <ul id="ul_F382C8E7708846A08676E1534BC92878"> 
      <li id="li_70E89504525C4119B588C230DCE772E8"> <p>오퍼 정보 카드와 유사한 프로필 스크립트 정보 팝업 카드를 볼 수 있습니다. 이러한 프로필 스크립트 정보 카드를 사용하면 선택한 프로필 스크립트를 참조하는 활동 목록과 함께 다른 유용한 메타데이터를 볼 수 있습니다. (TGT-28253) </p> <p>자세한 내용은 <a href="/help/main/c-target/c-visitor-profile/profile-parameters.md#concept_8C07AEAB0A144FECA8B4FEB091AED4D2" format="dita" scope="local"> 프로필 스크립트 특성 </a>의 프로필 스크립트 정보 카드 보기 섹션을 참조하십시오. </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>대상자 </p> </td> 
   <td colname="col2"> <p> 
     ...
     </ul> </p> <p>자세한 내용은 <a href="/help/main/c-target/c-audiences/c-target-rules/custom-parameters.md#concept_C4C6E00D7C5A4BE9B72D471DB2E3027B" format="dita" scope="local"> 사용자 지정 매개 변수 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>권장 사항 </p> </td> 
   <td colname="col2"> <p> 
...
<table id="table_AE38682151A948AEA21E35A353F18D76"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p>대상자 </p> </td> 
   <td colname="col2"> <p>이제 대상 라이브러리의 대상 정의 팝업 카드를 보는 동안 해당 대상을 참조하는 다른 활동(해당되는 경우)을 볼 수 있습니다. 이 방법을 사용하면 대상을 편집하는 동안 활동에 실수로 영향을 미치는 경우를 방지할 수 있습니다. </p> <p>이전에는 활동에서 참조한 대상을 삭제하려고 하면 대상을 참조하는 최대 10개의 활동이 있어서 대상을 삭제할 수 없다는 경고가 표시되었습니다. </p> <p>자세한 내용은 대상 정보 <a href="/help/main/c-target/c-audiences/audiences.md#concept_65BE870D290E412D8BBF557EEA67C271" format="dita" scope="local">에 대해 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>보고서 </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_63613AD2D744442AA12CD23F4DAC75B4"> 
      ...
      <li id="li_14FCFDD0A0E84B47AF1488DB4DDF197B">이제 알고리즘 포함 규칙을 만드는 동안 "값이 있음" 및 "값이 없음" 연산자를 사용할 수 있습니다. <a href="/help/main/c-recommendations/c-algorithms/use-dynamic-and-static-inclusion-rules.md#concept_4CB5C0FA705D4E449BD0B37B3D987F9F" format="dita" scope="local"> 동적 및 정적 포함 규칙 사용 </a>을(를) 참조하십시오. (TGT-24110) </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
...
* 테이블에서의 스크롤 작업과 필터 드롭다운 목록의 배치 기능이 향상되도록 보고서 UI가 개선되었습니다. (TGT-23713 및 TGT-26819)
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>양식 기반 경험 작성기 및 HTML 오퍼 편집기에서는 이제 VEC(시각적 경험 작성기)가 사용자 지정 코드에서 사용하는 것과 동일한 코드 편집기를 사용합니다. (TGT-25808) </p> <p>이 개선 사항은 양식 기반 시각적 경험 작성기에서 코드 편집기를 사용할 때와 HTML 오퍼를 만들 때 다음과 같은 이점을 제공합니다. </p> <p> 
     ...
     </ul> </p> <p>자세한 내용은 <a href="/help/main/c-experiences/c-visual-experience-composer/c-vec-code-editor/vec-code-editor.md#concept_B3A6E9EE3A60406DB640E205EA1745B5" format="dita" scope="local"> 코드 편집기 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
...
* 활동 또는 오퍼를 만들거나 편집하는 동안 사용자의 [!DNL Target] 세션 시간이 초과될 경우의 워크플로가 개선되었습니다. 사용자가 [!UICONTROL Save]을(를) 클릭하면 세션 만료 메시지가 표시되지만, 다시 로그인하면 성공적인 로그인을 사용자에게 알리는 대화 상자가 표시되며, UI는 데이터 손실 없이 [!DNL Target]의 동일한 페이지에 그대로 유지됩니다.
...
* [!DNL Target]&#x200B;(A4T)을(를) 보고 소스로 사용하는 활동에서 리디렉션 오퍼를 지원하도록 [!UICONTROL Analytics for Target] 사용자 인터페이스가 업데이트되었습니다. 이 기능을 사용하려면 곧 출시될 예정인 [!DNL at.js] 0.9.6이 필요합니다.
* [!DNL Target] 사용자 인터페이스가 다음을 비롯한 일부 위치에서 업데이트되었습니다.

   * 보고서 및 활동에서 이제 [!UICONTROL Edit] 아이콘([!UICONTROL Share to Feed]icon_more_options 이미지[!UICONTROL View Experience URLs])을 클릭하여 일부 옵션([!UICONTROL More Options], ![, ](assets/icon_more_options.png) 등)에 액세스할 수 있습니다.

     .
   * [!UICONTROL Offers] 라이브러리에서 오퍼가 카드가 아닌 목록에 표시됩니다. [!UICONTROL Offers] 라이브러리 UI 전체에서 다른 부수적 UI가 변경되었습니다.

* [!UICONTROL Activity] 및 [!UICONTROL Audience] 목록의 성능이 크게 향상되었습니다. 또한 검색 결과 로드 시간도 크게 단축되었습니다.
* 이제 [!UICONTROL Offer Level Report] 보고서의 [!UICONTROL Automated Personalization]에서 &quot;보기 수&quot;는 &quot;방문 수&quot;입니다.
* [!DNL Target]은(는) 이제 [!UICONTROL Automated Personalization] 활동에 대한 환경(호스트 그룹) 전환을 지원합니다.
* [!UICONTROL Automated Personalization] 활동이 이제 호스트 그룹을 지원합니다.
...
* 이제 모든 활동 이름이 목록에 로드되기 전에 [!UICONTROL Create Activity] 페이지의 [!UICONTROL Activities] 단추가 활성화됩니다. 이러한 개선을 통해 특히, 계정에 구성된 활동이 많은 경우 새 활동을 더 빠르게 만들 수 있습니다. (TGT-21470)
* 프록시를 통해 액세스하는 HTTPS 실행 웹 사이트의 로드 시간을 단축하도록 EEC(고급 경험 작성기)가 개선되었습니다. Target은 더 이상 프록시를 통해 정적 리소스를 가져오지 않습니다. (TGT-21793)
* [!UICONTROL Goals & Settings] 페이지의 성능, 특히 활동에 대해 많은 지표가 정의된 경우 로드 시간이 개선되었습니다. (TGT-21654)
* 활동 페이지에 at.js(버전 0.9.1 이상)가 로드된 경우 추적 서버가 필요하지 않음을 사용자에게 알리는 [!UICONTROL Goals & Settings]&#x200B;(A4T) 보고를 사용하여 모든 활동의 [!UICONTROL Analytics for Target] 페이지에 도구 설명이 추가되었습니다. (TGT-22607)
* 이제 사용자가 전체 지표 이름을 보기 위해 각 지표를 확장할 필요 없이 지표 이름이 [!UICONTROL Goals & Settings] 페이지에 표시됩니다. 이를 통해 사용자는 지표를 보다 빠르고 효율적으로 편집할 수 있습니다. (TGT-21276)
* 이제 다른 기준과 마찬가지로 [!DNL Recommendations] 포함 규칙을 사용자 지정 기준(CSV를 통해 업로드됨)에 적용할 수 있습니다. (TGT-21896)
* 특히 폴더를 만들거나 관리하고 오퍼를 만들 때 [!UICONTROL Offers] 페이지의 사용자 인터페이스가 개선되고 보다 편리해졌습니다. (TGT-22509 및 TGT-22187)
* 숨길 항목을 선택할 때 [!UICONTROL Visual Experience Composer]&#x200B;(VEC)의 사용자 경험이 개선되었습니다.
(TGT-22224)
* [!UICONTROL Form-Based Experience Composer]을(를) 사용하여 활동을 만들 때 사용자 경험이 개선되었습니다. mbox 위치를 선택할 때 [!UICONTROL Next]을(를) 클릭하면 유효성 검사 테두리가 강조 표시된 상태로 유지됩니다. (TGT-22221)
...
* 예외적인 주문이 다운로드된 [!UICONTROL Order Details]CSV 보고서에 올바르게 표시되지 않은 문제가 수정되었습니다. (TGT-22500)
* 다운로드한 [!UICONTROL Campaign Audit] CSV 보고서에 올바른 주문 날짜가 표시되었더라도 해당 보고서에 잘못된 주문 시간이 표시되는 문제가 해결되었습니다. (TNT-26469)
* 다중 페이지 활동에서 [!UICONTROL Disable JavaScript] 옵션이 제대로 작동하지 않는 문제를 해결했습니다. (TGT-15130)
* 자동으로 만든 글로벌 mbox(`target-global-mbox`) 이외의 mbox와 함께 양식 기반 경험 작성기를 사용한 다음, 참여 지표를 성공 지표로 선택하는 경우, 지표는 활동에 해당 mbox가 사용된 페이지에서만 증분됩니다. 예를 들어 mbox가 `homepage_mbox`이면 [!UICONTROL Pages Per Visit] 지표는 해당 방문 동안 `homepage_mbox`에 대한 히트 수입니다.
  ...
  <tr> 
   ...
   <td colname="col2"> <p>이제 대상에 대해 다음 개선 사항을 사용할 수 있습니다. </p> <p> 
     <ul id="ul_F1D1F97266134D4ABE627CF2DCE2C6D4"> 
      <li id="li_99A611FCC1254D229D79B8FD075B952A"> <p> 이제 대상자 타기팅 모바일 디바이스를 만들 때 드롭다운 목록에서 <span class="wintitle"> 디바이스 마케팅 이름 </span>을 기본 제공 옵션으로 사용할 수 있습니다. </p> <p>이 변경을 통해 해당 디바이스 모델 번호를 검색하지 않고도 디바이스 모델 이름을 쉽게 선택할 수 있습니다. 예를 들어 Galaxy S7의 마케팅 디바이스 이름은 "Samsung Galaxy S7 Edge"이지만 디바이스 모델은 "SM-G9350"입니다. (TGT-18393) </p> <p>자세한 내용은 <a href="/help/main/c-target/c-audiences/c-target-rules/mobile.md#concept_2A794199DC1A4D349FFFBC7DCF1FEB89" format="dita" scope="local"> 모바일 </a>을 참조하십시오. </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>권장 사항 </p> </td> 
...
<table id="table_AE048CB9EA1C4C7BBC2E9D90D26F7395"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>계정 수준(계정에서 만든 모든 활동에 적용) 또는 개별 활동 수준에서 <span class="wintitle"> 고급 경험 작성기 </span>를 활성화/비활성화합니다. </p> <p>이전에는 계정 수준에서만 고급 경험 작성기를 활성화/비활성화할 수 있었습니다. </p> <p>자세한 내용은 <a href="/help/main/c-experiences/experiences.md#concept_A2E10F6AFB3D4AEAB6951EE14688848D" format="dita" scope="local">개의 경험 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_E2E4BE72BD79413A821C6A6D1A3AB0F8"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>코드 편집기 UI가 좀 더 간편하고 사용하기 쉽게 업데이트되었습니다. </p> <p>자세한 내용은 <a href="/help/main/c-experiences/c-visual-experience-composer/c-vec-code-editor/vec-code-editor.md#concept_B3A6E9EE3A60406DB640E205EA1745B5" format="dita" scope="local"> 코드 편집기 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* [!UICONTROL Category Affinity] 기능에 대한 일부 UI 텍스트가 영어로만 표시됩니다. 다른 언어 텍스트는 9월 [!DNL Target] 릴리스에서 지원될 예정입니다.
...
<table id="table_EBA34BD2F5C745DD9EC5231AD79F6C00"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>이제 기존 대상을 복사한 후 편집하여 유사한 대상을 만들 수 있습니다. </p> <p>자세한 내용은 <a href="/help/main/c-target/c-audiences/create-audience.md#task_E18BD77A9A8F4ED0AC50569F94556558" format="dita" scope="local"> 대상자 만들기 </a>을(를) 참조하십시오. </p> <p>기존 프로필 스크립트를 복사할 수도 있습니다. </p> <p>자세한 내용은 <a href="/help/main/c-target/c-visitor-profile/profile-parameters.md#concept_8C07AEAB0A144FECA8B4FEB091AED4D2" format="dita" scope="local"> 프로필 스크립트 특성 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 클래스를 사용하여 요소 선택기 결정 </td> 
   <td colname="col2"> <p>이제 Automated Personalization 및 다변량 테스트 활동에서 클래스 또는 ID를 기준으로 요소 선택기를 선택할 수 있습니다. 이전 버전에서는 A/B 테스트 활동에만 이 옵션을 사용할 수 있었습니다. </p> <p>자세한 내용은 시각적 경험 작성기 <a href="/help/main/c-experiences/c-visual-experience-composer/vec-selectors.md#concept_4EB7663E255F439B8D24079D23479337" format="dita" scope="local">에서 사용되는 </a> 요소 선택기를 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
...
* 리디렉션이 적용된 경험에 대한 경험 미리보기 URL에 문제가 있습니다. 이 문제를 해결하려면 경험 작성기에서 **[!UICONTROL Configure]**&#x200B;을(를) 클릭하고 **[!UICONTROL Multiple Audiences]**&#x200B;을(를) 선택한 후 유일한 대상으로 **[!UICONTROL All visitors]**&#x200B;을(를) 추가합니다. 계속해서 활동을 저장합니다. 활동의 전달은 변경되지 않지만 미리보기가 제대로 작동합니다. 이 문제는 Adobe Target의 7월 릴리스에서 수정될 예정입니다.
...
<table id="table_162CC5A0DB324B38A8A4252A18976686"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1" class="premium"> 추천: <span class="codeph"> mboxTrace </span>를 통한 텍스트 영역의 새 디버깅 기능 </td> 
  ...
  <tr> 
   <td colname="col1"> 새로운 지침 비디오 </td> 
   <td colname="col2"> 도움말에 지침 비디오가 추가되었습니다. 현재 <a href="/help/main/c-experiences/experiences.md#concept_A2E10F6AFB3D4AEAB6951EE14688848D" format="dita" scope="local"> 시각적 경험 작성기 및 양식 기반 경험 작성기 </a>에 대한 비디오를 볼 수 있습니다. 향후 몇 주 이내에 더 많은 비디오가 추가될 예정입니다. </td> 
  </tr> 
 </tbody> 
</table>
...
* Chrome 버전 48 업데이트는 Visual Experience Composer가 Chrome에서 제대로 작동할 수 없게 하는 문제를 야기합니다. Google을 사용하면 문제가 해결됩니다. 자세한 내용은 [https://code.google.com/p/chromium/issues/detail?id=582603](https://code.google.com/p/chromium/issues/detail?id=582603)을 참조하십시오. 이 문제를 해결하려면 다음을 수행하십시오.
   ...
   * **[!UICONTROL Administration]** > **[!UICONTROL Visual Experience Composer]** 탭 내에서 구성할 수 있는 고급 경험 작성기를 사용하도록 설정합니다.
...
<table id="table_8525ECC9B6D0435ABEF8C27F747B7A0C"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>대상을 구성할 때 중첩된 AND/OR 논리를 사용합니다. </p> <p><a href="/help/main/c-target/c-audiences/create-audience.md#task_E18BD77A9A8F4ED0AC50569F94556558" format="dita" scope="local"> 대상자 만들기 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
...
* 비 HTML 추천 템플릿을 양식 기반 워크플로에서 사용할 경우 `<div>` 요소에 래핑하는 문제를 수정했습니다.
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>다음과 같은 내장 대상 옵션이 새로 추가되었습니다. </p> <p> 
     ...
     </ul> </p> <p><a href="/help/main/c-target/c-audiences/c-target-rules/browser.md#concept_221D8EEF53CC45AEACEB17CF336A3658" format="dita" scope="local"> 브라우저 옵션 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_907A952F54084C2A9C61F10E2B7A7BFF"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 조직의 목표가 전환 지표인 경우 이제 오퍼 세부 사항 보고서에 상승도 및 증분 전환에서 주요 예측 변수가 미치는 영향이 표시됩니다. 이 보고서는 이전에는 매출만을 기준으로 했으므로 이 기능을 통해 매출 데이터가 없는 활동도 여전히 관련있고 실용적인 통찰력을 생성할 수 있습니다. </p> <p><a href="/help/main/c-reports/personalization-reports/reports-ap.md" format="dita" scope="local">개의 Automated Personalization 보고서 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_46FF5AF77D824ADC941B1E472234F05C"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>변경 로그는 활동의 변경 사항을 나열합니다. 작업 및 사용자는 각 변경에 대한 타임스탬프와 함께 나열됩니다. </p> <p><a href="/help/main/c-activities/change-log.md#task_D6F224E8CE8346699187D21CD9A2B4AB" format="dita" scope="local"> 작업 변경 로그 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 세분화된 옵션을 사용하여 성공 지표를 카운트하는 방법을 결정할 수 있습니다. 옵션에는 노출당 지표 카운트 또는 방문자당 한 번 카운트, 활동에 사용자를 유지할지 또는 제거할지를 선택하는 옵션이 포함됩니다. 이러한 옵션은 Target Classic에서 사용할 수 있는 성공 지표에 대한 "고급 옵션"과 같습니다. </p> <p><a href="/help/main/c-activities/r-success-metrics/success-metrics.md#reference_D011575C85DA48E989A244593D9B9924" format="dita" scope="local"> 성공 지표 </a>을(를) 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> 이제 사이트 방문자가 세션 중간에 로그인하고 3rdpartyId를 얻으면 3rdPartyId에 연결되어 있는 이전에 로드된 모든 프로필 속성을 즉시 사용할 수 있게 됩니다. <a href="/help/main/c-target/c-audiences/c-target-rules/visitor-profile.md#concept_E972690B9A4C4372A34229FA37EDA38E" format="dita" scope="local"> 방문자 프로필 </a>을(를) 참조하십시오. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_E0D6A9C12B514DE3B3EA753BB4D56662"> 
      <li id="li_2A728C8938834162A0C0C1C926AC5DD9"> 부분 템플릿 렌더링 <p><a href="/help/main/c-recommendations/c-algorithms/create-new-algorithm.md#content" format="dita" scope="local"> 콘텐츠 설정 </a>을(를) 참조하십시오. </p> </li> 
...
| 기능/향상 | 설명 |
|---|---|
| JavaScript 삽입 및 편집 허용 | 작업 메뉴에서 **[!UICONTROL Edit HTML]**&#x200B;을(를) 선택하면 경험 편집기에서 사용자 지정 JavaScript을 편집하고 주입할 수 있는 기능을 추가했습니다. |
...
* 사용자가 **[!UICONTROL Select Link]** 페이지에서 [!UICONTROL Goals and Settings]을(를) 클릭할 때 선택한 링크가 선택된 링크로 표시되지 않는 문제를 해결했습니다.

* [!UICONTROL Activity List] 페이지에서 새 활동이 활성화된 후 [!UICONTROL Overview]에 표시되지 않던 오류를 수정했습니다.
...
* 삭제된 일부 공유 대상자가 [!UICONTROL Audiences] 목록에 계속 표시되는 문제를 해결했습니다.
* Internet Explorer 10에서 예기치 않은 [!UICONTROL Save] 대화 상자가 나타나는 오류를 해결했습니다.
...
* 이미지가 CSS에서 참조된 경우 교환할 수 없습니다.
* Visual Experience Composer를 사용하여 다시 정렬한 요소에는 클릭 추적이 작동하지 않습니다. 이 버그가 수정될 때까지 다시 정렬된 요소에 대해서는 클릭 추적을 설정하지 마십시오.
* 사용자는 mbox에 래핑된 콘텐츠에 대해 **[!UICONTROL Remove]** 작업을 선택할 수 없습니다.
