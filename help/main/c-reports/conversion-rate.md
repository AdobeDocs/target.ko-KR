...
주문( )을 삽입한 경우 각 경험에 대해 AOV, RPV 및 판매 데이터가 표시됩니다.`orderConfirmPage`) mbox를 전환 mbox로 선택했습니다.
...
다음 *신뢰 구간* 는 주어진 신뢰 수준에서 지표의 실제 값을 찾을 수 있는 예상 범위입니다. Target은 항상 95% 신뢰 구간을 표시합니다. 신뢰 구간은 전환율 열에 밝은 회색 +/- 백분율로 표시됩니다. 아래 예에서 경험 B의 상승도에 대한 신뢰 구간은 + 또는 - 15.65%입니다.

![conversion_rate 이미지](assets/conversion_rate.png)

**예:** 경험의 관찰된 RPV는 $10이며, 95%입니다. **신뢰 구간** 5달러에서 15달러입니다 알 수 없지만, 실제 RPV는 $12입니다. 그런 다음 이 테스트를 여러 번 실행한 경우 계산하는 신뢰 구간의 95%에 _true_ RPV 값 $12.
...
이러한 계산된 지표를 계산하려면 Target [완료 신뢰도 계산기](/help/main/assets/complete_confidence_calculator.xlsx) 활동의 값을 입력하거나 검토할 Excel 파일 [A/Bn 테스트의 통계 계산](/help/main/c-reports/statistical-methodology/statistical-calculations.md).
...
## 이유는 다음과 같습니다 [!DNL Target] welch의 t-test를 사용하는 것이 좋습니까? {#t-test}

A/B 테스트는 제어 변형(경험이라고도 함)의 일부 비즈니스 지표의 평균 값을 하나 이상의 대체 경험에서 동일한 지표의 평균 값과 비교하기 위한 실험입니다.

[!DNL Target] 다음을 권장합니다. [웰치 티테스트](https://en.wikipedia.org/wiki/Welch%27s_t-test), 이는 z 테스트와 같은 대안보다 적은 가정을 필요로 하며 제어 경험과 대체 경험 간에 (정량적) 비즈니스 지표의 쌍별 비교를 수행하는 데 적합한 통계 테스트입니다.
...
비즈니스 지표 자체는 일부 임의 분포에 따라 분포될 수 있지만 이 지표의 평균 분포(각 변형 내)는 를 통해 정규 분포로 수렴해야 합니다. [중심 극한 정리](https://en.wikipedia.org/wiki/Central_limit_theorem). 평균의 이 샘플링 분포가 얼마나 빨리 정상으로 수렴되는지에 대한 보장은 없지만 온라인 테스트의 방문자 규모를 고려할 때 일반적으로 이 조건이 달성됩니다.

평균의 이러한 정규성이 주어지면, 데이터의 추정치(수단 차이의 표준 오차)를 기반으로 한 스케일링 항에 대한 정규적으로 분포된 값(비즈니스 지표의 수단 차이)의 비율이기 때문에 사용되는 테스트 통계는 t-분포를 따르는 것으로 표시될 수 있다. 다음 **-테스트** 검정 통계량이 t-분포를 따른다면 은 적절한 가설 검정입니다.

### 다른 검사가 사용되지 않는 이유

A **-테스트** 는 일반적인 A/B 테스트 시나리오에서 테스트 통계의 분모가 알려진 분산에서 파생된 것이 아니라 데이터에서 추정해야 하므로 기술적으로 부적절합니다. 그러나 충분한 샘플 크기가 큰 경우, z-테스트 및 t-테스트는 동일하다.

**카이 제곱 검정** 두 변형 간에 질적 관계가 있는지(즉, 변형 간에 차이가 없다는 귀무 가설) 여부를 결정하는 데 적합하므로 사용되지 않습니다. T-테스트가 의 시나리오에 더 적합합니다. _양적으로_ 지표 비교.

다음 **맨휘트니 검사** 는 비모수적 테스트이며, 평균 비즈니스 지표(각 변형에 대한)의 샘플링 분포가 정상적으로 분포되지 않은 경우에 적합합니다. 그러나 앞에서 논의한 바와 같이, 온라인 테스트에 수반되는 트래픽의 크기를 고려할 때, 중심 한계 정리가 일반적으로 적용되므로 t-테스트가 안전하게 적용될 수 있다.

다음과 같은 보다 복잡한 방법 **아노바** (t-테스트를 둘 이상의 변형으로 일반화) 테스트에 두 개 이상의 경험(&quot;A/Bn 테스트&quot;)이 있는 경우 적용할 수 있습니다. 그러나 ANOVA는 일반적인 A/Bn 테스트에서 더 관심이 있는 반면 &quot;모든 변형의 평균이 동일한지 여부&quot;에 대한 질문에 답합니다 _특정 변형_ 최고지 위치 [!DNL Target]따라서 여러 비교를 위해 각 변형을 Bonferroni 수정과 비교하는 정기적인 t-테스트를 제어에 적용합니다.