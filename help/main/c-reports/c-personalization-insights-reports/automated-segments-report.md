...
1. 원하는 날짜 범위를 선택합니다.
   ...
   [!UICONTROL 요약] 보고서(성과 보고)와 달리 [!UICONTROL 자동화된 세그먼트]를 포함한 [!UICONTROL Personalization 인사이트]는 고정된 날짜 범위인 15일, 30일 및 60일에 대해서만 사용할 수 있습니다. 이러한 고정 날짜 범위를 사용하면 [!UICONTROL 개인화 인사이트]에서 사용자 활동의 단기 패턴에서 인사이트를 파생할 가능성을 줄이기 위해 충분히 큰 데이터 범위를 사용할 수 있습니다. 날짜 범위에 대해 가능한 두 가지 의사 결정은 &quot;종료 날짜&quot; 및 &quot;지속 시간&quot;입니다. &quot;시작&quot;이 회색으로 표시됩니다. 시작 날짜는 선택한 종료 날짜 및 지속 시간에 따라 자동으로 변경됩니다.

   [!UICONTROL 날짜 범위 사전 설정] 드롭다운 목록에서 사용 가능한 고정 날짜 범위에 액세스할 수 있습니다.

1. [!UICONTROL 자동화된 세그먼트] 보고서 데이터를 검토하십시오.

1. (선택 사항) Excel 및 기타 도구에서 분석할 [CSV 형식으로 보고서를 다운로드](/help/main/c-reports/c-report-settings/report-settings.md#section_77E65C50BAAF4AB79242DB3A8778ADEF)하려면 **[!UICONTROL 다운로드]**( ![다운로드 아이콘](/help/main/assets/icons/Download.svg)) 아이콘을 클릭합니다.
...
* 활동에는 수입 최적화 목표가 있습니다. 현재 [!UICONTROL Personalization Insights]는 전환 최적화 목표 활동에만 사용할 수 있습니다. Adobe은 향후 릴리스에서 수익 최적화 목표 활동에 대한 지원을 추가할 예정입니다.
...
AP 및 AT 활동은 오퍼마다 한 개의 모델(AP의 경우)과 경험마다 한 개의 모델(AT의 경우)을 빌드합니다. 이러한 활동은 개인화된 트래픽을 제공하고 빌드된 최소 두 개의 모델로 [!UICONTROL 개인화 인사이트]를 작성합니다. [!UICONTROL Personalization Insights]에서 일부 오퍼/경험이 표시되지 않으면 특정 오퍼/경험을 위해 만들어진 모델이 없을 수 있습니다. 활동의 [!UICONTROL 요약] 보고서를 확인하고 해당 오퍼/경험 옆에 시계 아이콘이 있는지 확인할 수 있습니다. 이 아이콘은 모델이 해당 오퍼/경험용으로 아직 빌드되지 않았음을 나타냅니다.
...
**[!UICONTROL 자동화된 세그먼트] 및 [!UICONTROL 중요 특성] 보고서의 정보가 CSV 다운로드와 같습니까?**
...
**사용자 지정 날짜 범위에 대한 [!UICONTROL Personalization 인사이트]를 볼 수 있습니까?**

Personalization Insights 보고([!UICONTROL 자동화된 세그먼트] 및 [!UICONTROL 중요 속성] 모두)는 고정 날짜 범위인 15일, 30일 및 60일에 대해서만 사용할 수 있습니다. 이러한 고정 날짜 범위를 사용하면 [!UICONTROL 개인화 인사이트]에서 사용자 활동의 단기 패턴에서 인사이트를 파생할 가능성을 줄이기 위해 충분히 큰 데이터 범위를 사용할 수 있습니다. 종료 날짜에 대해 이러한 지속 시간을 선택할 수 있습니다(활동에 지속 시간을 충족할 충분한 데이터가 있는 경우).
...
[!UICONTROL Personalization 인사이트] 보고서에는 Target의 개인화 모델에서 선택한 콘텐츠를 본 방문자만 포함됩니다(즉, 전체 승자 모델에서 제공하는 트래픽 또는 제어 트래픽은 고려하지 않음). 이 트래픽 유형을 &quot;개인화된&quot; 트래픽이라고 합니다. AP/AT의 요약 성능 보고서에는 제어와 &quot;대상&quot; 트래픽이 포함됩니다. 타깃팅된 트래픽에는 개인화된 트래픽뿐만 아니라, 전체 승자 모델을 사용하여 제공된 트래픽과 학습을 계속하는 데 사용되는 임의로 제공된 일부 트래픽이 포함되어 있습니다.
...
현재 [!UICONTROL 개인화 인사이트]는 변환 최적화 목표 활동에만 사용할 수 있습니다. Adobe은 향후 릴리스에서 수익 최적화 목표 활동에 대한 지원을 추가할 예정입니다.
