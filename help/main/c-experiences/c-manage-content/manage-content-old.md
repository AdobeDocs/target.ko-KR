...
1. (선택 사항) [!UICONTROL 코드 오퍼] 탭에서 원하는 오퍼 또는 폴더로 마우스를 가져간 후 원하는 아이콘을 클릭하여 추가 작업을 수행합니다.
   ...
   * 편집
   * 복사
   * 이동(예: 하나 이상의 항목을 폴더로 이동하려면 원하는 항목에 대한 **[!UICONTROL 이동]** 아이콘을 클릭하고 원하는 폴더를 클릭한 다음 **[!UICONTROL 삭제]**&#x200B;를 클릭합니다.)
   * 삭제
...
1. (선택 사항) [!UICONTROL 이미지 오퍼] 탭에서 원하는 이미지 오퍼 또는 폴더로 마우스를 가져간 후 원하는 아이콘을 클릭하여 추가 작업을 수행합니다.
...
오퍼를 열지 않고 [!UICONTROL 오퍼] 라이브러리의 팝업 카드에서 오퍼 정의 세부 사항을 볼 수 있습니다.
...
[!UICONTROL 전체 세부 정보 보기] 링크를 클릭하여 오퍼 콘텐츠와 코드 오퍼를 참조하는 활동을 봅니다. 오퍼를 편집할 때 이 방법으로 다른 활동에 영향을 주는 일을 피할 수 있습니다. 정보에는 [!UICONTROL 라이브 활동] 및 [!UICONTROL 비활성 활동]이 포함됩니다.
