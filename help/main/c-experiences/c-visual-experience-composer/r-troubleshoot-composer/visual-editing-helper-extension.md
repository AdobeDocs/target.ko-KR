...
[!DNL Adobe Experience Cloud]에 대한 [!UICONTROL Visual Editing Helper] [!DNL Google Chrome] 브라우저 확장을 사용하면 웹 사이트를 [!UICONTROL Adobe Target] [!UICONTROL Visual Experience Composer]&#x200B;(VEC)에 안정적으로 로드하여 웹 경험을 빠르게 작성 및 QA할 수 있습니다.
...
1. 확장 기능을 사용하려면 VEC 또는 QA 모드에서 Chrome 브라우저의 도구 모음에서 [!UICONTROL Visual Editing Helper] 브라우저 확장 기능 아이콘(![시각적 편집 확장 기능 아이콘](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/assets/visual-editing-helper.png))을 클릭합니다.
   ...
   작성 기능을 향상시키기 위해 [!UICONTROL Visual Editing Helper] VEC에서 웹 사이트를 열면 [!UICONTROL Target]이(가) 자동으로 활성화됩니다. 확장 기능에는 조건부 설정이 없습니다. 확장 기능은 SameSite 쿠키 설정을 포함한 모든 설정을 자동으로 처리합니다.
...
* [!DNL Target]의 경우 확장 기능은 [!DNL Target] > [!UICONTROL Administration]의 [!UICONTROL Implementation] UI에서 사용할 수 있는 최신 at.js 버전을 로드하고 at.js에서 작성 라이브러리를 다운로드합니다.
