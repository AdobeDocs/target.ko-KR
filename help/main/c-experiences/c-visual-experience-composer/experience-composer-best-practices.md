...
description: '[!UICONTROL 시각적 경험 작성기]​(VEC)를 사용할 때 경험이 예상대로 작동하도록 하는 모범 사례에 대해 알아봅니다.'
title: '[!UICONTROL 시각적 경험 작성기] 모범 사례 및 제한 사항은 무엇입니까?'
...
# [!UICONTROL 시각적 경험 작성기] 모범 사례 및 제한 사항

경험이 의도한 대로 작동하도록 하려면 [!DNL Adobe Target] [!UICONTROL 시각적 경험 작성기]&#x200B;(VEC)를 사용할 때 모범 사례를 따르십시오. 성능을 극대화하고 일반적인 문제를 방지하기 위한 주요 팁과 제한 사항을 알아 두십시오.
...
+++세부 정보 보기
[!UICONTROL 방문자 API 서비스]도 사용하는 경우 at.js 위에 방문자 API 스크립트를 배치하십시오.

+++

### 계정 수준(계정에서 만들어진 모든 활동에 대해 활성화됨)이나 개별 활동 수준에서 [!UICONTROL 향상된 경험 작성기]를 활성화할 수 있습니다.

+++세부 정보 보기
계정 수준에서 [!UICONTROL 향상된 경험 작성기]를 사용하려면 [!UICONTROL [!UICONTROL 관리] > [!UICONTROL 시각적 경험 작성기]]를 클릭한 다음 [!UICONTROL 향상된 경험 작성기 사용] 스위치를 [켜짐] 위치로 전환하십시오.

[!UICONTROL 시각적 경험 작성기]에서 활동을 만드는 동안 활동 수준에서 [!UICONTROL 향상된 경험 작성기]를 사용하려면 [!UICONTROL 구성 > [!UICONTROL 페이지 전달]]을 클릭한 다음 [!UICONTROL 향상된 경험 작성기 사용] 스위치를 [켜짐] 위치로 전환하십시오.

+++

### [!UICONTROL 향상된 IP Composer]가 사이트의 보안 페이지에서 로드되지 않는 경우 특정 IP 주소를 허용 목록 할 수 있습니다.

+++세부 정보 보기
다음 IP 주소를 허용 목록에 추가 하여 [!UICONTROL 향상된 경험 작성기]를 로드하는 동안 문제가 해결될 수 있습니다. 이러한 IP 주소는 [!UICONTROL 향상된 경험 작성기] 프록시에 사용되는 [!DNL Adobe] 서버용 IP 주소입니다. 활동 편집에만 필요합니다. 사이트 방문자는 이러한 IP 주소를 허용 목록에추가된으로 사용할 필요가 없습니다.
...
[!DNL Target]에는 ID가 필요하지 않지만 ID를 사용하면 경험 작성기로 만든 경험의 안정성이 향상됩니다. [!DNL Target]은(는) 경험이 전달될 때 CSS 선택기를 사용하여 콘텐츠를 수정합니다. 경험을 편집할 때 [!UICONTROL 시각적 경험 작성기]는 수정되는 HTML 요소에 null이 아닌 ID 특성이 있는 가장 가까운 상위 항목에 선택기를 고정합니다. 따라서 HTML ID 속성을 설정하거나 수정하는 JavaScript 라이브러리를 포함한 어떤 메커니즘도 사용하지 않는 것이 좋습니다. [!DNL Target] 경험 작성기에서 해당 ID를 활동 생성에 사용할 수 있지만, JavaScript에서 ID를 수정하는 경우 경험이 생성될 때 사용된 ID는 경험이 실행될 때 사용할 수 없을 수 있습니다. ID를 사용할 수 없는 경우에는 ID에 앵커로 설정된 선택기가 실패합니다.
...
+++세부 사항
블록 수준 요소는 앵커, 범위 등과 같은 인라인 요소 내에서 사용할 수 없습니다. 이렇게 하면 인라인 요소의 높이와 너비가 손실되므로 [!UICONTROL 시각적 경험 작성기]의 오버레이 도구가 예상대로 작동하지 않을 수 있습니다.
...
[[!UICONTROL 시각적 경험 작성기에 사용된 [요소 선택기]]](/help/main/c-experiences/c-visual-experience-composer/vec-selectors.md#concept_4EB7663E255F439B8D24079D23479337)를 참조하십시오.
...
+++세부 사항
[!UICONTROL 향상된 경험 작성기]를 사용하는 경우 모든 링크 URL을 업데이트하여 프록시에서 작동하도록 하는 프록시 서버에서 웹 사이트를 백그라운드에서 조작합니다. 기본 태그를 추가하면 이러한 URL이 모두 브라우저에 의해 확인되므로 손상된 것으로 표시됩니다.
...
+++세부 사항
[!UICONTROL 향상된 경험 작성기]가 활성화되면 iframe 버스팅을 비활성화하도록 top 및 self 변수 값이 업데이트됩니다. 사용자 지정 JavaScript 코드 대신 iframe 버스팅을 추가하려면 X-frame-options 헤더를 사용하십시오.
...
[!UICONTROL 시각적 경험 작성기]를 사용하여 활동을 디자인할 때 다음 주의 사항을 고려하십시오.

### [!UICONTROL 이동] 기능은 z-색인을 지원하지 않습니다.
...
+++세부 사항
스테이징 환경과 같이 라이브가 아닌 사이트에서 [!UICONTROL 향상된 경험 작성기]를 사용하는 경우 사이트에서 RIP를 차단하는 경우 시간 초과 및 액세스 거부 오류가 표시될 수 있습니다.
...
+++세부 사항
[!UICONTROL Button] 요소를 다시 정렬하기 위해 직접 선택할 수 없습니다. 재배치를 사용하려면 더 큰 컨테이너에 버튼을 배치하십시오.
...
+++세부 사항
[!UICONTROL 클래스 편집] 및 [!UICONTROL 다시 정렬]과 같은 작업은 mbox 내에서 허용되지 않습니다.
...
### [!UICONTROL 이미지 변경] 작업은 회전 메뉴에 있는 이미지에 작동하지 않습니다.

+++세부 사항
예를 들어 페이지에 6개의 이미지가 있는 회전 메뉴가 포함되어 있고 이미지를 회전 메뉴의 두 번째 이미지와 바꾸려는 경우 [!UICONTROL 이미지 변경] 작업이 작동하지 않습니다.

해결 방법은 상위 컨테이너를 선택하고 [!UICONTROL HTML 편집] 작업을 사용하여 회전판의 HTML을 편집하여 원하는 이미지의 이미지 소스를 업데이트하는 것입니다.
...
### 이미지를 교체한 후에는 [!UICONTROL 편집] 작업을 선택할 수 없습니다.
...
### [!UICONTROL 컨텐츠] 라이브러리(Scene7)에서 이미지를 삽입하고 HTML을 편집하면 이미지 URL이 손상됩니다.
...
[!UICONTROL 요소 삽입] 작업을 사용하여 이 div를 선택하여 이 더미 텍스트 div의 형제 이미지로 이미지를 삽입합니다.
...
+++세부 사항
VEC에서 클릭 추적에 [!UICONTROL 찾아보기] 모드를 사용할 수 있지만 [!UICONTROL 찾아보기] 모드는 페이지에서 클릭 추적을 추가하는 데 사용할 수 없습니다.
