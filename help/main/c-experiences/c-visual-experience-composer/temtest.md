...
description: Adobe 사용 방법 알아보기 [!DNL Target] VEC(시각적 경험 작성기)를 사용하여 유사하게 구조화되거나 동일한 템플릿 요소를 포함하는 여러 페이지에 동일한 경험을 포함할 수 있습니다.
...
에서 페이지 템플릿 사용 [!DNL Adobe Target] 를 사용하여 페이지에 구조를 제공하거나 페이지에 유사한 요소가 포함되어 있는 경우 유사한 구조의 페이지 요소에서 또는 전체 도메인에서 변형을 테스트할 수 있습니다.
...
변경 요소를 포함하는 페이지를 지정하거나 사이트 또는 도메인 전체에 변경 사항을 적용할 수 있습니다.

1. 에 설명된 대로 활동을 만들거나 편집합니다. [활동](/help/main/c-activities/activities.md#concept_D317A95A1AB54674BA7AB65C7985BA03).

1. 경험이 표시될 페이지를 지정하려면 [!UICONTROL 시각적 경험 작성기] (VEC) 톱니바퀴 아이콘을 클릭한 다음, **[!UICONTROL 페이지 전달]**.
...
1. 원하는 경우 다음을 클릭하여 추가 기준을 입력합니다. **[!UICONTROL 템플릿 규칙 추가]** 이전 단계의 절차를 반복합니다.

   여러 기준이 AND 논리를 사용하여 결합됩니다. [!DNL Target] 지정된 기준과 일치하는 모든 페이지에 경험을 추가합니다.

>[!IMPORTANT]
>
> [!DNL Target] 페이지는 예상대로 표시되는지 확인할 수 없으므로 이 기능을 사용할 때는 영향을 받는 페이지를 공개하기 전에 테스트하는 것이 중요합니다.
...
1. 다음에서 **[!UICONTROL 평가기 선택]** 드롭다운, 선택 **[!UICONTROL 다음 포함]**&#x200B;그런 다음 도메인을 지정합니다.

   ![도메인에 포함된 항목](/help/main/c-experiences/c-visual-experience-composer/assets/domain-template-rule.png)
