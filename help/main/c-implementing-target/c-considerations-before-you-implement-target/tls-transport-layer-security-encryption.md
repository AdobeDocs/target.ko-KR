...
지원되는 브라우저 및 해당 버전에 대한 자세한 내용은 [지원되는 브라우저](https://developer.adobe.com/target/before-implement/supported-browsers/){target=_blank}.
...
Target Standard/Premium 인터페이스에서 이미 [최신 웹 브라우저](https://developer.adobe.com/target/before-implement/supported-browsers/){target=_blank}, 문제가 예상되지 않습니다. Target에 연결할 수 없는 경우 브라우저를 최신 버전으로 업그레이드해야 합니다.
