...
description: 방법 알아보기 [!DNL Target] 페이지 로드 시 워크플로우를 이해하는 데 도움이 되는 시스템 다이어그램을 포함한 at.js JavaScript 라이브러리 함수입니다.
...
구현하려면 [!DNL Adobe Target] 클라이언트측: at.js JavaScript 라이브러리를 사용해야 합니다.
...
에서 [!DNL Target] 아래에 표시된 구현 [!DNL Adobe Experience Cloud] 솔루션 구현: Analytics, Target 및 Audience Manager. 또한 다음 Experience Cloud 핵심 서비스가 구현되었습니다. [!DNL Adobe Experience Platform], [!DNL Audiences], 및 [!DNL Visitor ID Service].
...
* at.js 2.x는 단일 페이지 애플리케이션(SPA)에 사용되는 &quot;보기&quot;라는 개념을 관리합니다. at.js 1.*x*&#x200B;는 이 개념을 알지 못합니다.
...
| 6 | 타깃팅된 콘텐츠는 다시 페이지로 전송되며, 원할 경우 추가적인 개인화를 위한 프로필 값을 포함할 수 있습니다.<br>현재 페이지의 타깃팅된 콘텐츠는 기본 콘텐츠의 플리커 없이 가능한 한 빨리 나타납니다.<br>SPA에서 사용자 작업의 결과로 표시되는 보기에 대한 타깃팅된 콘텐츠는 브라우저에서 캐시되므로 보기를 통해 트리거할 때 추가적인 서버 호출 없이 즉시 적용할 수 있습니다 `triggerView()`. |
