...
* **설명**: 다음을 사용해야 하는지 여부를 나타냅니다 `<clientCode>.tt.omtrdc.net` 도메인 또는 `mboxedge<clusterNumber>.tt.omtrdc.net` 도메인.
   ...
   이 값이 true면 `mboxedge<clusterNumber>.tt.omtrdc.net` 도메인이 쿠키에 저장됩니다. 현재 이 [CNAME](https://developer.adobe.com/target/before-implement/implement-cname-support-in-target/)at.js 1.8.2 및 at.js 2.3.1 이전 버전의 at.js를 사용할 때 target=_blank}. 문제가 되는 경우 [at.js 업데이트](https://developer.adobe.com/target/implement/client-side/atjs/target-atjs-versions/){target=_blank}을 지원되는 최신 버전으로 업그레이드했습니다.
