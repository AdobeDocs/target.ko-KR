---
keywords: 구현;javascript 라이브러리;js;atjs;on-device decisioning;device decisioning;규칙 객체
description: On-Device Decisioning에서 사용하는 JSON 규칙 아티팩트에 대해 알아봅니다.
title: On-Device Decisioning에서 JSON 규칙 아티팩트는 무엇입니까?
feature: at.js
role: Developer
...
규칙 아티팩트는 JSON 표현입니다 [!DNL Adobe Target] 장치 내 의사 결정 활동. 규칙 아티팩트는 [!DNL Target] 를 Akamai CDN에 전파하여 방문자에게 가능한 한 가깝게 사용할 수 있는 규칙 아티팩트가 있는지 확인합니다. 규칙 아티팩트에는 활동의 정확한 실행 및 전달을 보장하는 동시에 이벤트 추적을 통해 실시간 분석을 허용하는 메타데이터가 포함되어 있습니다.

다음은 JSON 규칙 아티팩트의 예입니다.
