...
| 모바일 - 장치 - 모델 이름 | 방문자가 활동에 액세스하는 데 사용한 모바일 장치의 모델 이름입니다. | iPhone XS | MOB_targeting.mobile.modelName |
...
| 모바일 - OS - OSX(Android, Windows 등) | 사용자가 OSX(또는 Android, Windows 등) 장치를 사용하여 활동에 액세스했는지 여부를 지정합니다. | 0이면 False이고, 1이면 True입니다. | MOB_targeting.mobile.os[OS]<br>예를 들어 MOB_targeting.mobile.osOSx, MOB_targeting.mobile.osAndroid, MOB_targeting.mobile.osLinux가 있습니다. |
