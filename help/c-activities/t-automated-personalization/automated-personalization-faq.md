...
* 목표를 기반으로 활동을 시작하기 전에 통제와 타깃팅 간의 할당을 결정하십시오.
   ...
   * **제어력과 활동 목표는 개인화 알고리즘의 효과를 테스트하는 것입니다**. 개인화 알고리즘을 평가하려면 리프트를 정확하게 파악해야 합니다. A/B 테스트 (임의로 제공된 제어) 를 수행한 경우 경험/오퍼에 대한 전환율을 비교할 수도 있습니다. 이 경우, 임의로 제공된 경험의 제어에 50% 할당을 사용하는 것이 권장됩니다.
   * **사용자의 제어력과 활동 목표는 개인화된 트래픽을 극대화하는 것입니다**. 알고리즘을 사용하는 데 익숙하고 트래픽의 최대 양을 원하는 경우 10% ~ 30%의 할당이 권장됩니다. 여기서 단점은 리프트 정보에서 볼 수 있는 정확성입니다 (트래픽으로 유입되는 트래픽이 감소하므로 제어 트래픽의 신뢰 구간이 더 클 수 있으므로).
   * **다음 두 가지 목표 유형을 사용하여 제어로 특정 경험**: 특정 마케터 중심의 경험을 개인화 모델과 비교하려면 10-30%의 할당이 필요합니다. 하나의 경험만 제어로 선택하면 활동이 활동의 모든 오퍼/경험에 걸쳐 확산되지 않습니다.
