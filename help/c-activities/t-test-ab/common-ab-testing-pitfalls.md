...
description: A/B 테스트는 대부분의 디지털 마케팅 최적화 프로그램의 중추를 형성하며 마케터가 최적화 및 타깃팅된 경험을 방문자와 고객에게 제공하는 데 도움이 됩니다. 이 섹션에서는 Adobe Target에서 A/B 테스트를 수행할 때 기업이 당면하게 되는 가장 중요한 문제점 중 9가지에 대해 간략하게 설명합니다. 또한 여기에는 회사가 테스트 노력을 통해 더 많은 ROI를 달성하고 보고된 A/B 테스트 결과를 더욱 신뢰할 수 있도록 이러한 위험을 방지하는 방법도 포함되어 있습니다.
...
A/B 테스트는 대부분의 디지털 마케팅 최적화 프로그램의 중추를 형성하며 마케터가 최적화 및 타깃팅된 경험을 방문자와 고객에게 제공하는 데 도움이 됩니다. 이 섹션에서는 A/B 테스트를 수행할 때 기업이 당면하게 되는 가장 중요한 문제 중 10가지에 대해 간략하게 설명합니다. 또한 여기에는 회사가 테스트 노력을 통해 더 많은 ROI를 달성하고 보고된 A/B 테스트 결과를 더욱 신뢰할 수 있도록 이러한 위험을 방지하는 방법도 포함되어 있습니다.
...
## Pitfall 6:테스트 조기 중지 {#section_DF01A97275E44CA5859D825E0DE2F49F}
...
예를 들어 트래픽의 80%가 경험 A(제어)에 할당되고 트래픽의 20%가 경험 B에 할당되는 A/B 테스트 활동이 있다고 가정합니다.테스트 기간 동안 각 경험에 대한 할당을 50%로 변경합니다. 며칠 후 트래픽 할당을 100%로 경험 B로 변경합니다.
...
테스트 기간 동안 서로 다른 경험에 대한 백분율을 변경하는 경우 특히 많은 구매자가 재방문자를 찾는 경우 데이터가 표준화되려면 며칠이 걸립니다.
다른 예로, A/B 테스트의 트래픽 할당이 50/50으로 분할된 다음 분할을 80/20으로 변경하면 변경 후 처음 며칠 동안 결과가 왜곡되어 보일 수 있습니다. 평균 전환 시간이 높은 경우 구매 시간이 몇 시간 또는 며칠이 소요되므로 이러한 전환 지연은 보고서에 영향을 줄 수 있습니다. 따라서 숫자가 50%에서 80%로 증가했고 평균 전환 시간은 2일입니다. 오늘날 전체 인구의 80%가 경험으로 들어가고 있지만, 전체 인구의 50%가 테스트 첫날 전환하고 있는 첫 번째 경험입니다. 이렇게 하면 전환율이 하락한 것처럼 보이지만 이 방문자의 80%가 이틀 동안 전환한 후 다시 표준화됩니다.
