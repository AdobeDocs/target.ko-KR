...
You can use the existing [sample size calculator](https://docs.adobe.com/content/target-microsite/testcalculator.html) to get an estimate of how long the test will run. (일반적인 A/B 테스트와 마찬가지로 두 개 이상의 오퍼나 두 개 이상의 전환 지표/가설을 테스트하는 경우 Bonferroni 교정을 적용합니다.) 이 계산기는 기존의 고정 수평선 A/B 테스트를 위해 설계되었으며 예상 가격만을 제공합니다. 자동 할당을 사용하면 우승자가 선언되므로 자동 할당 활동에 계산기를 사용하는 것은 선택 사항입니다. 지정된 값을 통계적으로 항상 사용할 수 있으므로 자동 할당에서 우승자를 선언합니다. 우리의 실험에서 우리는 다음과 같은 것을 발견했습니다.
* 두 개의 경험을 정확하게 테스트할 때 자동 할당을 사용하면 경험 간의 성능 차이가 큰 경우 고정 영역 테스트(예: 샘플 크기 계산기로 제시된 일정)보다 우승자를 더 빨리 발견하지만 경험 간의 성능 차이가 작은 경우 우승자를 식별하기 위해 추가 시간이 필요할 수 있습니다. 이러한 경우 고정 영역 테스트는 일반적으로 통계적으로 유의한 결과 없이 끝났을 것입니다.
* 두 개 이상의 경험을 테스트할 때 자동 할당을 사용하면 단일 경험이 다른 모든 경험을 크게 수행하지 못할 때 고정 수평선 테스트(예: 샘플 크기 계산기에서 제안하는 일정)보다 우승자를 보다 빠르게 찾습니다. 두 개 이상의 경험이 다른 경험에 대해 &quot;우승&quot;하지만 서로 가깝게 일치하는 경우 자동 할당에 더 많은 시간이 걸릴 수 있습니다. 이러한 경우 고정 영역 테스트는 일반적으로 &quot;우승&quot; 경험이 낮은 경험보다 우수하지만 어떤 경험이 우수한지 식별하지 못함으로써 끝이 났습니다.

### 자동 할당 활동에서 성과가 낮은 경험을 제거하여 우승자를 결정하는 프로세스를 가속화해야 합니까?

성과가 낮은 경험을 제거할 이유는 없습니다. 자동 할당 기능은 성과가 높은 경험을 보다 자주 제공하고 성과가 낮은 경험을 덜 제공합니다. 성과가 낮은 경험을 활동에 그대로 두면 우승자를 결정하는 속도에 크게 영향을 주지 않습니다.

방문자의 20%가 모든 경험에 무작위로 할당됩니다. 성과가 낮은 경험으로 제공된 트래픽의 양은 최소화됩니다(20%가 경험 수로 나누기).
