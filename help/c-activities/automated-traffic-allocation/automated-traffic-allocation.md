...
You can use the existing [sample size calculator](https://docs.adobe.com/content/target-microsite/testcalculator.html) to get an estimate of how long the test will run. (기존 A/B 테스트와 마찬가지로 두 개 이상의 오퍼나 두 개 이상의 전환 지표/가설을 테스트하는 경우 Bonferroni 교정을 적용합니다.) 이 계산기는 기존의 고정 수평선 A/B 테스트를 위해 설계되었으며 예측값만 제공합니다. 자동 할당 활동은 우승자를 선언하기 때문에 자동 할당 활동에 계산기를 사용하는 것은 선택 사항입니다. 주어진 값은 항상 통계적으로 유효합니다. Adobe는 다음과 같은 실험을 통해
* 두 개의 경험을 정확하게 테스트할 때 자동 할당에서는 경험 간의 성능 차이가 클 때 고정 대상 기간 테스트(예: 샘플 크기 계산기에서 제안하는 일정)보다 우승자를 더 빠르게 찾지만 경험 간의 성능 차이가 작을 때 우승자를 식별하기 위해 추가 시간이 필요할 수 있습니다. 이러한 경우 고정 수평선 테스트는 일반적으로 통계적으로 유의한 결과 없이 끝났을 것입니다.
* 두 개 이상의 경험을 테스트할 때 자동 할당에서는 단일 경험이 다른 모든 경험을 크게 수행하지 않을 때 고정 대상 기간 테스트(예: 샘플 크기 계산기에서 제안하는 일정)보다 우승자를 보다 빠르게 찾습니다. 두 개 이상의 경험이 다른 경험에 대해 &quot;우승&quot;이지만 서로 가깝게 일치하면 자동 할당에 더 많은 시간이 필요할 수 있습니다. 이러한 경우 고정 영역 테스트는 일반적으로 &quot;우승&quot; 경험이 낮은 경험보다 우수하지만 어떤 경험이 우수한지 식별하지 못함으로써 끝났을 것입니다.
