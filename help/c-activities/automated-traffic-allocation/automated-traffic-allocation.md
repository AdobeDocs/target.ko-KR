...
**자동 할당 A/B 활동은 더 이상 Analytics for Target(A4T)에서 지원되지 않습니다.**

16.10.1.0 릴리스(2016년 10월 25일)를 사용 할 경우, 앞으로 Target에서는 자동 할당 A/B 활동의 보고 소스로서 Analytics를 지원하지 않습니다. A4T를 사용하는 모든 활성 상태의 자동 할당 A/B 활동은 수동 모드(동일한 트래픽 할당)로 전환됩니다.
...
You can use the existing [sample size calculator](https://docs.adobe.com/content/target-microsite/testcalculator.html) to get an estimate of how long the test will run. (기존 A/B 테스트와 마찬가지로 2개 이상의 오퍼나 1개 이상의 전환 지표/가설을 테스트하는 경우 Bonferroni 교정을 적용합니다.) 이 계산기는 기존의 고정 수평선 A/B 테스트를 위해 설계되었으며 예측값만 제공합니다. 자동 할당 활동은 우승자를 선언하기 때문에 자동 할당 활동에 계산기를 사용하는 것은 선택 사항입니다. 자동 할당은 테스트 결과를 보기 위해 정해진 시점을 선택할 필요가 없습니다. 제공된 값은 항상 통계적으로 유효합니다. Adobe는 다음과 같은 실험을 통해
* 정확한 2개의 경험을 테스트할 때 자동 할당은 경험 간의 성능 차이가 큰 경우 고정 대상 기간 테스트(예: 샘플 크기 계산기에서 제안하는 일정)보다 우승자를 더 빨리 찾지만 경험 간의 성능 차이가 작은 경우 우승자를 식별하기 위해 추가 시간이 필요할 수 있습니다. 이러한 경우 고정 수평선 테스트는 일반적으로 통계적으로 유의한 결과 없이 끝났을 것입니다.
* 2개 이상의 경험을 테스트할 때 자동 할당에서는 단일 경험이 다른 모든 경험을 크게 수행하지 않을 때 고정 대상 기간 테스트(예: 샘플 크기 계산기에서 제안하는 일정)보다 우승자를 더 빠르게 찾습니다. 두 개 이상의 경험이 다른 경험에 대해 &quot;우승&quot;하지만 서로 가깝게 일치하면 자동 할당에 더 많은 시간이 필요할 수 있습니다. 이러한 경우 고정 영역 테스트는 일반적으로 &quot;우승&quot; 경험이 낮은 경험보다 우수하지만 어떤 경험이 우수한지 식별하지 못함으로써 끝났을 것입니다.
