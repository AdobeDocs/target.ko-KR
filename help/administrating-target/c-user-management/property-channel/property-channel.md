...
채널은 Target 활동이 전달되는 컨텐츠 유형, 즉 웹 페이지, 모바일 앱, 이메일 메시지 등을 나타냅니다.
...
* 모든 내 작업 공간 보기를 사용하는 경우:
   ...
   * [모든 내 작업 공간 보기]를 선택하면 새 열이 항목의 작업 공간 및 해당 항목과 연관된 사용자 권한(관찰자, 편집자 또는 승인자)을 나열한 활동, 대상 및 오퍼 페이지에 추가됩니다.
...
### 속성이 지정된 경우에도 이 활동과 연결되어 있지 않음을 나타내는 오류 메시지가 표시되는 이유는 무엇입니까?

with you implemented [!DNL Target] and [!DNL Adobe Launch] get an error message that there are no property associated with the activity associated, pass the `at_property` parameter with the `targetPageParams` function.

### 리디렉션 추적 전환이 리디렉션 페이지 및 활동 URL 이 다른 속성에 속하는 경우 기록됩니까?

페이지 및 활동 URL 이 다른 속성에 속하는 페이지에 대해서는 클릭 추적이 기록되지 않습니다.

다음 시나리오를 고려하십시오 (. js 및 mbox. js 모두에 적용).

* 페이지 1는 속성 1에 속합니다.
* 2 페이지는 속성 2에 속합니다.
* 활동에서, 페이지 1는 Clicktracks가 포함된 페이지 2로 리디렉션됩니다.

방문자가 브라우저에서 1 페이지를 열면 2 페이지로 리디렉션됩니다. 2 페이지는 활동을 제공할 수 없기 때문에 Target 호출에는 응답에서 Clicktracks가 포함되지 않습니다.

리디렉션 페이지와 활동 URL 이 동일한 속성에 속하는 경우 clicktracks가 예상대로 작동합니다. 자세한 내용은 클릭 추적을 참조하십시오 [](/help/c-activities/r-success-metrics/click-tracking.md).

## 교육 비디오: 엔터프라이즈 권한 교육 비디오 {#section_2FA080303A064242B63FF16CFA6DB31D}
