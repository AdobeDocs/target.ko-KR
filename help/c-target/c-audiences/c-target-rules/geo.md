---
description: 국가, 시/도, 도시, 우편 번호, DMA 또는 이동통신사를 포함하여 지리적 위치를 기반으로 하는 Target 사용자.
...
seo-description: 국가, 시/도, 도시, 우편 번호, DMA 또는 이동통신사를 포함하여 지리적 위치를 기반으로 하는 Target 사용자.
seo-title: 지역
...
국가, 시/도, 도시, 우편 번호, DMA 또는 이동통신사를 포함하여 지리적 위치를 기반으로 하는 Target 사용자.

지리적 위치 매개 변수를 사용하여 방문자의 지리적 위치에 따라 활동과 경험을 타깃팅할 수 있습니다. 국가, 시/도, 도시, 우편 번호, DMA 또는 이동통신사를 기반으로 방문자를 포함하거나 제외할 수 있습니다. 이 데이터는 각 mbox 요청을 통해 전송되고 방문자의 IP 주소를 기준으로 합니다. 이러한 매개 변수는 타깃팅 값과 같이 선택합니다.
...
지리 기반의 타깃팅의 정확도는 몇 가지 요인에 의존합니다. WiFi 연결은 셀룰러 네트워크(무선 통신망)보다 더 정확합니다. 방문자가 셀룰러 데이터 연결을 사용하는 경우 지역 조회의 정확도에는 위치, deviceatlas와의 제공자 데이터 관계 및 기타 요인의 영향을 받을 수 있습니다. 셀 타워 기반 네트워크 연결은 유선 또는 WiFi 연결보다 정확도가 떨어질 수 있습니다. 또한 방문자의 IP 주소는 자신의 ISP 위치에 매핑될 수 있으므로 방문자의 실제 위치와 동일하지 않을 수도 있습니다. 지리 [위치 API를 사용하여 일부 모바일 지리적 위치 문제를 해결할](https://developer.mozilla.org/en-US/docs/Web/API/Geolocation_API)수 있습니다.
...
**다른 위치에서 온 사용자의 경우 캠페인을 어떻게 테스트할 수 있습니까?**
