...
지리적 위치 매개 변수를 사용하여 방문자의 지리적 위치에 따라 활동과 경험을 타깃팅할 수 있습니다. 국가, 시/도, 도시, 우편 번호, 위도, 경도, DMA 또는 이동통신사를 기반으로 방문자를 포함하거나 제외할 수 있습니다. 이 데이터는 각 [!DNL Target] 요청과 함께 전송되며 방문자의 IP 주소를 기반으로 합니다. 이러한 매개 변수는 타깃팅 값과 같이 선택합니다.
...
다음 그림은 44°보다 큰 위도와 22°보다 작은 경도에서 활동에 액세스하는 사용자를 대상으로 하는 대상을 보여줍니다.
...
대부분의 모바일 장치 사용자는 WiFi를 통해 컨텐츠에 액세스합니다. 즉, [!DNL Target]의 IP 기반 지역 타게팅은 데스크탑에서처럼 정확합니다. 기지국 기반의 연결은 방문자의 IP 주소가 신호를 선택하는 기지국을 기준으로 하므로 덜 정확할 수 있습니다. 일부 모바일 지리적 위치 문제는 [지리적 위치 API](https://developer.mozilla.org/en-US/docs/Web/API/Geolocation_API)를 사용하여 해결할 수 있습니다.
...
* **at.js 1.*x***:다른 위치의 IP 주소로 IP 주소를 재정의하고 매개 변수를 사용할 수  `mboxOverride.browserIp url` 있습니다. 따라서 회사가 영국에 있지만 글로벌 캠페인이 뉴질랜드 오클랜드에 있는 방문자를 대상으로 하는 경우, `60.234.0.39`이 오클랜드의 IP 주소임을 가정하는 다음 스타일의 URL을 사용하십시오.
   ...
   >[!NOTE]
   >
   >`mboxOverride.browserIp` 은 at.js 1에서 지원됩니다.*x*&#x200B;에만 사용할 수 있습니다. 이 기능은 at.js 2에서 지원되지 않습니다.*x*.

* **at.js 2.*x***:IP 주소를 at.js 2로 재정의하려면&#x200B;*x*, 브라우저 확장/플러그인(예: 크롬 또는 Firefox용 X-Forwarded-For Header)을 설치합니다. 이 확장을 사용하면 페이지 요청에 x-forwarded-for 헤더를 전달할 수 있습니다.
