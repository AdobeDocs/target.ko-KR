...
description: Adobe Experience Cloud 사람 핵심 서비스에서 고객 속성을 사용하여 Adobe Target에서 컨텐츠 타깃팅을 위해 고객 관계 관리(CRM) 데이터베이스의 기업 고객 데이터를 사용하는 방법에 대한 정보입니다.
...
[핵심](https://docs.adobe.com/content/help/en/core-services/interface/customer-attributes/attributes.html) 서비스의 고객 속성은 [!DNL People] 해당 서비스의 일부이며 기업에 고객 데이터를 [!DNL Adobe Experience Cloud] [!DNL Experience Cloud] 플랫폼으로 전달하는 도구를 제공합니다.
...
1. 에서 메뉴 아이콘( [!DNL Adobe Experience Cloud]메뉴 아이콘 ![)을 클릭한 다음 사용자를](/help/c-target/c-visitor-profile/assets/menu-icon.png) 클릭합니다 ****.
...
1. 속성 소스를 만들고 데이터 파일 업로드.
   ...
   데이터 파일은 업로드 요구 사항을 준수해야 하며 100MB를 초과할 수 없습니다. 파일이 너무 크거나 반복해서 업로드해야 하는 데이터가 있는 경우 FTP로 파일을 전송할 수 있습니다.
...
### 문제 3:Delivery not working based on the attribute
...
### 문제 5:제대로 수행되지 않음 `mboxUpdate`

`mboxUpdate`  가 제대로 수행되지 `mbox3rdPartyId`않았습니다.
