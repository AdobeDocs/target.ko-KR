---
description: 프로필 속성은 방문자와 관련된 매개 변수입니다. 이러한 속성은 방문자의 프로필에 저장되어 캠페인에 사용할 수 있는 방문자에 대한 정보를 제공합니다.
keywords: 프로필 스크립트;프로필 스크립트 속성;프로필 스크립트 우수 사례;디버그;디버깅
seo-description: 프로필 속성은 방문자와 관련된 매개 변수입니다. 이러한 속성은 방문자의 프로필에 저장되어 캠페인에 사용할 수 있는 방문자에 대한 정보를 제공합니다.
seo-title: 프로필 속성
solution: Target
title: 프로필 속성
...
프로필 속성은 방문자와 관련된 매개 변수입니다. 이러한 속성은 방문자의 프로필에 저장되어 캠페인에 사용할 수 있는 방문자에 대한 정보를 제공합니다.

## 프로필 속성 {#concept_01A30B4762D64CD5946B3AA38DC8A201}

프로필 속성은 방문자와 관련된 매개 변수입니다. 이러한 속성은 방문자의 프로필에 저장되어 캠페인에 사용할 수 있는 방문자에 대한 정보를 제공합니다.

방문자가 찾아보거나 다른 세션을 위해 돌아가면 저장된 프로필 특성을 사용하여 컨텐츠를 타깃팅하거나 세그먼트 필터링을 위해 정보를 기록할 수 있습니다.
...
프로필 스크립트를 사용하여 여러 방문에서 방문자 속성을 캡처할 수 있습니다. 프로필 스크립트는 서버측 JavaScript의 한 형태를 사용하여 Target 내에 정의된 코드 조각입니다. 예를 들어 프로필 스크립트를 사용하여 방문자가 사이트를 방문하는 빈도와 마지막으로 방문한 시기를 캡처할 수 있습니다.
...
* 매개 변수와 값은 대/소문자를 구분합니다. 캠페인이나 테스트 중에 받게 되는 매개 변수 및 값의 대소문자를 맞추십시오.
* 자세한 JavaScript 구문은 아래의 &quot;스크립트 프로필 매개 변수에 대한 JavaScript 참조&quot; 섹션을 참조하십시오.
...
## Target 특정 상황에서 프로필 스크립트 비활성화 {#section_C0FCB702E60D4576AD1174D39FBBE1A7}
...
## 모범 사례 {#section_64AFE5D2B0C8408A912FC2A832B3AAE0}
...
## 상호 배타적인 활동을 테스트하는 프로필 스크립트 {#section_FEFE50ACA6694DE7BF1893F2EFA96C01}
...
## 디버그 프로필 스크립트 {#section_E9F933DE47EC4B4E9AF2463B181CE2DA}
...
## 스크립트 프로필 매개 변수를 위한 JavaScript 참조

스크립트 프로필
매개 변수를 효과적으로 사용하려면 간단한 Javascript 지식이 필요합니다. 이 섹션에서는 단 몇 분 만에 이 기능을 사용하여 생산성을 높일 수 있는 빠른 참조 역할을 합니다.

스크립트 프로필 매개 변수는 mbox/Profiles 탭 아래에 있습니다. Javascript 유형 (문자열, 정수, 배열 등) 를 반환하는 Javascript 프로그램을 작성할 수 있습니다.
...
일 단위로 밀리초 단위로 변수를 만듭니다. mbox 이름이 있으면 `orderThankyouPage`현재 날짜 및 시간의 값을 갖도록 명명된 `lastPurchaseTime` 로컬 (보이지 않는) 사용자 프로필 속성을 설정하십시오. 마지막 구매 시간의 값을 읽고, 정의된 경우 마지막 구매 시간 이후에 경과된 시간을 하루 밀리초 (마지막 구매 이후 일 수) 로 나누어서 반환합니다.
...
이전 값이 없는 경우 이전 값 또는 0로 초기화하는 빈도를 만듭니다. mbox 이름이 있으면 `orderThankyouPage`증분 값이 반환됩니다.
...
지정된 방문자에 대한 `monetaryValue`현재 값을 조회하는 변수를 만들거나 이전 값이 없는 경우 0로 설정합니다. mbox 이름이 있으면 `orderThankyouPage`이전 ID와 mbox에 전달된 `orderTotal` 매개 변수의 값을 추가하여 새 통화 값을 반환합니다.
...
| --- | --- |
| `page.url` | 현재 URL. |
| `page.protocol` | 페이지에 사용된 프로토콜 (http 또는 https) 입니다. |
| page.domain | 현재 URL 도메인 (첫 번째 슬래시 앞의 모든 것). for example, `www.acme.com` in `http://www.acme.com/categories/men_jeans?color=blu e&size=small`. |
| `page.query` | 현재 페이지의 쿼리 문자열입니다. &#39;? &#39;. for example, `blue&size=small` in `http://www.acme.com/categories/mens_jeans?color=blue&size=small`. |
| `page.param(‘<par_name>’)` | 가 나타내는 매개 변수의 값 `<par_name>`. 현재 URL 이 Google의 검색 페이지이고 입력한 경우 `page.param('hl')`URL `http://www.google.com/search?hl=en& q=what+is+asdf&btnG=Google+Search`에 대해 &quot;en&quot; 를 받습니다. |
| `page.referrer` | 위와 동일한 작업 세트가 레퍼러 및 랜딩에 적용됩니다 (예: referrer. url는 레퍼러의 URL 주소). |
| `landing.url`, `landing.protocol`, `landing.query`, 및 `landing.param` | 페이지 전체와 비슷하지만 랜딩 페이지용입니다. |
| `mbox.name` | 활성 mbox 이름. |
| `mbox.param(‘<par_name>’)` | 활성 mbox에서 주어진 이름의 mbox 매개 변수. |
| `profile.get(‘<par_name>’)` | 이름으로 `<par_name>`클라이언트가 생성한 사용자 프로필 매개 변수 예를 들어 사용자가 «gender» 라는 프로필 매개 변수를 설정하면 «profile. gender» 를 사용하여 값을 추출할 수 있습니다. 현재 방문자에 대한 &quot;`profile.<par_name>`» 세트의 값을 반환합니다. 설정된 값이 없으면 null를 반환합니다. |
| `user.get(‘<par_name>’)` | 현재 방문자에 대한 &quot;`user.<par_name>`» 세트의 값을 반환합니다. 설정된 값이 없으면 null를 반환합니다. |
| `user.categoryAffinity` | 가장 좋은 범주의 이름을 반환합니다. |
| `user.categoryAffinities` | 가장 좋은 카테고리가 있는 배열을 반환합니다. |
| `user.isFirstSession` | 방문자의 첫 번째 세션인 경우 true를 반환합니다. |
| `user.browser` | HTTP 헤더의 사용자 에이전트를 반환합니다. 예를 들어 Safari 사용자를 대상으로 하는 표현식 타겟을 만들 수 있습니다. `if (user.browser != null && user.browser.indexOf('Safari') != -1) { return true; }` |

### 일반 연산자


모든 표준 JavaScript 연산자가 존재하며 사용할 수 있습니다. JavaScript 연산자는 문자열 및 숫자와 기타 데이터 유형에서 사용할 수 있습니다. 간단한 요약:

| 연산자 | 설명 |
| --- | --- |
| `==` | equality를 나타냅니다. 양쪽 피연산자가 동일한 경우 true를 유지합니다. |
| `!=` | inequality를 나타냅니다. 양쪽 피연산자가 동일하지 않은 경우 true를 유지합니다. |
| `<` | 왼쪽의 변수가 오른쪽의 변수보다 작음을 나타냅니다. 변수가 동일한 경우 false로 평가됩니다. |
| `>` | 왼쪽의 변수가 오른쪽의 변수보다 크다는 것을 나타냅니다. 변수가 동일한 경우 false로 평가됩니다. |
| `<=` | 변수가 동일한 경우는 `<` 제외하고 true로 평가됩니다. |
| `>=` | 변수가 동일한 경우는 `>` 제외하고 true로 평가됩니다. |
| `&&` | 표현식의 왼쪽과 오른쪽에 논리적으로 «EAN» 표현식으로 표시됩니다. 양쪽이 참이면 true를 반환하고 그렇지 않으면 false를 반환합니다. |
| `||` | 논리적으로 표현식의 왼쪽과 오른쪽에 &quot;정렬&quot; 합니다. 측면 중 하나가 true 이면 true (그렇지 않으면 false) 입니다. |
| `//` | 소스에 Target Boolean의 모든 요소 (Array Source, Array Target) 가 포함되어 있는지 확인합니다.<br>`//` Target (regexp에 해당함) 에서 하위 문자열을 추출하고 `Array/*String*/ decode(String encoding, String regexp, String target)`디코딩합니다.<br>이 기능은 상수 문자열 값, 그룹화 (`condition1 || condition2) && condition3``/[^a-z]$/.test(landing.referring.url)`및 |
