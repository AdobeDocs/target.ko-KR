---
keywords: 타겟 지정
description: 각 경험에 대한 Adobe [!DNL Target] 전환율, 상승도, 신뢰도(통계적 중요도) 및 신뢰 구간을 표시하고 계산하는 방법을 알아봅니다.
...
이러한 계산된 지표를 계산하려면 Target의 [Complete Confidence Calculator](/help/assets/complete_confidence_calculator.xlsx) Excel 파일을 다운로드하여 활동의 값을 입력하거나, Target](/help/assets/statistical-calculations.pdf)에서 사용하는 통계 계산을 검토하십시오.[
...
A4T의 경우 연속 변수(2진 지표가 아님)를 위한 학생 t 검증(t-test) 계산을 사용합니다. Analytics에서 방문자는 항상 추적되며 수행된 모든 작업이 카운트됩니다. 따라서 방문자가 여러 번 구입하거나 성공 지표를 여러 번 방문하면 해당 추가 히트가 카운트됩니다. 이 작업으로 지표는 연속 변수가 됩니다. student&#39;s t-test 계산을 수행하려면 t-statistics의 분모로 사용되는 분산을 계산하기 위해 &quot;정사각형의 합계&quot;가 필요합니다. [이 문서에서는 ](/help/assets/statistical-calculations.pdf) 사용된 수식의 세부 사항에 대해 설명합니다. 제곱의 합은 [!DNL Analytics]에서 검색할 수 있습니다. 제곱합 데이터를 얻으려면, 최적화할 지표에 대한 방문자 수준 내보내기를 샘플 기간 동안 수행해야 합니다.
...
## [!DNL Target]이 학생 t 테스트를 사용하도록 권장하는 이유는 무엇입니까? {#t-test}

A/B 테스트는 제어 변형(경험이라고도 함)에 있는 일부 비즈니스 지표의 평균 값을 하나 이상의 대체 경험에서 동일한 지표의 평균 값과 비교하는 실험입니다.

[!DNL Target] 에서는 z 테스트와 같은 대안보다 더 적은 가정을 필요로 하므로 두 개의 샘플  [학생 t-테스트](https://en.wikipedia.org/wiki/Student%27s_t-test#:~:text=The%20t%2Dtest%20is%20any,the%20test%20statistic%20were%20known.)를 사용할 것을 권장합니다. 또한 제어 경험과 대체 경험 간에 (수량) 비즈니스 지표를 쌍으로 비교하기 위한 적절한 통계 테스트입니다.
...
비즈니스 지표 자체가 일부 임의 분포에 따라 배포될 수 있지만(각 변형 내에서) 이 지표의 평균의 분포는 [중앙 제한 정리](https://en.wikipedia.org/wiki/Central_limit_theorem)를 통해 일반 분포로 수렴해야 합니다. 이 평균 샘플링 분배가 얼마나 빨리 정상으로 수렴될 지에 대한 보장은 없지만, 일반적으로 온라인 테스트의 방문자 수를 기준으로 이 조건은 달성됩니다.

이러한 평균의 이 경우, 사용할 테스트 통계가 t 분포를 따르도록 표시될 수 있습니다. 이 값은 데이터로부터 추정치를 기반으로 하는 스케일링 용어(수단의 차이점)에 대해 정상적으로 분산된 값의 비율(비즈니스 지표에 대한 차이점)이기 때문입니다. 그러면 테스트 통계가 t-배포를 따르는 경우 **Student&#39;s t-test**&#x200B;가 적절한 가설 테스트입니다.

### 다른 테스트를 사용하지 않는 이유

**z-test**&#x200B;는 일반적인 A/B 테스트 시나리오에서 테스트 통계의 분모는 알려진 분산에서 파생되지 않으므로 부적합하며 대신 데이터에서 예측해야 합니다.

**카이 제곱** 테스트는 두 변형 간에 정성적 관계가 있는지 여부(즉, 변형 간에 차이가 없다는 null 가설)를 결정하는 데 적절하므로 사용되지 않습니다. T 테스트는 _지표 비교 시나리오에 더 적합합니다._

**Mann-Whitney U 테스트**&#x200B;는 비패라메트릭 테스트이며, 각 변형에 대해 평균 비즈니스 지표의 샘플링 분배가 정상적으로 배포되지 않을 때 적합합니다. 그러나 앞서 논의했듯이, 온라인 테스트에 관련된 트래픽의 크기를 고려할 때, 일반적으로 중앙 제한 정리 가 적용되므로 t-테스트가 안전하게 적용될 수 있습니다.

테스트에 두 개 이상의 경험(&quot;A/Bn 테스트&quot;)이 있는 경우 **ANOVA**(두 개 이상의 변형에 대한 t 테스트를 일반화)와 같은 더 복잡한 메서드를 적용할 수 있습니다. 하지만, ANOVA는 &quot;모든 변형이 동일한 평균을 가지고 있는지&quot; 라는 질문에 답하고, 일반적인 A/Bn 테스트에서는 특정한 변형이 가장 좋은 _에 더 관심이 있습니다._ 따라서 [!DNL Target]에서는 여러 비교를 설명하기 위해 각 변형을 제어와 비교하는 정규 t-테스트를 적용합니다.
