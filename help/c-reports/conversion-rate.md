...
비즈니스 지표 자체가 일부 임의의 분포에 따라 배포될 수 있지만, 각 변형 내에서 이 지표의 평균의 분포는 [중앙 한계 정리](https://en.wikipedia.org/wiki/Central_limit_theorem)를 통해 일반적인 분포로 수렴해야 합니다. 이 샘플 분포가 평균으로 얼마나 빨리 수렴되는지에 대한 보장이 없지만, 일반적으로 이 조건은 온라인 테스트의 방문자 수 수준에 따라 달성됩니다.

이러한 평균을 고려하면, 일반적으로 분포된 값(비즈니스 메트릭의 차이점)의 비율이며 데이터로부터 추정치를 기반으로 하는 크기 조절 용어(즉, 의미 차이의 표준 오류)의 비율이기 때문에 사용할 테스트 통계를 t 분포를 따르는 것으로 표시할 수 있습니다. T-분포에 따라 테스트 통계가 작성되므로 **Student&#39;s t-test**&#x200B;는 적절한 가설 테스트입니다.

### 다른 테스트를 사용하지 않는 이유

일반적인 A/B 테스트 시나리오에서 테스트 통계의 분모는 알려진 변화로부터 파생되지 않으므로 대신 데이터에서 추정해야 하기 때문에 **z-test**&#x200B;는 적합하지 않습니다.

**카이 제곱** 테스트는 두 변형 간 질적 관계(즉, 변형 간 차이가 없다는 null 가정)를 확인하는 데 적합하기 때문에 사용되지 않습니다. T 테스트는 메트릭 비교 _양적 비교 시나리오에 더 적합합니다._

**Mann-Whitney U 테스트**&#x200B;는 비파라메트릭 테스트이며, 각 변형에 대해 평균 비즈니스 메트릭의 샘플링 분배가 정상적으로 배포되지 않을 때 적합합니다. 그러나 앞서 논의했듯이, 온라인 실험에 관련된 트래픽의 강도를 고려했을 때, 중앙 한계 정리는 일반적으로 적용되며 t-테스트가 안전하게 적용될 수 있습니다.

테스트에는 두 개 이상의 경험(&quot;A/Bn 테스트&quot;)이 있을 때 **ANOVA** 등의 보다 복잡한 메서드가 적용될 수 있습니다. 그러나 ANOVA는 일반적인 A/B 테스트에서는 특정 변형&#x200B;_이 가장 적합한_&#x200B;에 더 관심이 있는 반면 &quot;모든 변형의 평균이 동일한지&quot; 질문에 대한 대답을 제공합니다. 따라서 [!DNL Target]에서는 여러 비교를 고려하여 Bonferroni 교정을 통해 각 변형을 컨트롤에 비교하는 정규 t-테스트를 적용합니다.
