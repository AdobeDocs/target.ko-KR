...
seo-description: 중요 속성 보고서에 대한 정보이며 두 전문 보고서 중 하나는 자동화된 개인 설정(AP) 및 Auto-Target(AT) 활동의 사용자가 사용할 수 있습니다.
seo-title: 중요 속성 보고서
solution: Target
title: 중요 속성 보고서
...
| 모델 속성 등급 차트 | 모델 속성 등급에는 Target의 개인화 모델이 각 방문자를 표시하는 컨텐츠를 결정하는 데 가장 중요한 상위 10개 속성이 포함되어 있습니다. 중요도 점수는 상위 100개 속성을 기준으로 하여 이 활동에서 특정 속성이 Target의 개인화 모델에 얼마나 중요한지를 보여 줍니다. |
...
**[!UICONTROL ]수입을 기반으로 한 모델링 목표/기본 목표에 개인화 인사이트를 사용할 수 있습니까?**
...
자동화된 세그먼트 내에서 아래 전환 혜택/경험에 대한 방문 수가 더 많이 표시될 가능성이 있는 이유는 다음과 같습니다.

* 특정 자동화된 세그먼트의 일부 또는 전체 오퍼/경험에 대한 보기 횟수가 적습니다.
* 특정 오퍼나 경험이 구축되지 않은 낮은 볼륨 활동.
* 일부 제안/경험에 대해 모델이 보다 일찍 빌드된 볼륨 활동이 더 적습니다. 예를 들어, 추가 모델이 22 일에 구축되었고, 사용자가 일 10-24 일부터의 데이터를 보고 있다고 가정합니다.
* 특정 오퍼에 대한 타깃팅 규칙을 참조하십시오.
* 인사이트 보고에는 신뢰 구간이 없습니다. 그러나 전환 비율이 충분히 가까우면 모델은 트래픽이 포인트 금액보다 높도록 트래픽을 제공할 수 있지만 &quot;통계적으로 다른&quot; 숫자가 아닙니다.

트래픽을 제공하는 모델이 어떻게 작동하는지 파악하는 것이 도움이 될 수 있습니다. 각 개인은 자신의 전체 프로필을 기반으로 제공됩니다. 하지만 인사이트 보고서는 이 동작을 일반화합니다. 따라서 세그먼트는 상호 배타적이 아닙니다. 이렇게 하면 동일한 사람이 여러 세그먼트에 나타날 수 있으므로 이러한 유형의 동작을 표시하는 개별 세그먼트가 표시됩니다.
...
* 방문자에게 적용된 컨텐츠 종류에 대한 인사이트를 얻습니다. 즉, 오퍼가 방문자에 대한 리프트를 담당합니다.
* 성과가 낮은 컨텐츠를 식별합니다.
