...
[계정에 저장된 활동이 너무 많으면 저장된 활동에 대한 활동 QA 미리 보기](/help/c-activities/c-activity-qa/activity-qa.md) 링크가 로드되지 않을 수 있습니다. 미리 보기 링크를 다시 시도합니다. 이 문제가 계속 발생하지 않도록 하려면 더 이상 적극적으로 사용되지 않는 저장된 활동을 보관합니다. (TNT-32697)
...
* 권장 사항 피드 UI에 올바른 색인화 상태가 표시되지 않습니다. 백엔드 작업은 제대로 작동하지만 UI가 현재 상태를 가져와 표시할 수 없습니다.
   ...
   **해결 방법**: 지정된 호스트 그룹에 대한 권장 사항 피드가 제대로 인덱싱되었는지를 확인하는 또 다른 방법은 제품 검색 UI(관리자로 로그인됨)를 확인하고 마지막 인덱싱 시간을 확인하는 것입니다. 이 타임스탬프는 지정된 호스트 그룹의 피드가 마지막으로 색인화된 시간을 나타냅니다. (TGT-27116)
